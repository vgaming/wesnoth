...
const t_translation::terrain_code& terrain_palette::selected_fg_item() const
{
	return fg_terrain;
}
const t_translation::terrain_code& terrain_palette::selected_bg_item() const
{
	return bg_terrain;
}
...
void terrain_palette::setup(const game_config_view& cfg)
{
	...
	for(const t_translation::terrain_code& t : items) {
		...
		DBG_ED << "Palette: processing terrain " << t_info.name() << "(editor name: '" << t_info.editor_name() << "') "
			   << "(" << t_info.number() << ")"
			   << ": " << t_info.editor_group();
...
void terrain_palette::setup_item(const t_translation::terrain_code& terrain,
	texture& base_image,
