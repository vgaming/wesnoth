...
tristate_button::tristate_button(editor::tristate_palette* palette, std::string button_image_name, const bool auto_join)
	: widget(auto_join)
	, baseImage_()
	, touchedBaseImage_()
	, activeBaseImage_()
	...
	, item_id_()
{
	// TODO: highdpi - there seem to be higher-quality 74-px versions of these available, but as that's not actually an
	// integer multiple of 38... this is not a straight swap

	if(button_image_name.empty()) {
		button_image_name = "buttons/button_selectable/button_selectable_38_";
	}

	baseImage_ = image::get_texture(button_image_name + "base.png");
	activeBaseImage_ = image::get_texture(button_image_name + "base-active.png");
	touchedBaseImage_ = image::get_texture(button_image_name + "base-touched.png");

	touchedBothImage_ = image::get_texture(button_image_name + "border-touched-both.png");
	touchedUpImage_ = image::get_texture(button_image_name + "border-touched-up.png");
	touchedDownImage_ = image::get_texture(button_image_name + "border-touched-down.png");

	pressedUpImage_ = image::get_texture(button_image_name + "border-pressed-up.png");
	pressedDownImage_ = image::get_texture(button_image_name + "border-pressed-down.png");
	pressedBothImage_ = image::get_texture(button_image_name + "border-pressed-both.png");

	pressedUpActiveImage_ = image::get_texture(button_image_name + "border-active-pressed-up.png");
	pressedDownActiveImage_ = image::get_texture(button_image_name + "border-active-pressed-down.png");
	pressedBothActiveImage_ = image::get_texture(button_image_name + "border-active-pressed-both.png");
...
{
	if(hit(event.x, event.y)) { // the cursor is over the widget
...
void tristate_button::mouse_down(const SDL_MouseButtonEvent& event)
{
	...
	// The widget is expected to be in one of the "active" states when the mouse cursor is hovering over it, but that
	// currently doesn't happen if the widget is moved under the cursor by scrolling the palette.
