...
void editor_map::sanity_check()
{
	...
	if(h() + 2 * border_size() != total_height()) {
		ERR_ED << "w is " << w() << " and border_size is " << border_size() << " but total_height is "
			   << total_height();
		++errors;
	}
...
std::set<map_location> editor_map::get_contiguous_terrain_tiles(const map_location& start) const
{
	...
	do {
		for(const map_location& adj : get_adjacent_tiles(queue.front())) {
			if(on_board_with_border(adj) && get_terrain(adj) == terrain && result.find(adj) == result.end()) {
				result.insert(adj);
				queue.push_back(adj);
			}
		}
...
std::set<map_location> editor_map::set_starting_position_labels(display& disp)
{
	...
	for(const auto& pair : special_locations().left) {
		bool is_number = std::find_if(pair.first.begin(), pair.first.end(), [](char c) { return !std::isdigit(c); })
			== pair.first.end();
		if(is_number) {
...
void editor_map::resize(int width, int height, int x_offset, int y_offset, const t_translation::terrain_code& filler)
{
	int old_w = w();
	int old_h = h();
	...
	if(x_offset || y_offset) {
		for(auto it = special_locations().left.begin(); it != special_locations().left.end(); ++it) {
			special_locations().left.modify_data(
				it, [=](t_translation::coordinate& loc) { loc.add(-x_offset, -y_offset); });
		}
	}
