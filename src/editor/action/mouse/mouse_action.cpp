...
std::set<map_location> mouse_action::affected_hexes(editor_display& /*disp*/, const map_location& hex)
{
...
std::unique_ptr<editor_action> mouse_action::drag_left(
	editor_display& /*disp*/, int /*x*/, int /*y*/, bool& /*partial*/, editor_action* /*last_undo*/)
{
	return nullptr;
}

std::unique_ptr<editor_action> mouse_action::drag_right(
	editor_display& /*disp*/, int /*x*/, int /*y*/, bool& /*partial*/, editor_action* /*last_undo*/)
{
	return nullptr;
}

std::unique_ptr<editor_action> mouse_action::drag_end_left(editor_display& /*disp*/, int /*x*/, int /*y*/)
{
	return nullptr;
}

std::unique_ptr<editor_action> mouse_action::drag_end_right(editor_display& /*disp*/, int /*x*/, int /*y*/)
{
	return nullptr;
}

std::unique_ptr<editor_action> mouse_action::up_right(editor_display& /*disp*/, int /*x*/, int /*y*/)
{
	return nullptr;
}

std::unique_ptr<editor_action> mouse_action::up_left(editor_display& /*disp*/, int /*x*/, int /*y*/)
{
	return nullptr;
}

std::unique_ptr<editor_action> mouse_action::key_event(editor_display& disp, const SDL_Event& event)
{
	...
	if((has_alt_modifier() && (event.key.keysym.sym >= '1' && event.key.keysym.sym <= '9'))
		...
		if(res > gamemap::MAX_PLAYERS || event.key.keysym.sym == SDLK_DELETE)
			res = 0;
...
void mouse_action::set_terrain_mouse_overlay(
	editor_display& disp, const t_translation::terrain_code& fg, const t_translation::terrain_code& bg)
{
	...
	path << blank_hex << "~BLIT(" << fg_path << "~SCALE(" << new_size << "," << new_size << ")," << offset << ","
		 << quarter_size << ")"
		 << "~BLIT(" << bg_path << "~SCALE(" << new_size << "," << new_size << ")," << half_size << "," << quarter_size
		 << ")";
...
std::set<map_location> brush_drag_mouse_action::affected_hexes(editor_display& /*disp*/, const map_location& hex)
{
...
std::unique_ptr<editor_action> brush_drag_mouse_action::drag_left(
	editor_display& disp, int x, int y, bool& partial, editor_action* last_undo)
{
	return drag_generic<&brush_drag_mouse_action::click_perform_left>(disp, x, y, partial, last_undo);
}

std::unique_ptr<editor_action> brush_drag_mouse_action::drag_right(
	editor_display& disp, int x, int y, bool& partial, editor_action* last_undo)
{
	return drag_generic<&brush_drag_mouse_action::click_perform_right>(disp, x, y, partial, last_undo);
}

std::unique_ptr<editor_action> brush_drag_mouse_action::drag_end(editor_display& /*disp*/, int /*x*/, int /*y*/)
{
	return nullptr;
}

template<std::unique_ptr<editor_action> (brush_drag_mouse_action::*perform_func)(
	editor_display&, const std::set<map_location>&)>
std::unique_ptr<editor_action> brush_drag_mouse_action::drag_generic(
	editor_display& disp, int x, int y, bool& partial, editor_action* last_undo)
{
	map_location hex = disp.hex_clicked_on(x, y);
	move(disp, hex);
	...
	editor_display& /*disp*/, const std::set<map_location>& hexes)
{
	...
	chain->append_action(
		std::make_unique<editor_action_paint_area>(hexes, terrain_palette_.selected_fg_item(), has_shift_modifier()));
	return chain;
}
	...
	editor_display& /*disp*/, const std::set<map_location>& hexes)
{
	...
	auto chain = std::make_unique<editor_action_chain>();
	chain->append_action(
		std::make_unique<editor_action_paint_area>(hexes, terrain_palette_.selected_bg_item(), has_shift_modifier()));
	return chain;
}

void mouse_action_paint::set_mouse_overlay(editor_display& disp)
{
	set_terrain_mouse_overlay(disp, terrain_palette_.selected_fg_item(), terrain_palette_.selected_bg_item());
}
...
std::set<map_location> mouse_action_paste::affected_hexes(editor_display& /*disp*/, const map_location& hex)
{
...
void mouse_action_paste::set_mouse_overlay(editor_display& disp)
{
	disp.set_mouseover_hex_overlay(image::get_texture(
		...
		image::locator("misc/blank-hex.png", "~BLIT(icons/action/editor-paste_60.png,6,6)")));
}

std::set<map_location> mouse_action_fill::affected_hexes(editor_display& disp, const map_location& hex)
{
...
std::unique_ptr<editor_action> mouse_action_fill::click_left(editor_display& disp, int x, int y)
{
	...
	if(has_ctrl_modifier()) {
		...
		return std::make_unique<editor_action_fill>(hex, terrain_palette_.selected_fg_item(), has_shift_modifier());
	}
}
...
std::unique_ptr<editor_action> mouse_action_fill::click_right(editor_display& disp, int x, int y)
{
	...
	if(has_ctrl_modifier()) {
		 ...
		 */
		// or use a different key modifier for that
		return std::make_unique<editor_action_fill>(hex, terrain_palette_.selected_bg_item(), has_shift_modifier());
	}
}
...
std::unique_ptr<editor_action> mouse_action_starting_position::click_left(
...
std::unique_ptr<editor_action> mouse_action_starting_position::click_right(
	editor_display& /*disp*/, int /*x*/, int /*y*/)
{
	return nullptr;
}
...
void mouse_action_starting_position::set_mouse_overlay(editor_display& disp)
{
	disp.set_mouseover_hex_overlay(image::get_texture(
		// center 60px icon on blank hex template
		image::locator("misc/blank-hex.png", "~BLIT(icons/action/editor-tool-starting-position_60.png,6,6)")));
}
