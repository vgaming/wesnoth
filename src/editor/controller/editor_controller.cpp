...
editor_controller::editor_controller(bool clear_id)
	...
	, tods_()
	, context_manager_(
		  new context_manager(*gui_.get(), game_config_, clear_id ? "" : editor_controller::current_addon_id_))
	, toolkit_(nullptr)
	, tooltip_manager_()
...
void editor_controller::init_gui()
{
	...
	//	Note that if that is what happens it might not actually be a good idea for the gui to own the halo manager, so
	//that it can be swapped out 	without deleting it.
}
...
bool editor_controller::quit_confirm()
{
	...
	if(amount == 0) {
		...
		message = _("Do you really want to quit? The following maps were modified and all changes since the last save "
					"will be lost:");
		message += "\n" + modified;
	}
...
bool editor_controller::can_execute_command(const hotkey::ui_command& cmd) const
{
	...
	case HOTKEY_UNIT_DESCRIPTION: {
		...
		return (toolkit_->is_mouse_action_set(HOTKEY_EDITOR_TOOL_UNIT) && units.find(loc) != units.end());
	}
	...
	case HOTKEY_EDITOR_MAP_REVERT:
		return !get_current_map_context().get_filename().empty() && get_current_map_context().modified();
	...
	case HOTKEY_EDITOR_CLIPBOARD_FLIP_VERTICAL:
		return !context_manager_->clipboard_empty() && toolkit_->is_mouse_action_set(HOTKEY_EDITOR_CLIPBOARD_PASTE);
...
hotkey::ACTION_STATE editor_controller::get_action_state(const hotkey::ui_command& cmd) const
{
	...
	case HOTKEY_EDITOR_AUTO_UPDATE_TRANSITIONS:
		return context_manager_->is_active_transitions_hotkey("editor-auto-update-transitions") ? ACTION_SELECTED
																								: ACTION_DESELECTED;
	case HOTKEY_EDITOR_PARTIAL_UPDATE_TRANSITIONS:
		return context_manager_->is_active_transitions_hotkey("editor-partial-update-transitions") ? ACTION_SELECTED
																								   : ACTION_DESELECTED;
	case HOTKEY_EDITOR_NO_UPDATE_TRANSITIONS:
		return context_manager_->is_active_transitions_hotkey("editor-no-update-transitions") ? ACTION_SELECTED
																							  : ACTION_DESELECTED;
	case HOTKEY_EDITOR_BRUSH_1:
	...
	case HOTKEY_EDITOR_SELECT_ALL:
		return get_current_map_context().map().everything_selected() ? ACTION_SELECTED : ACTION_DESELECTED;
	case HOTKEY_EDITOR_SELECT_NONE:
		return get_current_map_context().map().selection().empty() ? ACTION_SELECTED : ACTION_DESELECTED;
	case HOTKEY_EDITOR_TOOL_FILL:
	case HOTKEY_EDITOR_TOOL_LABEL:
	case HOTKEY_EDITOR_TOOL_PAINT:
	case HOTKEY_EDITOR_TOOL_SELECT:
	...
	case HOTKEY_NULL:
		switch(active_menu_) {
		case editor::MAP:
			return index == context_manager_->current_context_index() ? ACTION_SELECTED : ACTION_DESELECTED;
		case editor::LOAD_MRU:
			return ACTION_STATELESS;
		case editor::PALETTE:
			return ACTION_STATELESS;
		case editor::AREA:
			return index == get_current_map_context().get_active_area() ? ACTION_SELECTED : ACTION_DESELECTED;
		case editor::ADDON:
			return ACTION_STATELESS;
		case editor::SIDE:
			return static_cast<std::size_t>(index) == gui_->playing_team_index() ? ACTION_SELECTED : ACTION_DESELECTED;
		case editor::TIME:
			return index == get_current_map_context().get_time_manager()->get_current_time() ? ACTION_SELECTED
																							 : ACTION_DESELECTED;
		case editor::LOCAL_TIME:
			return index
					== get_current_map_context().get_time_manager()->get_current_area_time(
						get_current_map_context().get_active_area())
				? ACTION_SELECTED
				: ACTION_DESELECTED;
		case editor::MUSIC:
			return get_current_map_context().is_in_playlist(music_tracks_[index].id()) ? ACTION_ON : ACTION_OFF;
		case editor::SCHEDULE: {
...
bool editor_controller::do_execute_command(const hotkey::ui_command& cmd, bool press, bool release)
{
	...
	case HOTKEY_NULL:
		...
		case AREA: {
			...
			const std::set<map_location>& area = get_current_map_context().get_time_manager()->get_area_by_index(index);
	...
	case HOTKEY_EDITOR_OPEN_ADDON: {
		...
		dlg.set_title(_("Add-on Files")).set_path(filesystem::get_current_editor_dir(current_addon_id_));
		...
		if(dlg.show()) {
			...
			if(filesystem::is_map(filepath) || filesystem::is_cfg(filepath)) {
				...
				if(desktop::open_object_is_supported()) {
					...
					gui2::show_message("", _("Opening files is not supported, contact your packager"),
	...
	case HOTKEY_DELETE_UNIT: {
		...
		return true;
	case HOTKEY_EDITOR_CLIPBOARD_PASTE: // paste is somewhat different as it might be "one action then revert to
										// previous mode"
		toolkit_->hotkey_set_mouse_action(command);
		return true;
	...
	case HOTKEY_EDITOR_SELECTION_FILL:
		...
		return true;
	case HOTKEY_EDITOR_SELECTION_RANDOMIZE:
		context_manager_->perform_refresh(editor_action_shuffle_area(get_current_map_context().map().selection()));
		return true;
	...
	case HOTKEY_EDITOR_AREA_REMOVE:
		get_current_map_context().remove_area(get_current_map_context().get_active_area());
		return true;
	...
	case HOTKEY_EDITOR_MAP_CLOSE:
		...
		// This avoids the situation of having a scenario-specific tool active in a map context which can cause a crash
		// if used Not elegant but at least avoids a potential crash and is consistent with existing behaviour
		toolkit_->hotkey_set_mouse_action(HOTKEY_EDITOR_TOOL_PAINT);
		return true;
	case HOTKEY_EDITOR_MAP_LOAD:
...
void editor_controller::show_menu(
	const std::vector<config>& items_arg, int xloc, int yloc, bool context_menu, display& disp)
{
	...
	if(first_id == "menu-unit-facings") {
		...
		int dir = 0;
		std::generate_n(std::inserter<std::vector<config>>(items, pos),
			static_cast<int>(map_location::direction::indeterminate), [&dir]() -> config {
	...
	if(first_id == "editor-playlist") {
		...
		auto pos = items.erase(items.begin());
		std::transform(music_tracks_.begin(), music_tracks_.end(), std::inserter<std::vector<config>>(items, pos),
			[](const sound::music_track& track) -> config {
	...
	if(first_id == "editor-assign-schedule") {
		...
		auto pos = items.erase(items.begin());
		std::transform(tods_.begin(), tods_.end(), std::inserter<std::vector<config>>(items, pos),
			[](const tods_map::value_type& tod) -> config { return config{"label", tod.second.first}; });
	}
...
void editor_controller::copy_selection()
{
	if(!get_current_map_context().map().selection().empty()) {
		context_manager_->get_clipboard()
			= map_fragment(get_current_map_context().map(), get_current_map_context().map().selection());
...
{
	copy_selection();
	context_manager_->perform_refresh(
		editor_action_paint_area(get_current_map_context().map().selection(), get_selected_bg_terrain()));
}
...
void editor_controller::touch_motion(
	int /* x */, int /* y */, const bool /* browse */, bool /* update */, map_location /* new_loc */)
{
