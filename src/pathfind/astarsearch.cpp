...
struct node
{
	...
	node(double s,
		const map_location& c,
		const map_location& p,
...
plain_route a_star_search(const map_location& src,
	const map_location& dst,
	double stop_at,
	...
	bool border)
{
	...
	while(!pq.empty()) {
		...
		for(auto i = locs.rbegin(); i != locs.rend(); ++i) {
			...
			if(!loc.valid(width, height, border))
				continue;
			if(loc == n.curr)
				continue;
			...
			if(in_list) {
				std::push_heap(
					pq.begin(), std::find(pq.begin(), pq.end(), static_cast<int>(index(loc))) + 1, node_comp);
			} else {
