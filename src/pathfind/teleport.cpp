...
teleport_group::teleport_group(const config& cfg)
	: cfg_(cfg)
	, reversed_(cfg["reversed"].to_bool(false))
	, id_(cfg["id"])
{
...
teleport_group::teleport_group(const vconfig& cfg, bool reversed)
	: cfg_(cfg.get_config())
	, reversed_(reversed)
	, id_()
{
...
void teleport_group::get_teleport_pair(teleport_pair& loc_pair, const unit& u, const bool ignore_units) const
{
	...
	const unit_filter ufilt(filter); // Note: Don't use the ignore units filter context here, only for the terrain
									 // filters. (That's how it worked before the filter contexts were introduced)
	if(ufilt.matches(u)) {
...
teleport_map::teleport_map(const std::vector<teleport_group>& groups,
	const unit& unit,
...
const teleport_map get_teleport_locations(
	const unit& u, const team& viewing_team, bool see_all, bool ignore_units, bool check_vision)
{
...
manager::manager(const config& cfg)
	: tunnels_()
	, id_(cfg["next_teleport_group_id"].to_int(0))
{
