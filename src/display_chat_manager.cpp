	...
	bool bell)
{
	...
	msg_flabel.set_position(rect.x + chat_message_x + font::get_floating_label_rect(speaker_handle).w, rect.y + ypos);
...
void display_chat_manager::prune_chat_messages(bool remove_all)
{
	...
	const auto remove_before = enable_aging ? std::chrono::steady_clock::now() - message_aging
	...
	if(enable_aging || remove_all || chat_messages_.size() > max_chat_messages) {
		while(!chat_messages_.empty()
			&& (remove_all || chat_messages_.front().created_at < remove_before
