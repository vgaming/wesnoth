...
cave_map_generator::cave_map_generator_job::cave_map_generator_job(
	const cave_map_generator& pparams, utils::optional<uint32_t> randomseed)
	: params(pparams)
	, flipx_(false)
	, flipy_(false)
	, map_(t_translation::ter_map(
		  params.width_ + 2 * gamemap::default_border, params.height_ + 2 * gamemap::default_border /*, params.wall_*/))
	, starting_positions_()
	, chamber_ids_()
	, chambers_()
	, passages_()
	...
	res_.add_child("event",
		config{
			...
			config{
				...
				1,
				"message",
				"Use the Lua cave generator instead, with scenario_generation=lua and create_scenario= (see wiki for "
				"details).",
			},
		});
...
void cave_map_generator::cave_map_generator_job::build_chamber(
	map_location loc, std::set<map_location>& locs, std::size_t size, std::size_t jagged)
{
		...
		if(!xpos.empty()) {
			...
			if(items.empty() == false) {
				try {
					...
					lg::log_to_chat() << "Invalid min/max coordinates in cave_map_generator: " << items.front() << ", "
									  << items.back() << "\n";
					ERR_WML << "Invalid min/max coordinates in cave_map_generator: " << items.front() << ", "
							<< items.back();
					continue;
				}
			}
...
void cave_map_generator::cave_map_generator_job::place_chamber(const chamber& c)
{
	...
	if(c.items == nullptr || c.locs.empty())
		return;
...
void cave_map_generator::cave_map_generator_job::place_castle(int starting_position, const map_location& loc)
{
	if(starting_position != -1) {
		...
		t_translation::coordinate coord(loc.x + gamemap::default_border, loc.y + gamemap::default_border);
		starting_positions_.insert(
			t_translation::starting_positions::value_type(std::to_string(starting_position), coord));
	}
