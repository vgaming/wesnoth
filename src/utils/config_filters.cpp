...
bool utils::config_filters::bool_matches_if_present(
	const config& filter, const config& cfg, const std::string& attribute, bool def)
{
...
bool utils::config_filters::set_includes_if_present(
	const config& filter, const config& cfg, const std::string& attribute)
{
	...
	// then there is no point in wasting resources on building the set from the filter and the value is returned
	// immediately.
...
bool utils::config_filters::int_matches_if_present(
	const config& filter, const config& cfg, const std::string& attribute, utils::optional<int> def)
{
	if(!filter.has_attribute(attribute)) {
		return true;
	}
...
bool utils::config_filters::double_matches_if_present(
	const config& filter, const config& cfg, const std::string& attribute, utils::optional<double> def)
{
	if(!filter.has_attribute(attribute)) {
		return true;
	}
	// there is no attribute returning a decimal value using which has a default value but the variable exists in case
	// this changes, otherwise in case the attribute is missing from the ability no value can match.
	if(!cfg.has_attribute(attribute) && !def) {
		return false;
	}
...
bool utils::config_filters::bool_or_empty(const config& filter, const config& cfg, const std::string& attribute)
{
	...
	// If the attribute is not present in the ability this induces a different behavior than if it takes a true or false
	// value and this presence must be verified before checking its value.
	if(!filter.has_attribute(attribute)) {
		return true;
	}
