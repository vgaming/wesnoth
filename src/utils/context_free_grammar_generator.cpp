...
context_free_grammar_generator::context_free_grammar_generator(const std::string& source)
{
	...
	while(*reading != 0) {
		if(*reading == '=') {
			...
			if(key == "!" || key == "(" || key == ")") {
				throw name_generator_invalid_exception(
					"[context_free_grammar_generator] Parsing error: nonterminals (, ! and ) may not be overridden");
			}
...
std::string context_free_grammar_generator::print_nonterminal(
	const std::string& name, uint32_t* seed, short seed_pos) const
{
	if(name == "!") {
		...
		if(found == nonterminals_.end()) {
			lg::log_to_chat() << "[context_free_grammar_generator] Warning: needed nonterminal " << name
							  << " not defined\n";
		...
		for(unsigned int i = 0; i < used.size(); i++) {
			if(used[i][0] == '{')
				result += print_nonterminal(used[i].substr(1), seed, seed_pos);
			else
				result += used[i];
		}
		return result;
