...
name_generator_factory::name_generator_factory(const config& config, std::vector<std::string> ids)
	: name_generators_()
{
...
void name_generator_factory::add_name_generator_from_config(
	const config& config, const std::string& id, const std::string& prefix)
{
	...
	if(config.has_attribute(markov_name)) {
		...
		if(!markov_name_list.blank()) {
			name_generators_[id].reset(
				new markov_generator(utils::split(markov_name_list), config["markov_chain_size"].to_int(2), 12));
		}
	}
}
