...
// Gets an english description of a terrain ter_list alias behavior: "Best of cave, hills", "Worst of Swamp, Forest"
// etc.
std::string print_behavior_description(const ter_iter& start,
	const ter_iter& end,
	...
	bool begin_best = true)
{
	...
	if(!last_change_pos) {
		...
		if(names.empty()) { // This alias list is apparently padded with junk at the end, so truncate it without adding
							// more parens
...
std::vector<std::string> get_special_notes(const terrain_type& type)
{
	...
	if(type.is_village()) {
		...
		// TRANSLATORS: special note for terrains such as the oasis; the only terrain in core with this property heals 8
		// hp just like a village. For the single-hitpoint variant, the wording is different because I assume the player
		// will be more interested in the curing-poison part than the minimal healing.
		auto message = VNGETTEXT("This terrain allows units to be cured of poison, or to heal a single hitpoint.",
			"This terrain allows units to heal $amount hitpoints, or to be cured of poison, as if stationed in a "
			"village.",
	...
	if(type.is_keep() && type.is_castle()) {
		...
		special_notes.push_back(
			_("This terrain is a keep — a leader can recruit from this hex onto connected castle hexes."));
	} else if(type.is_keep() && !type.is_castle()) {
		// TRANSLATORS: Special note for a terrain, but none of the terrains in mainline do this.
		special_notes.push_back(_("This unusual keep allows a leader to recruit while standing on it, but does not "
								  "allow a leader on a connected keep to recruit onto this hex."));
	}
...
std::string terrain_topic_generator::operator()() const
{
	...
	if(!type_.icon_image().empty()) {
		ss << markup::img(formatter() << "images/buttons/icon-base-32.png~RC(magenta>" << type_.id() << ")~BLIT("
	...
	if(type_.is_combined()) {
		ss << "Base terrain: ";
		const auto base_t
			= tdata->get_terrain_info(t_translation::terrain_code(type_.number().base, t_translation::NO_LAYER));
		ss << markup::make_link(base_t.editor_name(), (base_t.hide_help() ? "." : "") + terrain_prefix + base_t.id());
		ss << ", ";
		ss << "Overlay terrain: ";
		const auto overlay_t
			= tdata->get_terrain_info(t_translation::terrain_code(t_translation::NO_LAYER, type_.number().overlay));
		ss << markup::make_link(
			overlay_t.editor_name(), (overlay_t.hide_help() ? "." : "") + terrain_prefix + overlay_t.id());
		ss << "\n";
	}
	...
	if(!type_.is_indivisible()) {
		...
		if(type_.has_default_base()) {
			...
			symbols.clear();
			symbols["type"] = markup::make_link(
				base.editor_name(), (base.is_indivisible() ? ".." : "") + terrain_prefix + base.id());
...
std::string unit_topic_generator::operator()() const
{
	...
	// Unit Images
	ss << markup::img(formatter() << male_type.image() << "~RC(" << male_type.flag_rgb() << ">red)"
								  << (screen_width >= 1200 ? "~SCALE_SHARP(200%,200%)" : ""));

	if(female_type.image() != male_type.image()) {
		ss << markup::img(formatter() << female_type.image() << "~RC(" << female_type.flag_rgb() << ">red)"
									  << (screen_width >= 1200 ? "~SCALE_SHARP(200%,200%)" : ""));
	}
	...
	if(!type_.attacks().empty()) {
		...
		table_ss << markup::tag("row", {{"bgcolor", "table_header"}},
			// FIXME space/tab does not work, but nbsp does
			// empty tags will be skipped by rich_label
			markup::tag("col", font::nbsp), markup::tag("col", markup::bold(_("Name"))),
			markup::tag("col", markup::bold(_("Strikes"))), markup::tag("col", markup::bold(_("Range"))),
		...
		for(const attack_type& attack : type_.attacks()) {
			...
			// damage x strikes
			attack_ss << markup::tag("col", attack.damage(), font::weapon_numbers_sep, attack.num_attacks(), " ",
			...
			if(attack.min_range() > 1 || attack.max_range() > 1) {
				attack_ss << markup::tag("col", markup::img(range_icon), attack.min_range(), "-", attack.max_range(),
					' ', string_table["range_" + attack.range()]);
			} else {
				attack_ss << markup::tag("col", markup::img(range_icon), string_table["range_" + attack.range()]);
			}
		...
		for(const config& t : traits) {
			if(t["availability"].str() == "musthave") {
				for(const config& effect : t.child_range("effect")) {
					if(!effect.has_child("filter") // If this is musthave but has a unit filter, it might not always
												   // apply, so don't apply it in the help.
	...
	table_ss << markup::tag("row", {{"bgcolor", "table_header"}}, markup::tag("col", markup::bold(_("Attack Type"))),
	...
	for(std::pair<std::string, std::string> dam_it : dam_tab) {
		...
		table_ss << markup::tag("row", {{"bgcolor", (odd_row ? "table_row1" : "table_row2")}},
	...
	if(std::shared_ptr<terrain_type_data> tdata = load_terrain_types_data()) {
		...
		if(has_terrain_defense_caps) {
			row_ss << markup::tag("col", markup::bold(_("Defense Cap")));
		}
		if(has_vision) {
			row_ss << markup::tag("col", markup::bold(_("Vision Cost")));
		}
		if(has_jamming) {
			row_ss << markup::tag("col", markup::bold(_("Jamming Cost")));
		}
		...
		for(t_translation::terrain_code terrain : prefs::get().encountered_terrains()) {
			...
			if(info.is_indivisible() && info.is_nonnull()) {
				terrain_movement_info movement_info = {info.name(), info.id(),
