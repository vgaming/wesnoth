...
static bool is_cjk_char(const char32_t ch)
{
	...
	return
		...
		(ch >= 0x4e00 && ch < 0x9fcf) || (ch >= 0x3400 && ch < 0x4dbf) || (ch >= 0x20000 && ch < 0x2a6df)
		|| (ch >= 0xf900 && ch < 0xfaff) || (ch >= 0x3190 && ch < 0x319f) ||

		// Radicals: all except Ideographic Description
		(ch >= 0x2e80 && ch < 0x2eff) || (ch >= 0x2f00 && ch < 0x2fdf) || (ch >= 0x31c0 && ch < 0x31ef) ||

		// Chinese-specific: Bopomofo and Bopomofo Extended
		(ch >= 0x3104 && ch < 0x312e) || (ch >= 0x31a0 && ch < 0x31bb) ||

		// Yi-specific: Yi Radicals, Yi Syllables
		(ch >= 0xa490 && ch < 0xa4c7) || (ch >= 0xa000 && ch < 0xa48d) ||

		// Japanese-specific: Hiragana, Katakana, Kana Supplement
		(ch >= 0x3040 && ch <= 0x309f) || (ch >= 0x30a0 && ch <= 0x30ff) || (ch >= 0x1b000 && ch <= 0x1b001) ||
		...
		(ch >= 0xac00 && ch < 0xd7af) || (ch >= 0x1100 && ch <= 0x11ff) || (ch >= 0xa960 && ch <= 0xa97c)
		|| (ch >= 0xd7b0 && ch <= 0xd7fb) ||
...
bool section_is_referenced(const std::string& section_id, const config& cfg)
{
	if(auto toplevel = cfg.optional_child("toplevel")) {
		const std::vector<std::string> toplevel_refs = utils::quoted_split(toplevel["sections"]);
		if(std::find(toplevel_refs.begin(), toplevel_refs.end(), section_id) != toplevel_refs.end()) {
			return true;
		}
	}

	for(const config& section : cfg.child_range("section")) {
		const std::vector<std::string> sections_refd = utils::quoted_split(section["sections"]);
		if(std::find(sections_refd.begin(), sections_refd.end(), section_id) != sections_refd.end()) {
			return true;
		}
	}
	return false;
...
{
	if(auto toplevel = cfg.optional_child("toplevel")) {
		const std::vector<std::string> toplevel_refs = utils::quoted_split(toplevel["topics"]);
		if(std::find(toplevel_refs.begin(), toplevel_refs.end(), topic_id) != toplevel_refs.end()) {
			return true;
		}
	}

	for(const config& section : cfg.child_range("section")) {
		const std::vector<std::string> topics_refd = utils::quoted_split(section["topics"]);
		if(std::find(topics_refd.begin(), topics_refd.end(), topic_id) != topics_refd.end()) {
			return true;
		}
	}
	return false;
	...
	if(level > max_section_level) {
		...
		for(it = sections.begin(); it != sections.end(); ++it) {
			if(auto child_cfg = help_cfg->find_child("section", "id", *it)) {
				...
				std::stringstream ss;
				ss << "Help-section '" << *it << "' referenced from '" << id << "' but could not be found.";
				throw parse_error(ss.str());
			}
		}
		...
		for(it = topics_id.begin(); it != topics_id.end(); ++it) {
			if(auto topic_cfg = help_cfg->find_child("topic", "id", *it)) {
				...
				std::stringstream ss;
				ss << "Help-topic '" << *it << "' referenced from '" << id << "' but could not be found." << std::endl;
				throw parse_error(ss.str());
			}
		}
		...
		if(sort_topics) {
			...
			std::merge(generated_topics.begin(), generated_topics.end(), topics.begin(), topics.end(),
				std::back_inserter(sec.topics), title_less());
		} else {
			sec.topics.insert(sec.topics.end(), topics.begin(), topics.end());
			sec.topics.insert(sec.topics.end(), generated_topics.begin(), generated_topics.end());
		}
	}
}
...
{
	return markup::span_color(
		(time_of_day_bonus > 0 ? "green" : (time_of_day_bonus < 0 ? "red" : "white")), time_of_day_bonus);
}
...
std::vector<topic> generate_time_of_day_topics(const bool /*sort_generated*/)
{
	...
	for(const time_of_day& time : times) {
		...
		const int lawful_bonus = generic_combat_modifier(
			time.lawful_bonus, unit_alignments::type::lawful, false, resources::tod_manager->get_max_liminal_bonus());
		const int neutral_bonus = generic_combat_modifier(
			time.lawful_bonus, unit_alignments::type::neutral, false, resources::tod_manager->get_max_liminal_bonus());
		const int chaotic_bonus = generic_combat_modifier(
			time.lawful_bonus, unit_alignments::type::chaotic, false, resources::tod_manager->get_max_liminal_bonus());
		const int liminal_bonus = generic_combat_modifier(
			time.lawful_bonus, unit_alignments::type::liminal, false, resources::tod_manager->get_max_liminal_bonus());

		row_ss << markup::tag("col", markup::make_link(time.name.str(), id)) << markup::tag("col", image)
...
std::vector<topic> generate_weapon_special_topics(const bool sort_generated)
{
	...
	for(std::map<t_string, std::string>::iterator s = special_description.begin(); s != special_description.end();
		++s) {
	...
	for(const config& f : era.child_range("multiplayer_side")) {
		...
		for(const std::string& u_id : recruit_ids) {
			if(const unit_type* t = unit_types.find(u_id, unit_type::HELP_INDEXED)) {
				...
				alignments.insert(markup::make_link(
					type.alignment_description(type.alignment(), type.genders().front()), "time_of_day"));
			}
		}
		...
		const std::vector<std::string> leaders = make_unit_links_list(utils::split(f["leader"]), true);
		...
		const std::vector<std::string> recruit_links = make_unit_links_list(recruit_ids, true);
	...
	while(!last_sweep.empty()) {
		...
		for(const auto& race_id : last_sweep) {
			if(const unit_race* r = unit_types.find_race(race_id)) {
				...
				if(!help_taxonomy.empty() && !visible_races.count(help_taxonomy)
					&& unit_types.find_race(help_taxonomy)) {
...
void generate_terrain_sections(section& sec, int /*level*/)
{
	...
	for(const t_translation::terrain_code& t : t_listi) {
		...
		if(prefs::get().encountered_terrains().find(t) == prefs::get().encountered_terrains().end()
			&& !info.is_overlay())
			hidden = true;
...
void generate_unit_sections(
	const config* /*help_cfg*/, section& sec, int /*level*/, const bool /*sort_generated*/, const std::string& race)
{
...
std::vector<topic> generate_unit_topics(const bool sort_generated, const std::string& race)
{
	...
	for(const unit_type_data::unit_type_map::value_type& i : unit_types.types()) {
		...
		if(!type.hide_help()) {
			...
			alignments.insert(
				markup::make_link(type.alignment_description(type.alignment(), type.genders().front()), "time_of_day"));
		}
	}
...
{
	if(game_config::debug || prefs::get().show_all_units_in_help() || hotkey::is_scope_active(hotkey::SCOPE_EDITOR)) {
...
{
	topic_list::const_iterator tit = std::find_if(sec.topics.begin(), sec.topics.end(), has_id(id));
...
{
	const auto sit = std::find_if(sec.sections.begin(), sec.sections.end(), has_id(id));
