...
{
	return {color.r / 255.0 * std::numeric_limits<uint16_t>::max(),
		color.g / 255.0 * std::numeric_limits<uint16_t>::max(), color.b / 255.0 * std::numeric_limits<uint16_t>::max()};
}
...
void add_attribute_size(attribute_list& list, unsigned offset_start, unsigned offset_end, int size)
{
	...
	attribute attr{pango_attr_size_new_absolute(PANGO_SCALE * size), offset_start, offset_end};
...
{
	attribute attr{pango_attr_weight_new(weight), offset_start, offset_end};
...
{
	attribute attr{pango_attr_style_new(style), offset_start, offset_end};
...
{
	attribute attr{pango_attr_underline_new(underline), offset_start, offset_end};
...
void add_attribute_fg_color(attribute_list& list, unsigned offset_start, unsigned offset_end, const color_t& color)
{
	...
	attribute attr{pango_attr_foreground_new(col_r, col_g, col_b), offset_start, offset_end};
...
void add_attribute_bg_color(attribute_list& list, unsigned offset_start, unsigned offset_end, const color_t& color)
{
	...
	attribute attr{pango_attr_background_new(col_r, col_g, col_b), offset_start, offset_end};
...
void add_attribute_font_family(
	attribute_list& list, unsigned offset_start, unsigned offset_end, font::family_class family)
{
	const t_string& family_name = get_font_families(family);

	attribute attr{pango_attr_family_new(family_name.c_str()), offset_start, offset_end};
