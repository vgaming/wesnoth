...
void floating_textbox::update_location(game_display& gui)
{
	...
	if(box_ != nullptr) {
		const SDL_Rect rect{area.x + label_area.w + border_size * 2, ypos, textbox_width, box_->height()};
...
void floating_textbox::show(
	gui::TEXTBOX_MODE mode, const std::string& label, const std::string& check_label, bool checked, game_display& gui)
{
	close();
...
void floating_textbox::tab(const std::set<std::string>& dictionary)
{
	...
	if(matches.size() == 1 && mode_ == gui::TEXTBOX_MESSAGE) {
		...
		game_display::get_singleton()->get_chat_manager().add_chat_message(
			time(nullptr), "", 0, completion_list, events::chat_handler::MESSAGE_PRIVATE, false);
	}
	box_->set_text(text);
}
