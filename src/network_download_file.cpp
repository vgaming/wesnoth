...
void download(const std::string& url, const std::string& local_path)
{
	...
	std::string buffer;
	// curl doesn't initialize the error buffer until version 7.60.0, which isn't currently available on all supported
	// macOS versions
	...
	if(curl) {
		...
		if(res != CURLE_OK) {
			...
			try {
				if(filesystem::file_exists(local_path)) {
					const int res = gui2::show_message(
						_("Confirm overwrite"), _("Overwrite existing file?"), gui2::dialogs::message::yes_no_buttons);
			...
			gui2::show_message(_("Download complete"), _("The file has been downloaded."),
				gui2::dialogs::message::button_style::auto_close);
		}
