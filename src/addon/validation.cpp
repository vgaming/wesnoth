...
const std::array<std::string, ADDON_TYPES_COUNT> addon_type_strings{{"unknown", "core", "campaign", "scenario",
	"campaign_sp_mp", "campaign_mp", "scenario_mp", "map_pack", "era", "faction", "mod_mp", "media", "theme", "other"}};
...
{
	if(name.empty() || std::find_if(name.begin(), name.end(), addon_name_char_illegal()) != name.end()) {
		return false;
	} else {
		return true;
	}
...
bool check_case_insensitive_duplicates_internal(
	const config& dir, const std::string& prefix, std::vector<std::string>* badlist)
{
...
std::string addon_check_status_desc(unsigned int code)
{
	static const std::map<ADDON_CHECK_STATUS, std::string> message_table = {
		...
		{ADDON_CHECK_STATUS::SUCCESS, N_("Success.")},
		{ADDON_CHECK_STATUS::UNAUTHORIZED, N_("Incorrect add-on passphrase.")},
		...
		{ADDON_CHECK_STATUS::DENIED, N_("Upload denied. Please contact the server administration for assistance.")},
		{ADDON_CHECK_STATUS::UNEXPECTED_DELTA, N_("Attempted to upload an update pack for a non-existent add-on.")},

		//
		// Structure errors
		//

		{ADDON_CHECK_STATUS::EMPTY_PACK, N_("No add-on data was supplied by the client.")},
		{ADDON_CHECK_STATUS::BAD_DELTA, N_("Invalid upload pack.")},
		{ADDON_CHECK_STATUS::BAD_NAME, N_("Invalid add-on name.")},
		{ADDON_CHECK_STATUS::NAME_HAS_MARKUP, N_("Formatting character in add-on name.")},
		...
		{ADDON_CHECK_STATUS::INVALID_UTF8_NAME, N_("The add-on name contains an invalid UTF-8 sequence.")},

		//
		// .pbl errors
		//

		{ADDON_CHECK_STATUS::NO_TITLE, N_("No add-on title specified.")},
		{ADDON_CHECK_STATUS::NO_AUTHOR, N_("No add-on author/maintainer name specified.")},
		{ADDON_CHECK_STATUS::NO_VERSION, N_("No add-on version specified.")},
		{ADDON_CHECK_STATUS::NO_DESCRIPTION, N_("No add-on description specified.")},
		{ADDON_CHECK_STATUS::NO_EMAIL, N_("No add-on author/maintainer email specified.")},
		{ADDON_CHECK_STATUS::NO_PASSPHRASE, N_("Missing passphrase.")},
		{ADDON_CHECK_STATUS::TITLE_HAS_MARKUP, N_("Formatting character in add-on title.")},
		{ADDON_CHECK_STATUS::BAD_TYPE, N_("Invalid or unspecified add-on type.")},
		{ADDON_CHECK_STATUS::VERSION_NOT_INCREMENTED,
			N_("Version number not greater than the latest uploaded version.")},
		{ADDON_CHECK_STATUS::BAD_FEEDBACK_TOPIC_ID, N_("Feedback topic id is not a number.")},
		{ADDON_CHECK_STATUS::FEEDBACK_TOPIC_ID_NOT_FOUND, N_("Feedback topic does not exist.")},
		...
		{ADDON_CHECK_STATUS::ICON_TOO_LARGE, N_("The add-on’s icon’s file size is too large.")},

		//
		// Server errors
		//

		{ADDON_CHECK_STATUS::SERVER_UNSPECIFIED, N_("Unspecified server error.")},
		{ADDON_CHECK_STATUS::SERVER_READ_ONLY, N_("Server is in read-only mode.")},
		{ADDON_CHECK_STATUS::SERVER_ADDONS_LIST, N_("Corrupted server add-ons list.")},
		{ADDON_CHECK_STATUS::SERVER_DELTA_NO_VERSIONS, N_("Empty add-on version list on the server.")},
