...
std::ostream& metrics::games(std::ostream& out) const
{
	if(terminations_.empty())
		return out << "No game ended so far.";
...
std::ostream& metrics::requests(std::ostream& out) const
{
	...
	for(const auto& s : ordered_samples) {
		out << "'" << s.name << "' called " << s.nsamples << " times " << s.parsing_time.count() << "("
			<< s.max_parsing_time.count() << ") parsing time, " << s.processing_time.count() << "("
			<< s.max_processing_time.count() << ") processing time\n";
		n += s.nsamples;
		pa += s.parsing_time;
...
std::ostream& operator<<(std::ostream& out, metrics& met)
{
	...
	out << "METRICS\nUp " << days.count() << " days, " << hours.count() << " hours, " << minutes.count() << " minutes, "
		<< seconds.count() << " seconds\n"
		<< met.nrequests_ << " requests serviced. " << requests_immediate << " (" << percent_immediate << "%) "
