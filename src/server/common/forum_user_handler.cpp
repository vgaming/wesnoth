	...
	, forum_admin_group_(0)
{
	try {
		...
		ERR_UH << "Failed to convert the mp_mod_group value of '" << c["mp_mod_group"].str()
			   << "' into an int!  Defaulting to " << mp_mod_group_ << ".";
	}
	try {
		site_admin_group_ = std::stoi(c["site_admin_group"].str());
	} catch(...) {
		ERR_UH << "Failed to convert the site_admin_group_ value of '" << c["site_admin_group"].str()
			   << "' into an int!  Defaulting to " << site_admin_group_ << ".";
	}
	try {
		forum_admin_group_ = std::stoi(c["forum_admin_group"].str());
	} catch(...) {
		ERR_UH << "Failed to convert the forum_admin_group_ value of '" << c["forum_admin_group"].str()
...
bool fuh::user_is_moderator(const std::string& name)
{
	...
	return conn_.get_user_int(db_extra_table_, "user_is_moderator", name) == 1
		|| (mp_mod_group_ != 0 && conn_.is_user_in_groups(name, {mp_mod_group_}));
}
...
std::string fuh::user_info(const std::string& name)
{
	...
	info << "Name: " << name << "\n"
		 << "Registered: " << reg_string << "Last login: " << ll_string;
...
void fuh::db_update_addon_download_count(
...
void fuh::async_get_and_send_game_history(boost::asio::io_context& io_service,
	wesnothd::server& s,
...
{
	boost::asio::post(
		[this, &s, socket, player_id, offset, &io_service, search_game_name, search_content_type, search_content] {
			boost::asio::post(io_service,
				[socket, &s,
					doc = conn_.get_game_history(player_id, offset, search_game_name, search_content_type,
						search_content)] { s.send_to_player(socket, *doc); });
		});
}
...
bool fuh::db_is_user_primary_author(
	const std::string& instance_version, const std::string& id, const std::string& username)
{
	return conn_.is_user_author(instance_version, id, username, 1);
}

bool fuh::db_is_user_secondary_author(
	const std::string& instance_version, const std::string& id, const std::string& username)
{
