...
void server_base::start_server()
{
	...
	boost::asio::spawn(
		io_service_,
		[this, endpoint_v6](const boost::asio::yield_context& yield) { serve(yield, acceptor_v6_, endpoint_v6); }
	...
	boost::asio::spawn(
		io_service_,
		[this, endpoint_v4](const boost::asio::yield_context& yield) { serve(yield, acceptor_v4_, endpoint_v4); }
#if BOOST_VERSION >= 108000
		,
		[](const std::exception_ptr& e) {
			if(e)
...
#ifndef _WIN32
	sighup_.async_wait([this](const boost::system::error_code& error, int sig) { this->handle_sighup(error, sig); });
#endif
}

void server_base::serve(const boost::asio::yield_context& yield,
	...
	const boost::asio::ip::tcp::endpoint& endpoint)
{
	...
	if(accepting_connections()) {
		boost::asio::spawn(
			io_service_,
			[this, &acceptor, endpoint](const boost::asio::yield_context& yield) { serve(yield, acceptor, endpoint); }
#if BOOST_VERSION >= 108000
			,
			[](const std::exception_ptr& e) {
				if(e)
	...
	if(keep_alive_) {
		...
		setsockopt(socket->native_handle(), SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<const char*>(&timeout_ms),
			sizeof(timeout_ms));
		setsockopt(socket->native_handle(), SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<const char*>(&timeout_ms),
			sizeof(timeout_ms));
#endif
	}
	...
	case 1:
		...
		async_write(
			*socket, boost::asio::buffer(reinterpret_cast<const std::byte*>(&handshake_response), 4), yield[error]);
		if(check_error(error, socket))
			return;
...
void info_table_into_simple_wml(
	simple_wml::document& doc, const std::string& parent_name, const server_base::info_table& info)
{
	if(info.empty()) {
		return;
	}
 ...
 */
template<class SocketPtr>
void server_base::coro_send_doc(SocketPtr socket, simple_wml::document& doc, const boost::asio::yield_context& yield)
{
	if(dump_wml) {
	...
	try {
		...
		std::vector<boost::asio::const_buffer> buffers{{data_size.buf, 4}, {s.begin(), std::size_t(s.size())}};
...
template void server_base::coro_send_doc<socket_ptr>(
	socket_ptr socket, simple_wml::document& doc, const boost::asio::yield_context& yield);
template void server_base::coro_send_doc<tls_socket_ptr>(
	tls_socket_ptr socket, simple_wml::document& doc, const boost::asio::yield_context& yield);
...
void server_base::coro_send_file(
	tls_socket_ptr socket, const std::string& filename, const boost::asio::yield_context& yield)
{
...
void server_base::coro_send_file(
	const socket_ptr& socket, const std::string& filename, const boost::asio::yield_context& yield)
{
	std::size_t filesize{std::size_t(filesystem::file_size(filename))};
	int in_file{open(filename.c_str(), O_RDONLY)};
	ON_SCOPE_EXIT(in_file)
	{
		close(in_file);
	};
	off_t offset{0};
...
template<class SocketPtr>
std::unique_ptr<simple_wml::document> server_base::coro_receive_doc(
	SocketPtr socket, const boost::asio::yield_context& yield)
{
	union DataSize {
		uint32_t size;
		char buf[4];
	...
	if(size == 0) {
		ERR_SERVER << log_address(socket) << "\treceived invalid packet with payload size 0";
		return {};
	}
	if(size > simple_wml::document::document_size_limit) {
		ERR_SERVER << log_address(socket) << "\treceived packet with payload size over size limit";
		return {};
	}
	...
	try {
		...
		ERR_SERVER << log_address(socket) << "\tsimple_wml error in received data: " << e.message;
		async_send_error(socket, "Invalid WML received: " + e.message);
		return {};
	}
}
template std::unique_ptr<simple_wml::document> server_base::coro_receive_doc<socket_ptr>(
	socket_ptr socket, const boost::asio::yield_context& yield);
template std::unique_ptr<simple_wml::document> server_base::coro_receive_doc<tls_socket_ptr>(
	tls_socket_ptr socket, const boost::asio::yield_context& yield);

template<class SocketPtr>
void server_base::send_doc_queued(
	SocketPtr socket, std::unique_ptr<simple_wml::document>& doc_ptr, boost::asio::yield_context yield)
{
...
template<class SocketPtr>
void server_base::async_send_doc_queued(SocketPtr socket, simple_wml::document& doc)
{
	boost::asio::spawn(
		io_service_,
		[this, doc_ptr = doc.clone(), socket](
			boost::asio::yield_context yield) mutable { send_doc_queued(socket, doc_ptr, yield); }
#if BOOST_VERSION >= 108000
		,
		[](const std::exception_ptr& e) {
			if(e)
...
template<class SocketPtr>
void server_base::async_send_error(
	SocketPtr socket, const std::string& msg, const char* error_code, const info_table& info)
{
...
template void server_base::async_send_error<socket_ptr>(
	socket_ptr socket, const std::string& msg, const char* error_code, const info_table& info);
template void server_base::async_send_error<tls_socket_ptr>(
	tls_socket_ptr socket, const std::string& msg, const char* error_code, const info_table& info);

template<class SocketPtr>
void server_base::async_send_warning(
	SocketPtr socket, const std::string& msg, const char* warning_code, const info_table& info)
{
	simple_wml::document doc;
...
template void server_base::async_send_warning<socket_ptr>(
	socket_ptr socket, const std::string& msg, const char* warning_code, const info_table& info);
template void server_base::async_send_warning<tls_socket_ptr>(
	tls_socket_ptr socket, const std::string& msg, const char* warning_code, const info_table& info);
...
void server_base::load_tls_config(const config& cfg)
{
	...
	tls_context_.set_options(boost::asio::ssl::context::default_workarounds | boost::asio::ssl::context::no_sslv2
	...
	if(!cfg["tls_dh"].str().empty())
		tls_context_.use_tmp_dh_file(cfg["tls_dh"].str());
}
