...
std::string dbconn::get_tournaments()
{
	...
	try {
		...
		for(const auto& child : t.child_range("tournament")) {
			text += "\nThe tournament " + child["title"].str() + " is " + child["status"].str()
				+ ". More information can be found at " + child["url"].str();
		}
		return text;
	} catch(const mariadb::exception::base& e) {
...
std::unique_ptr<simple_wml::document> dbconn::get_game_history(
	int player_id, int offset, std::string search_game_name, int search_content_type, std::string search_content)
{
		...
		while(rslt->next()) {
			...
			for(const auto& player_info : utils::split(rslt->get_string("PLAYERS"))) {
				...
				if(info.size() == 2) {
					...
					ERR_SQL << "Expected player information to split into two fields, instead found the value `"
							<< player_info << "`.";
				}
			}
	...
	try {
		...
		std::string game_history_query = "select "
										 ...
										 "  IFNULL((select GROUP_CONCAT(distinct mods.NAME) from "
			+ db_game_content_info_table_
			+ " mods where mods.TYPE = 'modification' and mods.INSTANCE_UUID = game.INSTANCE_UUID and mods.GAME_ID = "
			  "game.GAME_ID), '') as MODIFICATION_NAMES, "
			  "  case "
			  "  when game.PUBLIC = 1 and game.INSTANCE_VERSION != 'trunk' "
			  "  then concat('https://replays.wesnoth.org/', substring(game.INSTANCE_VERSION, 1, 4), '/', "
			  "year(game.END_TIME), '/', lpad(month(game.END_TIME), 2, '0'), '/', lpad(day(game.END_TIME), 2, '0'), "
			  "'/', game.REPLAY_NAME) "
			  "  when game.PUBLIC = 1 and game.INSTANCE_VERSION = 'trunk' "
			  "  then concat('https://replays.wesnoth.org/', game.INSTANCE_VERSION, '/', year(game.END_TIME), '/', "
			  "lpad(month(game.END_TIME), 2, '0'), '/', lpad(day(game.END_TIME), 2, '0'), '/', game.REPLAY_NAME) "
			  "  else '' "
			  "  end as REPLAY_URL, "
			  "  case "
			  ...
			  "  end as VERSION "
			  "from "
			+ db_game_player_info_table_ + " player , " + db_game_content_info_table_ + " scenario , "
			+ db_game_content_info_table_ + " era , " + db_game_info_table_ + " game ";
		// modification id optional parameter
		if(search_content_type == 2 && !search_content.empty()) {
			game_history_query += ", " + db_game_content_info_table_
				+ " mods "
			...
			// put the where clause with an always true condition here so I don't need to check again a couple lines
			// down whether it needs "where" vs "and"
...
{
	try {
		return get_single_long(connection_,
			"SELECT IFNULL((SELECT user_id FROM `" + db_users_table_ + "` WHERE UPPER(username)=UPPER(?)), 0)", {name});
	} catch(const mariadb::exception::base& e) {
...
bool dbconn::is_user_in_groups(const std::string& name, const std::vector<int>& group_ids)
{
	...
	try {
		return exists(connection_,
			"SELECT 1 FROM `" + db_users_table_ + "` u, `" + db_user_group_table_
				+ "` ug WHERE UPPER(u.username)=UPPER(?) AND u.USER_ID = ug.USER_ID AND ug.GROUP_ID in ("
				+ utils::join(group_params) + ")",
			params);
	} catch(const mariadb::exception::base& e) {
...
config dbconn::get_ban_info(const std::string& name, const std::string& ip)
{
	...
	try {
		return get_complex_results(connection_, &ban_info_handler,
			"select ban_userid, ban_email, case when ban_ip != '' then 1 when ban_userid != 0 then 2 when ban_email != "
			"'' then 3 end as ban_type, ban_end from `"
				+ db_banlist_table_ + "` where (ban_ip = ? or ban_userid = (select user_id from `" + db_users_table_
...
{
	try {
		return get_single_string(
			connection_, "SELECT `" + column + "` from `" + table + "` WHERE UPPER(username)=UPPER(?)", {name});
	} catch(const mariadb::exception::base& e) {
		log_sql_exception(
			"Unable to query column `" + column + "` from table `" + table + "` for user `" + name + "`", e);
		return "";
	}
}
...
{
	try {
		return static_cast<int>(get_single_long(
			connection_, "SELECT `" + column + "` from `" + table + "` WHERE UPPER(username)=UPPER(?)", {name}));
	} catch(const mariadb::exception::base& e) {
		log_sql_exception(
			"Unable to query column `" + column + "` from table `" + table + "` for user `" + name + "`", e);
...
void dbconn::write_user_int(const std::string& column, const std::string& name, int value)
{
	try {
		...
		modify(connection_, "UPDATE `" + db_extra_table_ + "` SET " + column + "=? WHERE UPPER(username)=UPPER(?)",
			{value, name});
	} catch(const mariadb::exception::base& e) {
		log_sql_exception("Unable to write `" + std::to_string(value) + "` to column `" + column + "` on table `"
				+ db_extra_table_ + "` for user `" + name + "`",
			e);
	}
}
	...
	try {
		modify(connection_,
			"INSERT INTO `" + db_game_info_table_
				+ "`(INSTANCE_UUID, GAME_ID, INSTANCE_VERSION, GAME_NAME, RELOAD, OBSERVERS, PUBLIC, PASSWORD) "
				  "VALUES(?, ?, ?, ?, ?, ?, ?, ?)",
			{uuid, game_id, version, name, reload, observers, is_public, has_password});
	} catch(const mariadb::exception::base& e) {
		log_sql_exception(
			"Failed to insert game info row for UUID `" + uuid + "` and game ID `" + std::to_string(game_id) + "`", e);
	}
}
void dbconn::update_game_end(const std::string& uuid, int game_id, const std::string& replay_location)
{
	try {
		...
		log_sql_exception("Failed to update the game end for game info row for UUID `" + uuid + "` and game ID `"
	...
	try {
		modify(connection_,
			"INSERT INTO `" + db_game_player_info_table_
				+ "`(INSTANCE_UUID, GAME_ID, USER_ID, SIDE_NUMBER, IS_HOST, FACTION, CLIENT_VERSION, CLIENT_SOURCE, "
				  "USER_NAME, LEADERS) VALUES(?, ?, IFNULL((SELECT user_id FROM `"
				+ db_users_table_ + "` WHERE username = ?), -1), ?, ?, ?, ?, ?, ?, ?)",
			{uuid, game_id, username, side_number, is_host, faction, version, source, current_user, leaders});
	} catch(const mariadb::exception::base& e) {
		log_sql_exception("Failed to insert game player info row for UUID `" + uuid + "` and game ID `"
				+ std::to_string(game_id) + "`",
			e);
	}
}
	...
	const std::string& addon_version)
{
	try {
		...
		log_sql_exception("Failed to insert game content info row for UUID `" + uuid + "` and game ID `"
				+ std::to_string(game_id) + "`",
			e);
		return 0;
	}
...
void dbconn::set_oos_flag(const std::string& uuid, int game_id)
{
	try {
		...
		log_sql_exception(
			"Failed to set the OOS flag for UUID `" + uuid + "` and game ID `" + std::to_string(game_id) + "`", e);
	}
}
...
{
	try {
		return exists(connection_, "SELECT 1 FROM `" + db_topics_table_ + "` WHERE TOPIC_ID = ?", {topic_id});
	} catch(const mariadb::exception::base& e) {
	...
	try {
		modify(connection_,
			"INSERT INTO `" + db_addon_info_table_
				+ "`(INSTANCE_VERSION, ADDON_ID, ADDON_NAME, TYPE, VERSION, FORUM_AUTH, FEEDBACK_TOPIC, UPLOADER) "
				  "VALUES(?, ?, ?, ?, ?, ?, ?, ?)",
			{instance_version, id, name, type, version, forum_auth, topic_id, uploader});
	} catch(const mariadb::exception::base& e) {
		log_sql_exception(
			"Unable to insert add-on info for add-on `" + id + "` for instance `" + instance_version + "`.", e);
	}
}
...
{
	try {
		return modify_get_id(connection_,
			"INSERT INTO `" + db_connection_history_table_ + "`(USER_NAME, IP, VERSION) values(lower(?), ?, ?)",
			{username, ip, version});
	} catch(const mariadb::exception::base& e) {
...
void dbconn::update_logout(unsigned long long login_id)
{
	try {
		modify(connection_,
			"UPDATE `" + db_connection_history_table_ + "` SET LOGOUT_TIME = CURRENT_TIMESTAMP WHERE LOGIN_ID = ?",
			{login_id});
	} catch(const mariadb::exception::base& e) {
...
{
	try {
		mariadb::result_set_ref rslt = select(connection_,
			"SELECT USER_NAME, IP, date_format(LOGIN_TIME, '%Y/%m/%d %h:%i:%s'), coalesce(date_format(LOGOUT_TIME, "
			"'%Y/%m/%d %h:%i:%s'), '(not set)') FROM `"
				+ db_connection_history_table_ + "` WHERE IP LIKE ? order by LOGIN_TIME",
			{ip});

		*out << "\nCount of results for ip: " << rslt->row_count();

		while(rslt->next()) {
			*out << "\nFound user " << rslt->get_string(0) << " with ip " << rslt->get_string(1) << ", logged in at "
				 << rslt->get_string(2) << " and logged out at " << rslt->get_string(3);
		}
	} catch(const mariadb::exception::base& e) {
...
void dbconn::get_ips_for_user(const std::string& username, std::ostringstream* out)
{
	try {
		mariadb::result_set_ref rslt = select(connection_,
			"SELECT USER_NAME, IP, date_format(LOGIN_TIME, '%Y/%m/%d %h:%i:%s'), coalesce(date_format(LOGOUT_TIME, "
			"'%Y/%m/%d %h:%i:%s'), '(not set)') FROM `"
				+ db_connection_history_table_ + "` WHERE USER_NAME LIKE ? order by LOGIN_TIME",
...
void dbconn::update_addon_download_count(
	...
	const std::string& instance_version, const std::string& id, const std::string& username, int is_primary)
{
	try {
		...
		log_sql_exception("Unable to check whether `" + username + "` is an author of " + id + " for version "
				+ instance_version + ".",
			e);
		return false;
	}
...
void dbconn::insert_addon_author(
...
bool dbconn::do_any_authors_exist(const std::string& instance_version, const std::string& id)
{
	try {
		return exists(connection_,
			"SELECT 1 FROM `" + db_addon_authors_table_ + "` WHERE INSTANCE_VERSION = ? AND ADDON_ID = ?",
			{instance_version, id});
	} catch(const mariadb::exception::base& e) {
		log_sql_exception(
			"Unable to check whether authors exist for " + id + " for version " + instance_version + ".", e);
		return true;
	}
}
...
config dbconn::get_addon_downloads_info(const std::string& instance_version, const std::string& id)
{
	...
	try {
		return get_complex_results(connection_, &addon_downloads_handler,
			"select ADDON_NAME, VERSION, UPLOADED_ON, DOWNLOAD_COUNT from " + db_addon_info_table_
				+ " where INSTANCE_VERSION = ? and ADDON_ID = ? order by ADDON_NAME, UPLOADED_ON",
			{instance_version, id});
	} catch(const mariadb::exception::base& e) {
...
config dbconn::get_forum_auth_usage(const std::string& instance_version)
{
	...
	try {
		return get_complex_results(connection_, &forum_auth_usage_handler,
			"select (select count(distinct ADDON_ID) from " + db_addon_info_table_
				+ " where INSTANCE_VERSION = ?) as ALL_COUNT, "
				  "(select count(distinct ADDON_ID) from "
...
config dbconn::get_addon_admins(int site_admin_group, int forum_admin_group)
{
	...
	try {
		return get_complex_results(connection_, &addon_admin_handler,
			"SELECT u.USERNAME FROM `" + db_users_table_ + "` u, `" + db_user_group_table_
				+ "` ug WHERE u.USER_ID = ug.USER_ID AND ug.GROUP_ID in (?, ?)",
			{site_admin_group, forum_admin_group});
	} catch(const mariadb::exception::base& e) {
		log_sql_exception("Failed to get addon admins for groups '" + std::to_string(site_admin_group) + "' and '"
				+ std::to_string(forum_admin_group) + "'!",
			e);
		return {};
	}
}
...
//
template<typename F>
config dbconn::get_complex_results(
...
//
std::string dbconn::get_single_string(
...
//
mariadb::result_set_ref dbconn::select(
	const mariadb::connection_ref& connection, const std::string& sql, const sql_parameters& params)
{
	try {
...
unsigned long long dbconn::modify(
	const mariadb::connection_ref& connection, const std::string& sql, const sql_parameters& params)
{
	try {
		mariadb::statement_ref stmt = query(connection, sql, params);
...
unsigned long long dbconn::modify_get_id(
	const mariadb::connection_ref& connection, const std::string& sql, const sql_parameters& params)
{
	try {
		mariadb::statement_ref stmt = query(connection, sql, params);
...
mariadb::statement_ref dbconn::query(
	const mariadb::connection_ref& connection, const std::string& sql, const sql_parameters& params)
{
