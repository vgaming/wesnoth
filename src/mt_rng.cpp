...
mt_rng::mt_rng(const config& cfg)
	: random_seed_(42)
	, mt_(random_seed_)
	, // we don't have the seed at construction time, we have to seed after construction in this case. Constructing an
	  // mt19937 is somewhat expensive, apparently has about 2kb of private memory.
	random_calls_(0)
{
...
{
	return random_seed_ == other.random_seed_ && random_calls_ == other.random_calls_ && mt_ == other.mt_;
}
...
uint32_t mt_rng::get_next_random()
{
	uint32_t result = mt_();
	++random_calls_;
	DBG_RND << "pulled user random " << result << " for call " << random_calls_ << " with seed " << std::hex
			<< random_seed_;
...
void mt_rng::seed_random(const uint32_t seed, const unsigned int call_count)
{
	...
	DBG_RND << "Seeded random with " << std::hex << random_seed_ << std::dec << " with " << random_calls_ << " calls.";
}
...
void mt_rng::seed_random(const std::string& seed_str, const unsigned int call_count)
{
	...
	if(!(s >> std::hex >> new_seed)) {
		new_seed = 42;
		DBG_RND << "Failed to seed a random number generator using seed string '" << seed_str
