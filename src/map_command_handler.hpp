...
public:
	...
	struct command
	{
		...
		explicit command(command_handler h,
	...
	void dispatch(std::string cmd)
	{
		...
		if(const command* c = get_command(get_cmd())) {
			...
			if(has_command_proposal) {
				print("help",
					VGETTEXT("Unknown command ‘$command’, did you mean ‘$command_proposal’? try $help_command "
							 "for a list of available commands.",
						symbols));
			} else {
				print("help",
					VGETTEXT("Unknown command ‘$command’, try $help_command "
							 "for a list of available commands.",
						symbols));
			}
		}
...
protected:
	void init_map_default()
	{
		register_command("help", &map_command_handler<Worker>::help,
			...
			_("[all|<command>]\n“all” = overview of all commands, <command> = name of a specific command (provides "
			  "more detail)"));
	}
