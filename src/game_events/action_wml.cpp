...
std::vector<map_location> fake_unit_path(
	const unit& fake_unit, const std::vector<std::string>& xvals, const std::vector<std::string>& yvals)
{
	...
	for(std::size_t i = 0; i != std::min(xvals.size(), yvals.size()); ++i) {
		...
		pathfind::plain_route route = pathfind::a_star_search(src, dst, 10000, calc, game_map->w(), game_map->h());
		...
		if(route.steps.empty()) {
			...
			route = pathfind::a_star_search(src, dst, 10000, emergency_calc, game_map->w(), game_map->h());
			if(route.steps.empty()) {
		...
		path.insert(path.end(), route.steps.begin() + 1, route.steps.end());
...
WML_HANDLER_FUNCTION(do_command, , cfg)
{
	...
	static const std::set<std::string> allowed_tags{
		"attack", "move", "recruit", "recall", "disband", "fire_event", "custom_command"};

	const bool is_too_early
		= resources::gamedata->phase() == game_data::INITIAL || resources::gamedata->phase() == game_data::PRELOAD;
	...
	for(const auto& [key, child] : cfg.all_ordered()) {
		...
		// TODO: afaik run_in_synced_context_if_not_already thows exceptions when the executed action end the scenario
		// or the turn.
		...
		synced_context::run_in_synced_context_if_not_already(key, child.get_parsed_config(), spectator);
...
WML_HANDLER_FUNCTION(move_unit_fake, , cfg)
{
	...
	if(!path.empty()) {
		// Always scroll.
		unit_display::move_unit(
			path, dummy_unit.get_unit_ptr(), true, map_location::direction::indeterminate, force_scroll);
	}
}
...
WML_HANDLER_FUNCTION(move_units_fake, , cfg)
{
	...
	for(std::size_t step = 1; step < longest_path; ++step) {
		...
		for(std::size_t un = 0; un < num_units; ++un) {
			...
			unit_display::move_unit(
				path_step, units[un].get_unit_ptr(), true, map_location::direction::indeterminate, force_scroll);
...
WML_HANDLER_FUNCTION(recall, , cfg)
{
	...
	for(team& t : resources::gameboard->teams()) {
		...
		const unit_filter lfilt(leader_filter); // Note that if leader_filter is null, this correctly gives a null
												// filter that matches all units.
		for(std::vector<unit_ptr>::iterator u = avail.begin(); u != avail.end(); ++u) {
			...
			if(ufilt(*(*u), map_location())) {
				...
				for(unit_map::const_unit_iterator leader : leaders) {
					...
					if(lfilt(*leader) && unit_filter(vconfig(leader->recall_filter())).matches(*(*u), map_location())) {
						...
						if(resources::gameboard->map().on_board(loc)) {
							...
							actions::place_recruit(to_recruit, loc, leader->get_location(), 0, true,
								map_location::parse_direction(cfg["facing"]), cfg["show"].to_bool(true),
...
{
	map_choice(const std::string& filename)
		: filename_(filename)
	{
	}
...
WML_HANDLER_FUNCTION(replace_map, , cfg)
{
	...
	if(map.total_width() > game_map->total_width() || map.total_height() > game_map->total_height()) {
...
WML_HANDLER_FUNCTION(set_variables, , cfg)
{
	...
	if(cfg.has_attribute("to_variable")) {
		try {
			...
			ERR_NG << "Cannot do [set_variables] with invalid to_variable variable: " << cfg["to_variable"] << " with "
				   << cfg.get_config().debug();
		}
	} else {
		...
		for(const vchild& p : cfg.all_ordered()) {
			if(p.first == "value") {
				...
				if(separator_string.size() > 1) {
					ERR_NG << "[set_variables] [split] separator only supports 1 character, multiple passed: "
						   << split_element["separator"] << " with " << cfg.get_config().debug();
				}
				...
				if(separator == nullptr) {
					...
					split_vector = utils::split(split_string, *separator,
						remove_empty ? utils::REMOVE_EMPTY | utils::STRIP_SPACES : utils::STRIP_SPACES);
				}
	...
	try {
		...
		ERR_NG << "Cannot do [set_variables] with invalid destination variable: " << name << " with "
			   << cfg.get_config().debug();
	}
}
...
WML_HANDLER_FUNCTION(store_relative_direction, , cfg)
{
	...
	try {
		...
		ERR_NG << "Cannot do [store_relative_direction] with invalid destination variable: " << variable << " with "
			   << cfg.get_config().debug();
	}
}
...
WML_HANDLER_FUNCTION(store_rotate_map_location, , cfg)
{
	...
	try {
		...
		ERR_NG << "Cannot do [store_rotate_map_location] with invalid destination variable: " << variable << " with "
			   << cfg.get_config().debug();
	}
}
...
WML_HANDLER_FUNCTION(tunnel, , cfg)
{
	...
	} else if(cfg.get_children("source").empty() || cfg.get_children("target").empty()
		|| cfg.get_children("filter").empty()) {
		ERR_WML << "[tunnel] is missing a mandatory tag:\n" << cfg.get_config().debug();
	} else if(cfg.get_children("source").size() > 1 || cfg.get_children("target").size() > 1
...
WML_HANDLER_FUNCTION(unit, , cfg)
{
	...
	if(!to_variable.blank()) {
		...
		try {
			...
			var.clear();
			new_unit->write(var);
			if(const config::attribute_value* v = parsed_cfg.get("x"))
				var["x"] = *v;
			if(const config::attribute_value* v = parsed_cfg.get("y"))
				var["y"] = *v;
		} catch(const invalid_variablename_exception&) {
			ERR_NG << "Cannot do [unit] with invalid to_variable:  " << to_variable << " with "
				   << cfg.get_config().debug();
		}
		return;
	}
	...
	if((side < 1) || (side > static_cast<int>(resources::gameboard->teams().size()))) {
		ERR_NG << "wrong side in [unit] tag - no such side: " << side
			   << " ( number of teams :" << resources::gameboard->teams().size() << ")";
