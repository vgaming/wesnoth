 ...
 */
entity_location::entity_location(const map_location& loc, std::size_t id, const map_location& filter_loc)
	: map_location(loc)
	, id_(id)
...
bool entity_location::matches_unit_filter(const unit_map::const_iterator& un_it, const unit_filter& filter) const
{
	...
	return filter.matches(*un_it, filter_loc_) && matches_unit(un_it);
}
...
unit_const_ptr entity_location::get_unit() const
{
	...
	const auto un_it = id_ == 0 ? resources::gameboard->units().find(*this) : resources::gameboard->units().find(id_);
