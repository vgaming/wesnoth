...
#endif
{
	...
	const uint8_t reference_avg = old_rgb.empty() ? 255u : (old_rgb[0].r + old_rgb[0].g + old_rgb[0].b) / 3;
	...
	for(const color_t& old_c : old_rgb) {
		...
		if(reference_avg && old_avg <= reference_avg) {
			...
			clist.emplace_back(std::min(255u, static_cast<uint32_t>(old_ratio * mid_c.r + (1 - old_ratio) * min_c.r)),
			...
			clist.emplace_back(std::min(255u, static_cast<uint32_t>(old_ratio * mid_c.r + (1 - old_ratio) * max_c.r)),
...
std::string color_range::debug() const
{
	...
	o << '{' << mid_.to_hex_string() << ',' << max_.to_hex_string() << ',' << min_.to_hex_string() << ','
	  << rep_.to_hex_string() << '}';
