...
bool simulated_attack(
	const map_location& attacker_loc, const map_location& defender_loc, double attacker_hp, double defender_hp)
{
	...
	LOG_AI_SIM_ACTIONS << attack_unit->type_name() << " at " << attacker_loc << " attack " << defend_unit->type_name()
					   << " at " << defender_loc;
...
bool simulated_move(int side, const map_location& from, const map_location& to, int steps, map_location& unit_location)
{
	...
	move_unit->set_movement(
		move_unit->movement_left() - steps); // Following original logic, remove_movement_ will be considered outside.
...
bool simulated_recall(int side, const std::string& unit_id, const map_location& recall_location)
{
	...
	LOG_AI_SIM_ACTIONS << "recall " << recall_unit->type_name() << " at " << recall_location << " spend "
					   << own_team.recall_cost() << " gold";
...
bool simulated_recruit(int side, const unit_type* u, const map_location& recruit_location)
{
	...
	unit_ptr recruit_unit = unit::create(
	...
	LOG_AI_SIM_ACTIONS << "recruit " << u->type_name() << " at " << recruit_location << " spend " << u->cost()
					   << " gold";
...
bool simulated_stopunit(const map_location& unit_location, bool remove_movement, bool remove_attacks)
{
	...
	if(remove_movement) {
		...
		LOG_AI_SIM_ACTIONS << "remove (" << stop_unit->get_location() << ") " << stop_unit->type_name()
						   << "'s movement";
		changed = true;
	}
	if(remove_attacks) {
