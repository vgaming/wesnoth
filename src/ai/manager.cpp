...
holder::holder(side_number side, const config& cfg)
	: ai_()
	, side_context_(nullptr)
	, readonly_context_(nullptr)
	, readwrite_context_(nullptr)
	, default_ai_context_(nullptr)
	, side_(side)
	, cfg_(cfg)
{
...
std::string holder::get_ai_overview()
{
	...
	// These assignments are necessary because the code will otherwise not compile on some platforms with an
	// lvalue/rvalue mismatch error
...
const std::string manager::internal_evaluate_command(side_number side, const std::string& str)
{
	...
	if(str == "!") {
		...
		return evaluate_command(
			side, history_.back().get_command()); // no infinite loop since '!' commands are not present in history
	};
	// show last command
	if(str == "?") {
		// this command should not be recorded in history
	...
	if(cmd.size() == 3) {
		...
		if(cmd.at(0) == "!") {
			...
			if(j != history_.rend()) {
				return evaluate_command(
					side, j->get_command()); // no infinite loop since '!' commands are not present in history
			}
