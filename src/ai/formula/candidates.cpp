...
base_candidate_action::base_candidate_action(
	const std::string& name, const std::string& type, const config& cfg, function_symbol_table* function_table)
	: name_(name)
	, type_(type)
...
int base_candidate_action::execute_formula(
	const const_formula_ptr& formula, const formula_callable& callable, const ai::formula_ai* ai)
{
	int res = 0;
	try {
	...
	, filter_map_()
{
	...
	if(filter_params) {
		for(const auto& [key, value] : filter_params->attribute_range()) {
			const_formula_ptr filter_formula(new formula(value, function_table));
...
variant candidate_action_with_filters::do_filtering(
	ai::formula_ai* ai, variant& input, const const_formula_ptr& formula)
{
...
move_candidate_action::move_candidate_action(
	const std::string& name, const std::string& type, const config& cfg, function_symbol_table* function_table)
...
attack_candidate_action::attack_candidate_action(
	const std::string& name, const std::string& type, const config& cfg, function_symbol_table* function_table)
	: candidate_action_with_filters(name, type, cfg, function_table)
	, my_unit_()
	, enemy_unit_()
