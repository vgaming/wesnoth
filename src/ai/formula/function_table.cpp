...
DEFINE_FAI_FUNCTION(distance_to_nearest_unowned_village, 1, 1)
{
	const map_location loc
		= args()[0]
			  ->evaluate(variables, add_debug_info(fdb, 0, "distance_to_nearest_unowned_village:location"))
...
void find_movemap(const unit_adapter& u,
	const map_location& loc,
...
DEFINE_FAI_FUNCTION(calculate_map_ownership, 2, 5)
{
	...
	for(unit_map::const_iterator i = resources::gameboard->units().begin(); i != resources::gameboard->units().end();
	++i) { unit_counter[i->second.side()-1]++; unit_adapter unit(i->second); find_movemap( resources::gameboard->map(),
	resources::gameboard->units(), unit, i->first, scores[i->second.side()-1], ai_.resources::gameboard->teams() , true
	);
	}
	*/
	...
	for(int x = 0; x < w; ++x) {
		for(int y = 0; y < h; ++y) {
			...
			if(valid) {
				if(enemy_border)
					res.emplace(variant(std::make_shared<location_callable>(map_location(x, y))),
						variant(scores[0][i] + 10000));
				else
					res.emplace(
						variant(std::make_shared<location_callable>(map_location(x, y))), variant(scores[0][i]));
			}
		}
	}
	return variant(res);
...
{
	const map_location starting_loc = args()[0]
										  ->evaluate(variables, add_debug_info(fdb, 0, "castle_locs:location"))
										  .convert_to<location_callable>()
										  ->loc();
...
DEFINE_WFL_FUNCTION(timeofday_modifier, 1, 2)
{
	...
	if(args().size() == 2) {
		loc = args()[1]
				  ->evaluate(variables, add_debug_info(fdb, 1, "timeofday_modifier:location"))
				  .convert_to<location_callable>()
				  ->loc();
	}
	...
	return variant(combat_modifier(
		resources::gameboard->units(), resources::gameboard->map(), loc, un.alignment(), un.is_fearless()));
}

DEFINE_FAI_FUNCTION(nearest_keep, 1, 1)
{
	const map_location loc = args()[0]
								 ->evaluate(variables, add_debug_info(fdb, 0, "nearest_keep:location"))
								 .convert_to<location_callable>()
								 ->loc();
	int best = 1000000;
	int best_i = -1;
	...
	if(best_i != -1)
		return variant(
			std::make_shared<location_callable>(ai_.get_keeps_cache()[best_i].convert_to<location_callable>()->loc()));
	else
		return variant();
}
...
{
	const map_location loc = args()[0]
								 ->evaluate(variables, add_debug_info(fdb, 0, "suitable_keep:location"))
								 .convert_to<location_callable>()
								 ->loc();
...
DEFINE_FAI_FUNCTION(find_shroud, 0, 1)
{
	...
	if(args().size() == 1) {
		const gamemap& m = args()[0]
							   ->evaluate(variables, add_debug_info(fdb, 0, "find_shroud:gamemap"))
							   .convert_to<gamemap_callable>()
							   ->get_gamemap();
		w = m.w();
		h = m.h();
	} else {
...
DEFINE_FAI_FUNCTION(close_enemies, 2, 2)
{
	std::vector<variant> vars;
	const map_location loc = args()[0]
								 ->evaluate(variables, add_debug_info(fdb, 0, "close_enemies:location"))
								 .convert_to<location_callable>()
								 ->loc();
...
DEFINE_WFL_FUNCTION(calculate_outcome, 3, 4)
{
	...
	int weapon;
	if(args().size() > 3)
		weapon = args()[3]->evaluate(variables, add_debug_info(fdb, 3, "calculate_outcome:weapon")).as_int();
	else
		weapon = -1;
	...
	if(units.count(attacker_location) == 0) {
		ERR_AI << "Performing calculate_outcome() with non-existent attacker at (" << attacker_location.wml_x() << ","
			   << attacker_location.wml_y() << ")";
		return variant();
	}
	...
	if(units.count(defender_location) == 0) {
		ERR_AI << "Performing calculate_outcome() with non-existent defender at (" << defender_location.wml_x() << ","
			   << defender_location.wml_y() << ")";
		return variant();
	}

	battle_context bc(units,
		args()[1]
			->evaluate(variables, add_debug_info(fdb, 1, "calculate_outcome:attacker_attack_location"))
			.convert_to<location_callable>()
			->loc(),
	...
	if(bc.get_defender_stats().petrifies
	...
	if(bc.get_defender_combatant().slowed != 0)
		status.emplace_back("Slowed");
	if(bc.get_attacker_stats().petrifies
...
DEFINE_WFL_FUNCTION(outcomes, 1, 1)
{
	...
	if(analysis->chance_to_kill > 0.0) {
		...
		vars.emplace_back(
			std::make_shared<position_callable>(/*&units,*/ static_cast<int>(analysis->chance_to_kill * 100)));
	}

	if(analysis->chance_to_kill < 1.0) {
		// unit_map units(units_with_moves);
		vars.emplace_back(
			std::make_shared<position_callable>(/*&units,*/ static_cast<int>(100 - analysis->chance_to_kill * 100)));
	}
...
DEFINE_WFL_FUNCTION(recall, 1, 2)
{
	...
	if(args().size() >= 2) {
		loc = args()[1]
				  ->evaluate(variables, add_debug_info(fdb, 1, "recall:location"))
				  .convert_to<location_callable>()
				  ->loc();
	}
...
DEFINE_WFL_FUNCTION(recruit, 1, 2)
{
	...
	if(args().size() >= 2) {
		loc = args()[1]
				  ->evaluate(variables, add_debug_info(fdb, 1, "recruit:location"))
				  .convert_to<location_callable>()
				  ->loc();
	}
...
DEFINE_FAI_FUNCTION(shortest_path, 2, 3)
{
	...
	const map_location src = args()[0]
								 ->evaluate(variables, add_debug_info(fdb, 0, "shortest_path:src"))
								 .convert_to<location_callable>()
								 ->loc();
	const map_location dst = args()[1]
								 ->evaluate(variables, add_debug_info(fdb, 1, "shortest_path:dst"))
								 .convert_to<location_callable>()
								 ->loc();
	...
	if(args().size() > 2)
		unit_loc = args()[2]
					   ->evaluate(variables, add_debug_info(fdb, 2, "shortest_path:unit_location"))
					   .convert_to<location_callable>()
					   ->loc();
	else
		unit_loc = src;
	...
	for(std::vector<map_location>::const_iterator loc_iter = route.steps.begin() + 1; loc_iter != route.steps.end();
		++loc_iter) {
...
DEFINE_FAI_FUNCTION(simplest_path, 2, 3)
{
	...
	const map_location src = args()[0]
								 ->evaluate(variables, add_debug_info(fdb, 0, "simplest_path:src"))
								 .convert_to<location_callable>()
								 ->loc();
	const map_location dst = args()[1]
								 ->evaluate(variables, add_debug_info(fdb, 1, "simplest_path:dst"))
								 .convert_to<location_callable>()
								 ->loc();
	map_location unit_loc;
	...
	if(args().size() > 2)
		unit_loc = args()[2]
					   ->evaluate(variables, add_debug_info(fdb, 2, "simplest_path:unit_location"))
					   .convert_to<location_callable>()
					   ->loc();
	else
		unit_loc = src;
	...
	pathfind::plain_route route = pathfind::a_star_search(src, dst, 1000.0, em_calc, resources::gameboard->map().w(),
	...
	for(std::vector<map_location>::const_iterator loc_iter = route.steps.begin() + 1; loc_iter != route.steps.end();
		++loc_iter) {
		if(unit_it->movement_cost(static_cast<const game_board*>(resources::gameboard)->map()[*loc_iter])
			< movetype::UNREACHABLE) {
			locations.emplace_back(std::make_shared<location_callable>(*loc_iter));
		} else {
			break;
		}
...
DEFINE_FAI_FUNCTION(next_hop, 2, 3)
{
	const map_location src
		= args()[0]->evaluate(variables, add_debug_info(fdb, 0, "next_hop:src")).convert_to<location_callable>()->loc();
	const map_location dst
		= args()[1]->evaluate(variables, add_debug_info(fdb, 1, "next_hop:dst")).convert_to<location_callable>()->loc();
	map_location unit_loc;
	...
	if(args().size() > 2)
		unit_loc = args()[2]
					   ->evaluate(variables, add_debug_info(fdb, 2, "next_hop:unit_location"))
					   .convert_to<location_callable>()
					   ->loc();
	else
		unit_loc = src;
...
{
	const map_location src
		= args()[0]->evaluate(variables, add_debug_info(fdb, 0, "move:src")).convert_to<location_callable>()->loc();
	const map_location dst
		= args()[1]->evaluate(variables, add_debug_info(fdb, 1, "move:dst")).convert_to<location_callable>()->loc();
	LOG_AI << "move(): " << src << ", " << dst << ")";
...
{
	const map_location src = args()[0]
								 ->evaluate(variables, add_debug_info(fdb, 0, "move_partial:src"))
								 .convert_to<location_callable>()
								 ->loc();
	const map_location dst = args()[1]
								 ->evaluate(variables, add_debug_info(fdb, 1, "move_partial:dst"))
								 .convert_to<location_callable>()
								 ->loc();
...
{
	return variant(std::make_shared<set_unit_var_callable>(
		args()[0]->evaluate(variables, add_debug_info(fdb, 0, "set_unit_var:key")).as_string(),
		args()[1]->evaluate(variables, add_debug_info(fdb, 1, "set_unit_var:value")),
		args()[2]
			->evaluate(variables, add_debug_info(fdb, 2, "set_unit_var:unit_location"))
			.convert_to<location_callable>()
			->loc()));
}
...
{
	const map_location move_from = args()[0]
									   ->evaluate(variables, add_debug_info(fdb, 0, "attack:move_from"))
									   .convert_to<location_callable>()
									   ->loc();
	const map_location src
		= args()[1]->evaluate(variables, add_debug_info(fdb, 1, "attack:src")).convert_to<location_callable>()->loc();
	const map_location dst
		= args()[2]->evaluate(variables, add_debug_info(fdb, 2, "attack:dst")).convert_to<location_callable>()->loc();
	const int weapon
		= args().size() == 4 ? args()[3]->evaluate(variables, add_debug_info(fdb, 3, "attack:weapon")).as_int() : -1;
...
{
	const gamemap& m = args()[0]
						   ->evaluate(variables, add_debug_info(fdb, 0, "is_village:map"))
						   .convert_to<gamemap_callable>()
						   ->get_gamemap();
	...
	if(args().size() == 2) {
		loc = args()[1]
				  ->evaluate(variables, add_debug_info(fdb, 1, "is_village:location"))
				  .convert_to<location_callable>()
				  ->loc();
	} else {
...
{
	const gamemap& m = args()[0]
						   ->evaluate(variables, add_debug_info(fdb, 0, "is_unowned_village:map"))
						   .convert_to<gamemap_callable>()
						   ->get_gamemap();
	const std::set<map_location>& my_villages = ai_.current_team().villages();
	...
	if(args().size() == 2) {
		loc = args()[1]
				  ->evaluate(variables, add_debug_info(fdb, 1, "is_unowned_village:location"))
				  .convert_to<location_callable>()
				  ->loc();
	} else {
...
DEFINE_WFL_FUNCTION(units_can_reach, 2, 2)
{
	...
	std::pair<ai::move_map::const_iterator, ai::move_map::const_iterator> range = dstsrc.equal_range(args()[1]
			->evaluate(variables, add_debug_info(fdb, 1, "units_can_reach:possible_move_list"))
			.convert_to<location_callable>()
			->loc());
...
public:
	ai_formula_function(const std::string& name, ai::formula_ai& ai)
...
ai_function_symbol_table::ai_function_symbol_table(ai::formula_ai& ai)
	: function_symbol_table(
		  std::make_shared<gamestate_function_symbol_table>(std::make_shared<action_function_symbol_table>()))
{
