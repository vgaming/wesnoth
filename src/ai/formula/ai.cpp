...
void formula_ai::display_message(const std::string& msg) const
{
	game_display::get_singleton()->get_chat_manager().add_chat_message(
		std::time(nullptr), "wfl", get_side(), msg, events::chat_handler::MESSAGE_PUBLIC, false);
}
...
void formula_ai::add_formula_function(const std::string& name,
	const const_formula_ptr& formula,
	const const_formula_ptr& precondition,
...
bool formula_ai::can_reach_unit(map_location unit_A, map_location unit_B) const
{
	...
	std::pair<move_map::const_iterator, move_map::const_iterator> unit_moves;
	...
	for(const config& func : cfg_.child_range("function")) {
		...
		try {
			add_formula_function(
				name, create_optional_formula(formula_str), create_optional_formula(func["precondition"]), args);
		} catch(const formula_error& e) {
