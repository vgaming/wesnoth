...
void ai_testing::log_game_start()
{
	for(const team& tm : resources::gameboard->teams()) {
		...
		LOG_AI_TESTING << "AI_IDENTIFIER " << side << ": "
					   << ai::manager::get_singleton().get_active_ai_identifier_for_side(side);
		resources::recorder->add_log_data("ai_log", "ai_id" + std::to_string(side),
			ai::manager::get_singleton().get_active_ai_identifier_for_side(side));
	}
...
void ai_testing::log_game_end()
{
	...
	resources::recorder->add_log_data("ai_log", "end_turn", std::to_string(resources::tod_manager->turn()));
	for(const team& tm : resources::gameboard->teams()) {
		int side = tm.side();
		resources::recorder->add_log_data("ai_log", "end_gold" + std::to_string(side), std::to_string(tm.gold()));
		resources::recorder->add_log_data(
			"ai_log", "end_units" + std::to_string(side), std::to_string(resources::gameboard->side_units(side)));
	}
}
