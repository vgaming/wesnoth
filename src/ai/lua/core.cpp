...
static int impl_ai_aspect_get(lua_State* L)
{
	...
	if(typesafe_aspect<attacks_vector>* aspect_as_attacks_vector = try_aspect_as<attacks_vector>(iter->second)) {
		...
		while(real_aspect == nullptr) {
			...
			composite_aspect<attacks_vector>& composite
				= dynamic_cast<composite_aspect<attacks_vector>&>(*aspect_as_attacks_vector);
		...
		for(unit_map::const_iterator u = resources::gameboard->units().begin();
			u != resources::gameboard->units().end(); ++u) {
			if(!u.valid()) {
				continue;
			}
	...
	} else if(typesafe_aspect<unit_advancements_aspect>* aspect_as_unit_advancements_aspects
		= try_aspect_as<unit_advancements_aspect>(iter->second)) {
...
static int impl_ai_aspect_list(lua_State* L)
{
	...
	std::transform(
		aspects.begin(), aspects.end(), std::back_inserter(aspect_names), std::mem_fn(&aspect_map::value_type::first));
...
static luaL_Reg const mutating_callbacks[]
	= {{"attack", &cfun_ai_execute_attack}, {"move", &cfun_ai_execute_move_partial},
		{"move_full", &cfun_ai_execute_move_full}, {"recall", &cfun_ai_execute_recall},
		{"recruit", &cfun_ai_execute_recruit}, {"stopunit_all", &cfun_ai_execute_stopunit_all},
		{"stopunit_attacks", &cfun_ai_execute_stopunit_attacks}, {"stopunit_moves", &cfun_ai_execute_stopunit_moves},
		{"fallback_human", &cfun_ai_fallback_human}, {nullptr, nullptr}};
...
static void generate_and_push_ai_table(lua_State* L, ai::engine_lua* engine)
{
	...
	static luaL_Reg const callbacks[] = {// Move maps
		{"get_new_dst_src", &cfun_ai_get_dstsrc}, {"get_new_src_dst", &cfun_ai_get_srcdst},
		{"get_new_enemy_dst_src", &cfun_ai_get_enemy_dstsrc}, {"get_new_enemy_src_dst", &cfun_ai_get_enemy_srcdst},
		...
		{"is_dst_src_valid", &cfun_ai_is_dst_src_valid}, {"is_enemy_dst_src_valid", &cfun_ai_is_dst_src_enemy_valid},
		...
		{"get_aggression", &cfun_ai_get_aggression}, {"get_avoid", &cfun_ai_get_avoid},
		{"get_caution", &cfun_ai_get_caution}, {"get_grouping", &cfun_ai_get_grouping},
		{"get_leader_aggression", &cfun_ai_get_leader_aggression}, {"get_leader_goal", &cfun_ai_get_leader_goal},
		{"get_leader_ignores_keep", &cfun_ai_get_leader_ignores_keep}, {"get_leader_value", &cfun_ai_get_leader_value},
		...
		{"get_support_villages", &cfun_ai_get_support_villages}, {"get_village_value", &cfun_ai_get_village_value},
		{"get_villages_per_scout", &cfun_ai_get_villages_per_scout},
		// End of aspects
		{"suitable_keep", &cfun_ai_get_suitable_keep}, {"check_recall", &cfun_ai_check_recall},
		{"check_move", &cfun_ai_check_move}, {"check_stopunit", &cfun_ai_check_stopunit},
		{"check_attack", &cfun_ai_check_attack}, {"check_recruit", &cfun_ai_check_recruit}, {nullptr, nullptr}};
...
void lua_ai_action_handler::handle(
	const config& cfg, const config& filter_own, bool read_only, const lua_object_ptr& l_obj)
{
