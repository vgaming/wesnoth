...
int default_ai_context_impl::count_free_hexes_in_castle(const map_location& loc, std::set<map_location>& checked_hexes)
{
	...
	for(const map_location& adj : get_adjacent_tiles(loc)) {
		...
		if(resources::gameboard->map().is_castle(adj)) {
			...
			if(u == units_.end() || (current_team().is_enemy(u->side()) && u->invisible(adj))
				|| ((&resources::gameboard->get_team(u->side()) == &current_team()) && u->movement_left() > 0)) {
				ret += 1;
			}
		}
	}
...
std::vector<target> default_ai_context_impl::find_targets(const move_map& enemy_dstsrc)
{
	...
	if(has_leader && village_value > 0.0) {
		...
		for(const map_location& village_loc : map_.villages()) {
			...
			if(ally_village) {
				...
				LOG_AI << "found village target... " << village_loc << " with value: " << value
	...
	if(get_leader_value() > 0.0) {
		for(u = units_.begin(); u != units_.end(); ++u) {
			...
			if(u->can_recruit() && current_team().is_enemy(u->side()) && !u->invisible(u->get_location())) {
				assert(map_.on_board(u->get_location()));
				LOG_AI << "found enemy leader (side: " << u->side() << ") target... " << u->get_location()
					   << " with value: " << get_leader_value();
	...
	for(std::vector<goal_ptr>::iterator j = goals.begin(); j != goals.end(); ++j) {
		if(!(*j)->active()) {
			continue;
		}
	...
	for(std::vector<target>::iterator i = targets.begin(); i != targets.end(); ++i) {
		...
		for(std::vector<target>::const_iterator j = targets.begin(); j != targets.end(); ++j) {
			...
			const double distance = std::abs(j->loc.x - i->loc.x) + std::abs(j->loc.y - i->loc.y);
