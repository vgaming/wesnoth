	...
	double aggression)
{
	...
	target_value += (static_cast<double>(defend_it->experience()) / static_cast<double>(defend_it->max_experience()))
		* target_value;
	target_starting_damage = defend_it->max_hitpoints() - defend_it->hitpoints();
	...
	for(std::size_t i = 0; i != movements.size(); ++i) {
		...
		alternative_terrain_quality
			+= cost * ai_obj.best_defensive_position(movements[i].first, dstsrc, srcdst, enemy_dstsrc).chance_to_hit;
	}
	...
	for(m = movements.begin(); m != movements.end(); ++m) {
		...
		if(usc != ai_obj.unit_stats_cache().end()
			&& usc->second.first.attack_num < static_cast<int>(up->attacks().size())) {
			from_cache = true;
		...
		if(!from_cache) {
			ai_obj.unit_stats_cache().emplace(cache_key, std::pair(bc->get_attacker_stats(), bc->get_defender_stats()));
		}
		...
		if(!bc->get_defender_stats().is_poisoned) {
			avg_damage_inflicted
				+= game_config::poison_amount * 2 * bc->get_defender_combatant().poisoned * (1 - prob_killed);
		}
		...
		if(!up->advances_to().empty()) {
			...
			if(fight_xp >= xp_for_advance) {
				...
				avg_losses -= up->cost() * 0.25 * fight_xp * (prob_fought - prob_killed) / xp_for_advance;
			} else {
				avg_losses -= up->cost() * 0.25 * (kill_xp * prob_killed + fight_xp * (prob_fought - prob_killed))
					/ xp_for_advance;
			}
	...
	if(!defend_it->advances_to().empty() && def_avg_experience >= defend_it->experience_to_advance()) {
		...
		avg_damage_inflicted
			+= defend_it->hitpoints() - prev_def->average_hp(map.gives_healing(defend_it->get_location()));
	}
...
double attack_analysis::rating(double aggression, const readonly_context& ai_obj) const
{
	...
	if(terrain_quality > alternative_terrain_quality) {
		...
		const double exposure = exposure_mod * resources_used * (terrain_quality - alternative_terrain_quality)
			* vulnerability / std::max<double>(0.01, support);
		LOG_AI << "attack option has base value " << value << " with exposure " << exposure << ": " << vulnerability
			   << "/" << support << " = " << (vulnerability / std::max<double>(support, 0.1));
	...
	if(!is_surrounded || (support != 0 && avg_damage_taken != 0)) {
		...
		if(vulnerability > 50.0 && vulnerability > support * 2.0 && chance_to_kill < 0.02 && aggression < 0.75
			&& !attack_close(target)) {
			return -1.0;
		}
	}
	...
	LOG_AI << "attack on " << target << ": attackers: " << movements.size() << " value: " << value
		   << " chance to kill: " << chance_to_kill << " damage inflicted: " << avg_damage_inflicted
		   << " damage taken: " << avg_damage_taken << " vulnerability: " << vulnerability << " support: " << support
		   << " quality: " << terrain_quality << " alternative quality: " << alternative_terrain_quality;
...
wfl::variant attack_analysis::execute_self(wfl::variant ctxt)
{
	...
	if(unit == units.end()) {
		return wfl::variant(
			std::make_shared<wfl::safe_call_result>(fake_ptr(), attack_result::E_EMPTY_DEFENDER, move_from));
	}
	...
	if(move_from != att_src) {
		...
		if(!result->is_ok()) {
			...
			return wfl::variant(
				std::make_shared<wfl::safe_call_result>(fake_ptr(), result->get_status(), result->get_unit_location()));
		}
	}

	if(units.count(att_src)) {
		ai::attack_result_ptr result
