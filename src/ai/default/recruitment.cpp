...
std::string data::to_string() const
{
	...
	for(const score_map::value_type& entry : scores) {
		s << std::setw(20) << entry.first << " score: " << std::setw(7) << entry.second << "\n";
	}
	s << "----------------------------------------------------\n";
	return s.str();
}
...
action_result_ptr recruitment::execute_recall(const std::string& id, data& leader_data)
{
	...
	recall_result = check_recall_action(id, map_location::null_location(), leader_data.leader->get_location());
...
double recruitment::recall_unit_value(const unit_const_ptr& recall_unit) const
{
	...
	double xp_quantity = static_cast<double>(recall_unit->experience()) / recall_unit->max_experience();
...
const std::string* recruitment::get_appropriate_recall(const std::string& type, const data& leader_data) const
{
 ...
 */
data* recruitment::get_best_leader_from_ratio_scores(std::vector<data>& leader_data, const config* job) const
{
	assert(job);
 ...
 */
const std::string recruitment::get_best_recruit_from_scores(const data& leader_data, const config* job)
{
	assert(job);
...
void recruitment::compare_cost_maps_and_update_important_hexes(
	const pathfind::full_cost_map& my_cost_map, const pathfind::full_cost_map& enemy_cost_map)
{
	const gamemap& map = resources::gameboard->map();
...
double recruitment::get_average_defense(const std::string& u_type) const
{
	...
	double average_defense = (total_terrains == 0) ? 0.0 : static_cast<double>(summed_defense) / total_terrains;
...
const pathfind::full_cost_map recruitment::get_cost_map_of_side(int side) const
{
	...
	for(const unit& unit : units) {
		if(unit.side() != side || unit.can_recruit() || unit.incapacitated() || unit.total_movement() <= 0) {
			continue;
		}
		++unit_count;
...
void recruitment::update_important_hexes()
{
	...
	for(const unit& unit : units) {
		...
		if(is_enemy_in_radius(unit.get_location(), 1)) {
			...
			std::copy(
				surrounding.begin(), surrounding.end(), std::inserter(important_hexes_, important_hexes_.begin()));
 ...
 */
const double* recruitment::get_cached_combat_value(
	const std::string& a, const std::string& b, double a_defense, double b_defense)
{
...
struct attack_simulation
{
	...
	attack_simulation(const unit_type* attacker,
		const unit_type* defender,
		double attacker_defense,
		double defender_defense,
		const const_attack_ptr& att_weapon,
		const const_attack_ptr& def_weapon,
		int average_lawful_bonus)
		: attacker_type(attacker)
		, defender_type(defender)
		, attacker_stats(
			  attacker, att_weapon, true, defender, def_weapon, std::round(defender_defense), average_lawful_bonus)
		, defender_stats(
			  defender, def_weapon, false, attacker, att_weapon, std::round(attacker_defense), average_lawful_bonus)
	...
	bool better_result(const attack_simulation* other, bool for_defender)
	{
		...
		if(for_defender) {
			return battle_context::better_combat(
				defender_combatant, attacker_combatant, other->defender_combatant, other->attacker_combatant, 0);
		} else {
			return battle_context::better_combat(
				attacker_combatant, defender_combatant, other->attacker_combatant, other->defender_combatant, 0);
		}
	}
 ...
 */
void recruitment::simulate_attack(const unit_type* const attacker,
	const unit_type* const defender,
	double attacker_defense,
	double defender_defense,
	double* damage_to_attacker,
	double* damage_to_defender) const
{
	...
	for(const attack_type& att_weapon : attacker_weapons) {
		...
		for(const attack_type& def_weapon : defender_weapons) {
			...
			auto simulation = std::make_shared<attack_simulation>(attacker, defender, attacker_defense,
				defender_defense, att_weapon.shared_from_this(), def_weapon.shared_from_this(), average_lawful_bonus_);
		...
		if(!best_def_response) {
			...
			best_def_response.reset(new attack_simulation(attacker, defender, attacker_defense, defender_defense,
 ...
 * type-attribute.
 */
const std::string recruitment::get_random_pattern_type_if_exists(const data& leader_data, const config* job) const
{
	...
	if(job->operator[]("pattern").to_bool(false)) {
		...
		if(job_types.empty()) {
			...
			std::copy(leader_data.recruits.begin(), leader_data.recruits.end(), std::back_inserter(job_types));
		}
...
void recruitment::integrate_recruitment_pattern_in_recruitment_instructions()
{
	...
	config job;
	std::stringstream s;
	for(std::vector<std::string>::const_iterator type = recruitment_pattern.begin(); type != recruitment_pattern.end();
		++type) {
		s << *type;
 ...
 */
bool recruitment::recruit_matches_types(const std::string& recruit, const std::vector<std::string>& types) const
{
...
double recruitment::get_unit_ratio() const
{
	...
	for(const unit& unit : units) {
		...
		double value = unit.cost() * static_cast<double>(unit.hitpoints()) / static_cast<double>(unit.max_hitpoints());
...
void recruitment::update_own_units_count()
{
	...
	const unit_map& units = resources::gameboard->units();
	for(const unit& unit : units) {
		if(unit.side() != get_side() || unit.can_recruit() || unit.incapacitated() || unit.total_movement() <= 0) {
			continue;
		}
...
void recruitment::recruit_situation_change_observer::handle_generic_event(const std::string& event)
{
...
{
	jobs.emplace_back(std::make_shared<recruit_job>(utils::split(job["type"]), job["leader_id"], job["id"],
		job["number"].to_int(-1), job["importance"].to_int(1), job["total"].to_bool(false),
		job["blocker"].to_bool(true), job["pattern"].to_bool(true)));
}

void recruitment_aspect::create_limit(std::vector<std::shared_ptr<recruit_limit>>& limits, const config& lim)
{
	limits.emplace_back(std::make_shared<recruit_limit>(utils::split(lim["type"]), lim["id"], lim["max"].to_int(0)));
}
