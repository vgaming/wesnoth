...
public:
	remove_wrong_targets(const readonly_context& context)
		: avoid_(context.get_avoid())
		, map_(resources::gameboard->map())
	{
	}
...
void move_to_targets_phase::execute()
{
	...
	while(true) {
		if(targets.empty()) {
			...
			targets.insert(targets.end(), additional_targets().begin(), additional_targets().end());
		...
		assert(resources::gameboard->map().on_board(move.first) && resources::gameboard->map().on_board(move.second));
...
double move_to_targets_phase::rate_target(const target& tg,
...
std::pair<map_location, map_location> move_to_targets_phase::choose_move(std::vector<target>& targets)
{
	...
	for(u = units_.begin(); u != units_.end(); ++u) {
		if(!(u->side() != get_side() || (u->can_recruit() && !is_keep_ignoring_leader(u->id()))
			   || u->movement_left() <= 0 || u->incapacitated())) {
	...
	for(u = units_.begin(); u != units_.end(); ++u) {
		if(!(u->side() != get_side() || (u->can_recruit() && !is_keep_ignoring_leader(u->id()))
			   || u->movement_left() <= 0 || u->incapacitated() || !is_allowed_unit(*u))) {
			break;
		}
	}
	...
	for(; rated_tg != rated_targets.end(); ++rated_tg) {
		...
		pathfind::plain_route real_route
	...
	if(simple_targeting == false) {
		...
		for(++u; u != units_.end(); ++u) {
			if(u->side() != get_side() || (u->can_recruit() && !is_keep_ignoring_leader(u->id()))
				|| u->movement_left() <= 0 || u->get_state("guardian") || u->incapacitated() || !is_allowed_unit(*u)) {
				continue;
			}
			...
			const double locStopValue = 500.0;
			const pathfind::teleport_map allowed_teleports = pathfind::get_teleport_locations(*u, current_team());
			pathfind::plain_route cur_route = pathfind::a_star_search(
		...
		if(locs.empty() == false) {
			...
			for(std::vector<map_location>::const_iterator i = locs.begin(); i != locs.end(); ++i) {
				...
				if(best_loc.valid() == false || defense < best_defense
					|| (defense == best_defense && vulnerability < best_vulnerability)) {
					best_loc = *i;
	...
	if(get_grouping() != "no") {
		...
		for(std::vector<map_location>::const_iterator i = best_route.steps.begin();
			i != best_route.steps.end() && movement > 0; ++i) {
			// FIXME: suokko multiplied by 10 * get_caution(). ?
			...
			if((threat >= best->hitpoints() && threat > power_projection(*i, fullmove_dstsrc))
				|| (i + 1 >= best_route.steps.end() - 1 && unit_at_target != units_.end()
	...
	if(dangerous) {
		...
		if(our_strength > 0.5 + get_caution()) {
			...
			for(move_map::const_iterator i = itors.first; i != itors.second; ++i) {
				...
				if(best_loc.valid() == false
					|| (threat < std::max<double>(best_threat, max_acceptable_threat) && distance < best_distance)) {
					best_loc = i->second;
					best_threat = threat;
	...
	for(std::vector<map_location>::reverse_iterator ri = best_route.steps.rbegin(); ri != best_route.steps.rend();
		++ri) {
		...
		while(its.first != its.second) {
			if(its.first->second == best->get_location()) {
				if(!should_retreat(
					   its.first->first, best, fullmove_srcdst, fullmove_dstsrc, enemy_dstsrc, get_caution())) {
					...
					if(value > 0.0 && best_target->type != ai_target::type::mass) {
						...
						if(is_dangerous) {
							LOG_AI << "found reinforcement target... " << its.first->first
...
void move_to_targets_phase::access_points(
	const move_map& srcdst, const map_location& u, const map_location& dst, std::vector<map_location>& out)
{
	unit_map& units_ = resources::gameboard->units();
	const gamemap& map_ = resources::gameboard->map();
...
double move_to_targets_phase::compare_groups(const std::set<map_location>& our_group,
	const std::set<map_location>& their_group,
	const std::vector<map_location>& battlefield) const
{
...
void move_to_targets_phase::enemies_along_path(
	const std::vector<map_location>& route, const move_map& dstsrc, std::set<map_location>& res)
{
...
map_location move_to_targets_phase::form_group(
	const std::vector<map_location>& route, const move_map& dstsrc, std::set<map_location>& res)
{
	unit_map& units_ = resources::gameboard->units();
	if(route.empty()) {
	...
	for(i = route.begin(); i != route.end(); ++i) {
		...
		for(move_map::const_iterator j = itors.first; j != itors.second; ++j) {
			if(res.count(j->second) != 0) {
				...
				if(un == units_.end() || (un->can_recruit() && !is_keep_ignoring_leader(un->id()))
...
bool move_to_targets_phase::move_group(
	const map_location& dst, const std::vector<map_location>& route, const std::set<map_location>& units)
{
	unit_map& units_ = resources::gameboard->units();
	const gamemap& map_ = resources::gameboard->map();
	...
	for(std::set<map_location>::const_iterator i = units.begin(); i != units.end(); ++i) {
		...
		if(best_loc.valid()) {
			...
			for(std::size_t n = 0; n < adj.size(); ++n) {
				if(n != direction && ((n + 3) % 6) != direction && map_.on_board(adj[n]) && units_.count(adj[n]) == 0
					&& std::count(preferred_moves.begin(), preferred_moves.end(), adj[n]) == 0) {
...
double move_to_targets_phase::rate_group(
	const std::set<map_location>& group, const std::vector<map_location>& battlefield) const
{
	unit_map& units_ = resources::gameboard->units();
	const gamemap& map_ = resources::gameboard->map();
...
bool move_to_targets_phase::should_retreat(const map_location& loc,
