...
static void verify(const unit_map& units, const config& cfg)
{
	...
	if(nunits != units.size()) {
		errbuf << "SYNC VERIFICATION FAILED: number of units from data source differ: " << nunits
			   << " according to data source. " << units.size() << " locally\n";
		...
		for(const config& u : cfg.child_range("unit")) {
			...
			if(units.count(loc) == 0) {
				errbuf << "data source says there is a unit at " << loc << " but none found locally\n";
			}
		}

		for(unit_map::const_iterator j = units.begin(); j != units.end(); ++j) {
			if(locs.count(j->get_location()) == 0) {
				errbuf << "local unit at " << j->get_location() << " but none in data source\n";
			}
		}
	...
	for(const config& un : cfg.child_range("unit")) {
		...
		if(u == units.end()) {
			errbuf << "SYNC VERIFICATION FAILED: data source says there is a '" << un["type"] << "' (side "
				   << un["side"] << ") at " << loc << " but there is no local record of it\n";
			replay::process_error(errbuf.str());
			errbuf.clear();
		}
		...
		for(const std::string& field : fields) {
			if(u_cfg[field] != un[field]) {
				errbuf << "ERROR IN FIELD '" << field << "' for unit at " << loc << " data source: '" << un[field]
					   << "' local: '" << u_cfg[field] << "'\n";
				is_ok = false;
			}
		}
...
{
	if(!prefs::get().parse_should_show_lobby_join(cfg["id"], cfg["message"]))
		return;
	if(prefs::get().is_ignored(cfg["id"]))
		return;
	*i = chat_msg(cfg);
}
...
void replay::undo_cut(config& dst)
{
	...
	// Fix the [command]s after the undone action. This includes dependent commands for that user actions and async user
	// action.
...
REPLAY_ACTION_TYPE get_replay_action_type(const config& command)
{
	...
	if(key == "speak" || key == "label" || key == "surrender" || key == "clear_labels" || key == "rename"
		|| key == "countdown_update") {
...
/**
	@returns:
		if we expect a user choice and found something that prevents us from moving on we return REPLAY_FOUND_DEPENDENT
   (even if it is not a dependent command) else if we found an [end_turn] we return REPLAY_FOUND_END_TURN else if we
   found a player action and one_move=true we return REPLAY_FOUND_END_MOVE else (<=> we reached the end of the replay)
   we return REPLAY_RETURN_AT_END
*/
		...
		if(ch_itors.empty() || cfg->has_child("start")) {
			...
			if(resources::recorder->add_chat_message_location()) {
				...
				if(!resources::controller->is_skipping_replay() || is_whisper) {
					...
					game_display::get_singleton()->get_chat_manager().add_chat_message(as_time_t, speaker_name, side,
						message,
			...
			display::get_singleton()->labels().set_label(
				label.location(), label.text(), label.creator(), label.team_name(), label.color());
		} else if(auto clear_labels = cfg->optional_child("clear_labels")) {
			display::get_singleton()->labels().clear(
				std::string(clear_labels["team_name"]), clear_labels["force"].to_bool());
			...
			if(u.valid() && !u->unrenamable()) {
				...
				WRN_REPLAY << "attempt to rename unit at location: " << loc
						   << (u.valid() ? ", which is unrenamable" : ", where none exists (anymore)");
			}
		}
		...
		// if there is an end turn directive
			...
			if(tval <= 0 || tval > static_cast<int>(resources::gameboard->teams().size())) {
				...
				errbuf << "Illegal countdown update \n"
					   << "Received update for :" << tval << " Current user :" << side_num << "\n"
			...
			if(!is_unsynced) {
				...
				if((*cfg)["from_side"].to_int(0) != resources::controller->current_side()) {
					ERR_REPLAY << "received a synced [command] from side " << (*cfg)["from_side"].to_int(0)
							   << ". Expacted was a [command] from side " << resources::controller->current_side();
