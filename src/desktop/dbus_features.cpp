...
#ifndef HAVE_LIBDBUS
#error                                                                                                                 \
	"The HAVE_LIBDBUS symbol is not defined, you do not have lib dbus available, you should not be trying to compile dbus_features.cpp"
#endif
...
typedef boost::multi_index_container<wnotify,
	indexed_by<
		// hashed by ids
...
DBusHandlerResult filter_dbus_signal(DBusConnection*, DBusMessage* buf, void*)
{
	...
	uint32_t id;
	dbus_message_get_args(buf, nullptr, DBUS_TYPE_UINT32, &id, DBUS_TYPE_INVALID);
...
uint32_t send_dbus_notification(
	DBusConnection* connection, uint32_t replaces_id, const std::string& owner, const std::string& message)
{
	DBusMessage* buf
		= dbus_message_new_method_call(kde_style ? "org.kde.VisualNotifications" : "org.freedesktop.Notifications",
			kde_style ? "/VisualNotifications" : "/org/freedesktop/Notifications",
			kde_style ? "org.kde.VisualNotifications" : "org.freedesktop.Notifications", "Notify");
	const char* app_name = "Battle for Wesnoth";
	dbus_message_append_args(buf, DBUS_TYPE_STRING, &app_name, DBUS_TYPE_UINT32, &replaces_id, DBUS_TYPE_INVALID);
	if(kde_style) {
		const char* event_id = "";
		dbus_message_append_args(buf, DBUS_TYPE_STRING, &event_id, DBUS_TYPE_INVALID);
	}
	...
	dbus_message_append_args(buf, DBUS_TYPE_STRING, &app_icon, DBUS_TYPE_STRING, &summary, DBUS_TYPE_STRING, &body,
		DBUS_TYPE_ARRAY, DBUS_TYPE_STRING, &actions, 0, DBUS_TYPE_INVALID);
	...
	dbus_message_append_args(buf, DBUS_TYPE_INT32, &expire_timeout, DBUS_TYPE_INVALID);
	DBusError err;
	dbus_error_init(&err);
	...
	uint32_t id;
	dbus_message_get_args(ret, nullptr, DBUS_TYPE_UINT32, &id, DBUS_TYPE_INVALID);
...
T get_power_source_property(const std::string& name, T fallback)
{
	...
	std::unique_ptr<DBusMessage, std::function<void(DBusMessage*)>> msg(
	...
	dbus_message_append_args(
		msg.get(), DBUS_TYPE_STRING, &interface_name, DBUS_TYPE_STRING, &property_name, DBUS_TYPE_INVALID);

	DBusError err;
	dbus_error_init(&err);

	std::unique_ptr<DBusMessage, std::function<void(DBusMessage*)>> ret(
