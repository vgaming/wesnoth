...
button::button(const std::string& label,
	...
	, label_text_(label)
	, image_(nullptr)
	, pressedImage_(nullptr)
	, activeImage_(nullptr)
	, pressedActiveImage_(nullptr)
	, disabledImage_(nullptr)
	, pressedDisabledImage_(nullptr)
	, overlayImage_(nullptr)
	, overlayPressedImage_(nullptr)
	, overlayActiveImage_(nullptr)
	, state_(NORMAL)
	, pressed_(false)
	, spacing_(spacing)
	, base_height_(0)
	, base_width_(0)
	, button_image_name_()
	, button_overlay_image_name_(std::move(overlay_image))
	, button_image_path_suffix_()
	, font_size_(font_size <= 0 ? (type != TYPE_CHECK && type != TYPE_RADIO ? default_font_size : font::SIZE_SMALL)
								: font_size)
...
void button::load_images()
{
	...
	image_ = image::get_texture(button_image_name_ + ".png" + button_image_path_suffix_);
	pressedImage_ = image::get_texture(button_image_name_ + "-pressed.png" + button_image_path_suffix_);
	activeImage_ = image::get_texture(button_image_name_ + "-active.png" + button_image_path_suffix_);
	if(filesystem::file_exists(game_config::path + "/images/" + button_image_name_ + "-disabled.png")) {
		disabledImage_ = image::get_texture(button_image_name_ + "-disabled.png" + button_image_path_suffix_);
	} else {
	...
	if(!button_overlay_image_name_.empty()) {
		...
		overlayImage_
			= image::get_texture(button_overlay_image_name_ + size_postfix + ".png" + button_image_path_suffix_);
		overlayPressedImage_ = image::get_texture(
			button_overlay_image_name_ + size_postfix + "-pressed.png" + button_image_path_suffix_);

		if(filesystem::file_exists(
			   game_config::path + "/images/" + button_overlay_image_name_ + size_postfix + "-active.png"))
			overlayActiveImage_ = image::get_texture(
				button_overlay_image_name_ + size_postfix + "-active.png" + button_image_path_suffix_);

		if(filesystem::file_exists(
			   game_config::path + "/images/" + button_overlay_image_name_ + size_postfix + "-disabled.png"))
			overlayDisabledImage_ = image::get_texture(
				button_overlay_image_name_ + size_postfix + "-disabled.png" + button_image_path_suffix_);
		if(!overlayDisabledImage_)
			overlayDisabledImage_ = image::get_texture(
				button_overlay_image_name_ + size_postfix + ".png~GS()" + button_image_path_suffix_);

		if(filesystem::file_exists(
			   game_config::path + "/images/" + button_overlay_image_name_ + size_postfix + "-disabled-pressed.png"))
			overlayPressedDisabledImage_ = image::get_texture(
				button_overlay_image_name_ + size_postfix + "-disabled-pressed.png" + button_image_path_suffix_);
		if(!overlayPressedDisabledImage_)
			overlayPressedDisabledImage_ = image::get_texture(
				button_overlay_image_name_ + size_postfix + "-pressed.png~GS()" + button_image_path_suffix_);
	} else {
		overlayImage_.reset();
	}

	if(!disabledImage_) {
		disabledImage_ = image::get_texture(button_image_name_ + ".png~GS()" + button_image_path_suffix_);
	}
	...
	if(type_ == TYPE_CHECK || type_ == TYPE_RADIO) {
		touchedImage_ = image::get_texture(button_image_name_ + "-touched.png" + button_image_path_suffix_);
		if(!touchedImage_) {
			touchedImage_ = pressedImage_;
		}

		pressedActiveImage_
			= image::get_texture(button_image_name_ + "-active-pressed.png" + button_image_path_suffix_);
		if(!pressedActiveImage_) {
			pressedActiveImage_ = pressedImage_;
		}

		if(filesystem::file_exists(
			   game_config::path + "/images/" + button_image_name_ + size_postfix + "-disabled-pressed.png")) {
			pressedDisabledImage_
				= image::get_texture(button_image_name_ + "-disabled-pressed.png" + button_image_path_suffix_);
		}
		if(!pressedDisabledImage_) {
			pressedDisabledImage_
				= image::get_texture(button_image_name_ + "-pressed.png~GS()" + button_image_path_suffix_);
		}
	}
