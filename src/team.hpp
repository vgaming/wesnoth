...
private:
	struct team_info
	{
		...
		side_proxy_controller::type
			proxy_controller; // when controller == HUMAN, the proxy controller determines what input method is actually
							  // used. proxy controller is an interface property, not gamestate. it is not synced, not
							  // known to server. also not saved in save game file
		...
		bool carryover_add;
		// TODO: maybe make this integer percentage? I like the float version more but this might cause OOS error
		// because of floating point rounding differences on different hardware.
...
public:
	 ...
	 * Pointer fire_event should be the game_data for the game if it is desired to fire an event -- a "capture" event
	 * with owner_side variable scoped in will be fired. For no event, pass it nullptr. Default is the
	 * resources::gamedata pointer.
	 */
	...
	{
		return info_.scroll_to_leader;
	}
	void set_scroll_to_leader(bool value)
	{
		info_.scroll_to_leader = value;
	}
	...
	{
		return is_human() && is_local();
	}
	bool is_local_ai() const
	{
		return is_ai() && is_local();
	}
	bool is_network_human() const
	{
		return is_human() && is_network();
	}
	bool is_network_ai() const
	{
		return is_ai() && is_network();
	}

	void set_local(bool local)
	{
		info_.is_local = local;
	}
	void make_human()
	{
		info_.controller = side_controller::type::human;
	}
	void make_ai()
	{
		info_.controller = side_controller::type::ai;
	}
	...
	void make_droid()
	{
		info_.proxy_controller = side_proxy_controller::type::ai;
	}
	void make_idle()
	{
		info_.proxy_controller = side_proxy_controller::type::idle;
	}
	void make_proxy_human()
	{
		info_.proxy_controller = side_proxy_controller::type::human;
	}
	void clear_proxy()
	{
		make_proxy_human();
	...
	{
		info_.proxy_controller = proxy;
	}

	void toggle_droid()
	{
		info_.proxy_controller = (info_.proxy_controller == side_proxy_controller::type::ai)
			? side_proxy_controller::type::human
			: side_proxy_controller::type::ai;
	}
	void toggle_idle()
	{
		info_.proxy_controller = (info_.proxy_controller == side_proxy_controller::type::idle)
			? side_proxy_controller::type::human
			: side_proxy_controller::type::idle;
	}
	...
	{
		return info_.side_name.empty() ? t_string(info_.current_player) : info_.side_name;
	}
	void set_side_name(const t_string& new_name)
	{
		info_.side_name = new_name == info_.current_player ? "" : new_name;
	}
	...
	{
		return uses_shroud() || uses_fog();
	}
	bool clear_shroud(const map_location& loc)
	{
		return shroud_.clear(loc.wml_x(), loc.wml_y());
	}
	void place_shroud(const map_location& loc)
	{
		shroud_.place(loc.wml_x(), loc.wml_y());
	}
	bool clear_fog(const map_location& loc)
	{
		return fog_.clear(loc.wml_x(), loc.wml_y());
	}
	void reshroud()
	{
		shroud_.reset();
	...
	{
		shroud_.set_enabled(shroud);
	}
	void set_fog(bool fog)
	{
		fog_.set_enabled(fog);
	}
	std::string shroud_data() const
	{
		return shroud_.write();
	}
	...
	{
		fog_clearer_.insert(hexes.begin(), hexes.end());
	}
	...
	{
		info_.defeat_cond = defeat_condition::get_enum(value).value_or(info_.defeat_cond);
	}
	void have_leader(bool value = true)
	{
		info_.no_leader = !value;
	}
	bool hidden() const
	{
		return info_.hidden;
	...
	{
		info_.carryover_percentage = value;
	}
	int carryover_percentage() const
	{
		return info_.carryover_percentage;
	}
	void set_carryover_add(bool value)
	{
		info_.carryover_add = value;
	}
	bool carryover_add() const
	{
		return info_.carryover_add;
	}
	void set_carryover_bonus(double value)
	{
		info_.carryover_bonus = value;
	}
	double carryover_bonus() const
	{
		return info_.carryover_bonus;
	}
	void set_carryover_gold(int value)
	{
		info_.carryover_gold = value;
	}
	int carryover_gold() const
	{
		return info_.carryover_gold;
	}
	config& variables()
	{
		return info_.variables;
	}
	...
	{
		return info_.no_turn_confirmation;
	}
	void set_no_turn_confirmation(bool value)
	{
		info_.no_turn_confirmation = value;
	}
