...
BOOST_AUTO_TEST_CASE(test_fs_enum)
{
	...
	const std::vector<std::string> expected_filenames{
		"_initial.cfg", "A1.cfg", "A2.cfg", "A3.cfg", "B1.cfg", "B2.cfg", "B3.cfg", "_final.cfg"};
	const std::vector<std::string> expected_dirnames{"D1", "D2", "D3"};
...
BOOST_AUTO_TEST_CASE(test_fs_binary_path)
{
	...
	BOOST_CHECK_EQUAL(
		get_binary_file_location("music", "silence.ogg").value(), gamedata + "/data/core/music/silence.ogg");

	BOOST_CHECK_EQUAL(
		get_binary_file_location("sounds", "explosion.ogg").value(), gamedata + "/data/core/sounds/explosion.ogg");

	BOOST_CHECK_EQUAL(
		get_independent_binary_file_path("images", "wesnoth-icon.png").value(), "data/core/images/wesnoth-icon.png");
