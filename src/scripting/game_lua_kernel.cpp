...
void game_lua_kernel::lua_chat(const std::string& caption, const std::string& msg)
{
	if(game_display_) {
		game_display_->get_chat_manager().add_chat_message(
			std::time(nullptr), caption, 0, msg, events::chat_handler::MESSAGE_PUBLIC, false);
	}
}
...
std::vector<int> game_lua_kernel::get_sides_vector(const vconfig& cfg)
{
	...
	if(!ssf.null()) {
		if(!sides.empty()) {
			WRN_LUA << "ignoring duplicate side filter information (inline side=)";
		}
...
int game_lua_kernel::intf_gamestate_inspector(lua_State* L)
{
	if(game_display_) {
		...
		if(luaW_tovconfig(L, 1, cfg)) {
			name = cfg["name"].str();
			deprecated_message("gui.show_inspector(cfg)", DEP_LEVEL::INDEFINITE, {1, 19, 0},
				"Instead of {name = 'title' }, pass just 'title'.");
		} else {
...
int game_lua_kernel::intf_set_variable(lua_State* L)
{
	...
	if(m.empty())
		return luaL_argerror(L, 1, "empty variable name");
...
int game_lua_kernel::intf_highlight_hex(lua_State* L)
{
	...
	if(!map().on_board(loc))
		return luaL_argerror(L, 1, "not on board");
...
void game_lua_kernel::luaW_push_schedule(lua_State* L, int area_index)
{
	...
	if(luaL_newmetatable(L, "schedule")) {
		static luaL_Reg const schedule_meta[]{{"__index", &dispatch<&game_lua_kernel::impl_schedule_get>},
			{"__newindex", &dispatch<&game_lua_kernel::impl_schedule_set>},
			{"__dir", &dispatch<&game_lua_kernel::impl_schedule_dir>},
			{"__len", &dispatch<&game_lua_kernel::impl_schedule_len>}, {nullptr, nullptr}};
...
SCHEDULE_SETTER("time_of_day", std::string)
{
	...
	auto iter = std::find_if(times.begin(), times.end(), [&value](const time_of_day& tod) { return tod.id == value; });
	if(iter == times.end()) {
...
int game_lua_kernel::impl_get_terrain_info(lua_State* L)
{
	...
	if(t == t_translation::NONE_TERRAIN || !board().map().tdata()->is_known(t))
		return 0;
...
struct game_config_glk_tag
{
	...
	game_config_glk_tag(lua_kernel_base& k)
		: ref(dynamic_cast<game_lua_kernel&>(k))
	{
	}
	auto& pc() const
	{
		return ref.play_controller_;
	}
	auto& gamedata() const
	{
		return ref.gamedata();
...
{
	end_level_committer(end_level_data& data, play_controller& pc)
		: data_(data)
		, pc_(pc)
	{
	}
...
static int impl_mp_settings_get(lua_State* L)
{
	...
	if(lua_type(L, 2) == LUA_TNUMBER) {
		...
		return_string_attrib_deprecated("mp_era", "wesnoth.scenario.mp_settings", INDEFINITE, "1.17",
			"Use wesnoth.scenario.era.id instead", game.era_id);
		return_string_attrib_deprecated("active_mods", "wesnoth.scenario.mp_settings", INDEFINITE, "1.17",
			"Use wesnoth.scenario.modifications instead (returns an array of modification tables)",
			utils::join(game.active_mods));
		// Expose the raw config; this is a way to ensure any new stuff can be accessed even if someone forgot to add it
		// here.
...
struct scenario_tag
{
	...
	auto& cls() const
	{
		return ref.play_controller_.get_classification();
	}
...
SCENARIO_GETTER("end_level_data", lua_index_raw)
{
	...
	if(luaL_newmetatable(L, "end level data")) {
		static luaL_Reg const callbacks[]{{"__index", &impl_end_level_data_get}, {"__newindex", k.end_level_set()},
			{"__gc", &impl_end_level_data_collect}, {nullptr, nullptr}};
...
struct current_tag
{
	...
	auto& ev() const
	{
		return ref.get_event_info();
	}
	void push_schedule(lua_State* L) const
	{
		ref.luaW_push_schedule(L, -1);
	}
};
...
CURRENT_GETTER("event_context", config)
{
	...
	if(ev.loc1.valid()) {
		...
		// The position of the unit involved in this event, currently the only case where this is different from x1/y1
		// are enter/exit_hex events
...
int game_lua_kernel::intf_find_path(lua_State* L)
{
	...
	if(lua_istable(L, arg)) {
		...
		if(!lua_isnil(L, -1)) {
			...
			if(i >= 1 && i <= static_cast<int>(teams().size()))
				...
				if(u)
					see_all = true;
				deprecated_message("wesnoth.paths.find_path with viewing_side=0 (or an invalid side)",
		...
		deprecated_message("wesnoth.paths.find_path with cost_function as last argument", DEP_LEVEL::FOR_REMOVAL,
			{1, 17, 0}, "Use calculate=cost_function inside the path options table instead.");
	...
	if(!ignore_teleport) {
		if(viewing_side == 0) {
			lua_warning(L,
				"wesnoth.paths.find_path: ignore_teleport=false requires a valid viewing_side; continuing with "
				"ignore_teleport=true",
				false);
			ignore_teleport = true;
		} else {
			teleport_locations
				= pathfind::get_teleport_locations(*u, board().get_team(viewing_side), see_all, ignore_units);
		}
	}
	...
	if(!calc) {
		...
		calc.reset(new pathfind::shortest_path_calculator(
			*u, board().get_team(viewing_side), teams(), map, ignore_units, false, see_all));
	}

	pathfind::plain_route res
		= pathfind::a_star_search(src, dst, stop_at, *calc, map.w(), map.h(), &teleport_locations);
...
int game_lua_kernel::intf_find_reach(lua_State* L)
{
	...
	if(lua_istable(L, arg)) {
		...
		if(!lua_isnil(L, -1)) {
			...
			if(i >= 1 && i <= static_cast<int>(teams().size()))
				...
				if(u)
					see_all = true;
				deprecated_message("wesnoth.find_reach with viewing_side=0 (or an invalid side)",
					DEP_LEVEL::FOR_REMOVAL, {1, 17, 0},
					"To consider fogged and hidden units, use ignore_visibility=true instead.");
			}
		}
	...
	pathfind::paths res(*u, ignore_units, !ignore_teleport, viewing_team, additional_turns, see_all, ignore_units);
...
error:
	return luaL_argerror(L, arg,
		"unit type table malformed - each entry should be either array of 4 elements or table with keys x, y, side, "
		"type");
}
...
int game_lua_kernel::intf_find_cost_map(lua_State* L)
{
	...
	// build cost_map
	const team& viewing_team = viewing_side ? board().get_team(viewing_side) : board().teams()[0];

	pathfind::full_cost_map cost_map(ignore_units, !ignore_teleport, viewing_team, see_all, ignore_units);
...
int game_lua_kernel::intf_set_floating_label(lua_State* L, bool spawn)
{
			...
			if(!found_number) {
				...
				try {
					if(!value.empty() && value.back() == '%') {
						...
						throw std::invalid_argument(value.data());
			...
			if(lua_isstring(L, -1)) {
				...
				if(vec.size() != 3) {
					...
					if(luaW_tableget(L, idx, "r") && luaW_tableget(L, idx, "g") && luaW_tableget(L, idx, "b")) {
						...
						return luaL_error(L,
							"floating label text color should be a hex string, an array of 3 integers, or a table with "
							"r,g,b keys");
					}
				} else {
					color.r = vec[0];
			...
			if(lua_isstring(L, -1)) {
				...
				if(vec.size() != 3) {
					...
					if(luaW_tableget(L, idx, "r") && luaW_tableget(L, idx, "g") && luaW_tableget(L, idx, "b")) {
						...
						return luaL_error(L,
							"floating label background color should be a hex string, an array of 3 integers, or a "
							"table with r,g,b keys");
					}
				} else {
					bgcolor.r = vec[0];
					bgcolor.g = vec[1];
	...
	*handle = font::add_floating_label(flabel);
	...
	if(luaL_newmetatable(L, labelKey)) {
		...
		static const luaL_Reg methods[] = {{"remove", &dispatch<&game_lua_kernel::intf_remove_floating_label>},
			{"move", &dispatch<&game_lua_kernel::intf_move_floating_label>},
			{"replace", &dispatch2<&game_lua_kernel::intf_set_floating_label, false>},
			{"__index", &impl_floating_label_getmethod}, {nullptr, nullptr}};
...
int game_lua_kernel::intf_put_unit(lua_State* L)
{
	...
	// If the final boolean argument is omitted, the actual final argument (the unit or location) will always yield
	// true.
...
int game_lua_kernel::intf_put_recall_unit(lua_State* L)
{
	...
	if(static_cast<unsigned>(side) > teams().size())
		side = 0;
...
static int intf_unit_movement_cost(lua_State* L)
{
	...
	if(luaW_tolocation(L, 2, loc)) {
		...
		return luaW_type_error(L, 2, "location or terrain string");
...
static int intf_transform_unit(lua_State* L)
{
	...
	if(!utp)
		return luaL_argerror(L, 2, "unknown unit type");
	if(lua_isstring(L, 3)) {
		const std::string& m2 = lua_tostring(L, 3);
		if(!utp->has_variation(m2))
			return luaL_argerror(L, 2, "unknown unit variation");
...
int game_lua_kernel::intf_simulate_combat(lua_State* L)
{
	...
	battle_context context(units(), att->get_location(), def->get_location(), att_w, def_w, 0.0, nullptr, att, def);
...
{
	if(play_controller_.is_skipping_replay())
		return 0;
	char const* m = luaL_checkstring(L, 1);
 ...
 * - Ret 1: int - the achievement's current progress after adding amount or -1 if not a progressable achievement
 * (including if it's already achieved)
			...
			for(achievement& achieve : group.achievements_) {
				if(achieve.id_ == id) {
					...
					if(achieve.max_progress_ == 0 || achieve.sub_achievements_.size() > 0) {
						ERR_LUA << "Attempted to progress achievement " << id << " for achievement group "
								<< content_for << ", is not a progressible achievement.";
						lua_pushinteger(L, -1);
						lua_pushinteger(L, -1);
						return 2;
					}
					...
					if(!achieve.achieved_) {
						int progress = 0;
						if(!play_controller_.is_replay()) {
							progress = prefs::get().progress_achievement(
								content_for, id, limit, achieve.max_progress_, amount);
						}
		...
		if(group.content_for_ == content_for) {
			for(achievement& achieve : group.achievements_) {
				if(achieve.id_ == id) {
					...
					// sub-achievement not found - existing achievement group and achievement but non-existing
					// sub-achievement id
					lua_push(L,
						"Sub-achievement " + std::string(id) + " not found for achievement" + id
...
int game_lua_kernel::intf_select_unit(lua_State* L)
{
	...
	play_controller_.get_mouse_handler_base().select_hex(loc, false, highlight, fire_event);
	return 0;
}
...
static int intf_synchronize_choices(lua_State* L)
{
	...
	lua_push(L,
		mp_sync::get_user_choice_multiple_sides(tagname, lua_synchronize(L, desc, human_func, null_func),
			std::set<int>(sides_for.begin(), sides_for.end())));
	return 1;
}

/**
 * Calls a function in an unsynced context (this specially means that all random calls used by that function will be
 * unsynced). This is usually used together with an unsynced if like 'if controller != network'
...
static int intf_modify_ai(lua_State* L, const char* action)
{
	...
	config cfg{"action", action, "path", path};
...
static int intf_add_modification(lua_State* L)
{
	...
	if(sm == "advance") { // Maintain backwards compatibility
		sm = "advancement";
		deprecated_message(
			"\"advance\" modification type", DEP_LEVEL::PREEMPTIVE, {1, 15, 0}, "Use \"advancement\" instead.");
	}
...
static int intf_remove_modifications(lua_State* L)
{
	...
	// TODO
	if(filter.attribute_count() == 1 && filter.all_children_count() == 0
		&& filter.attribute_range().front().first == "duration") {
...
int game_lua_kernel::intf_add_tile_overlay(lua_State* L)
{
	...
	if(game_display_) {
		game_display_->add_overlay(loc,
			overlay(cfg["image"], cfg["halo"], team_name,
				cfg["name"], // Name is treated as the ID
				cfg["visible_in_fog"].to_bool(true), cfg["submerge"].to_double(0), cfg["z_order"].to_double(0)));
	}
	return 0;
}
 ...
 * id: Event ID
 * menu_item: True if this is a menu item (an ID is required); this means removing the menu item will automatically
 * remove this event. Default false. first_time_only: Whether this event should fire again after the first time; default
 * true. priority: Number that determines execution order. Events execute in order of decreasing priority, and
 * secondarily in order of addition. filter: Event filters as a config with filter tags, a table of the form
 * {filter_type = filter_contents}, or a function filter_args: Arbitrary data that will be passed to the filter, if it
 * is a function. Ignored if the filter is specified as WML or a table. content: The content of the event. This is a WML
 * table passed verbatim into the event when it fires. If no function is specified, it will be interpreted as ActionWML.
...
int game_lua_kernel::intf_add_event(lua_State* L)
{
	...
	bool repeat = !luaW_table_get_def(L, 1, "first_time_only", true),
		 is_menu_item = luaW_table_get_def(L, 1, "menu_item", false);
			...
			if(!luaW_toconfig(L, filterIdx, filters)) {
				if(lua_isfunction(L, filterIdx)) {
					...
					new_handler->add_filter(std::make_unique<lua_event_filter>(
						*this, fcnIdx, luaW_table_get_def(L, 1, "filter_args", config())));
					has_lua_filter = true;
				} else {
#define READ_ONE_FILTER(key, tag)                                                                                      \
	do {                                                                                                               \
		if(luaW_tableget(L, filterIdx, key)) {                                                                         \
			if(lua_isstring(L, -1)) {                                                                                  \
				filters.add_child("insert_tag", config{"name", tag, "variable", luaL_checkstring(L, -1)});             \
			} else {                                                                                                   \
...
{
	if(gamedata().phase() == game_data::PRELOAD || gamedata().phase() == game_data::PRESTART
		|| gamedata().phase() == game_data::INITIAL) {
...
static int intf_debug_ai(lua_State* L)
{
	...
	// ai::component* e = ai::manager::get_singleton().get_active_ai_holder_for_side_dbg(side).get_component(c,
	// "engine[lua]"); ai::engine_lua* lua_engine = dynamic_cast<ai::engine_lua *>(e);
...
int game_lua_kernel::intf_add_time_area(lua_State* L)
{
	...
	if(lua_gettop(L) == 1) {
		vconfig cfg = luaW_checkvconfig(L, 1);
		deprecated_message("Single-argument wesnoth.map.place_area is deprecated. Instead, pass ID, filter, and "
						   "schedule as three separate arguments.",
			DEP_LEVEL::INDEFINITE, {1, 17, 0});
		id = cfg["id"].str();
...
int game_lua_kernel::intf_replace_schedule(lua_State* L)
{
	...
	if(luaL_testudata(L, 1, "schedule")) {
		...
		if(area >= 0)
			tod_man().replace_schedule(tod_man().times(area));
	} else {
		vconfig cfg = luaW_checkvconfig(L, 1);
...
{
	reports::context temp_context = reports::context(board(), *game_display_, tod_man(),
...
{
	char const* m = luaL_checkstring(L, 2);
	lua_cpp::push_closure(
		L, std::bind(&game_lua_kernel::impl_theme_item, this, std::placeholders::_1, std::string(m)), 0);
	lua_pushvalue(L, 2);
	lua_pushvalue(L, -2);
	lua_rawset(L, 1);
...
#define CALLBACK_GETTER(name, type)                                                                                    \
	LATTR_GETTER(name, lua_index_raw, callbacks_tag, )                                                                 \
	{                                                                                                                  \
		lua_pushcfunction(L, &impl_null_callback<type>);                                                               \
		return lua_index_raw(L);                                                                                       \
	}
	...
	, map_locked_(0)
{
	...
	static luaL_Reg const callbacks[]{{"add_known_unit", &intf_add_known_unit}, {"get_era", &intf_get_era},
		{"get_resource", &intf_get_resource}, {"modify_ai", &intf_modify_ai_old},
		{"cancel_action", &dispatch<&game_lua_kernel::intf_cancel_action>},
		{"log_replay", &dispatch<&game_lua_kernel::intf_log_replay>}, {"log", &dispatch<&game_lua_kernel::intf_log>},
		{"redraw", &dispatch<&game_lua_kernel::intf_redraw>},
		{"simulate_combat", &dispatch<&game_lua_kernel::intf_simulate_combat>}, {nullptr, nullptr}};
	...
	if(play_controller_.get_classification().is_test()) {
		...
		static luaL_Reg const test_callbacks[]{
			{"fire_wml_menu_item", &dispatch<&game_lua_kernel::intf_fire_wml_menu_item>}, {nullptr, nullptr}};
	...
	lua_getglobal(L, "wml");
	static luaL_Reg const wml_callbacks[]{{"tovconfig", &lua_common::intf_tovconfig},
		...
		{"get_all_vars", &dispatch<&game_lua_kernel::intf_get_all_vars>}, {nullptr, nullptr}};
	luaL_setfuncs(L, wml_callbacks, 0);
	lua_pop(L, 1);

	// Add functions to the map module
	luaW_getglobal(L, "wesnoth", "map");
	static luaL_Reg const map_callbacks[]{// Map methods
		{"terrain_mask", &intf_terrain_mask}, {"on_board", &intf_on_board}, {"on_border", &intf_on_border},
		...
		{"matches", &dispatch<&game_lua_kernel::intf_match_location>}, {"replace_if_failed", intf_replace_if_failed},
		{nullptr, nullptr}};
	...
	static luaL_Reg const unit_callbacks[]{{"advance", &intf_advance_unit}, {"clone", &intf_copy_unit},
		...
		{"to_recall", &dispatch<&game_lua_kernel::intf_put_recall_unit>}, {"transform", &intf_transform_unit},
		{"teleport", &dispatch<&game_lua_kernel::intf_teleport>},

		{"ability", &dispatch<&game_lua_kernel::intf_unit_ability>}, {"defense_on", &intf_unit_defense},
		{"jamming_on", &intf_unit_jamming_cost}, {"movement_on", &intf_unit_movement_cost},
		{"resistance_against", intf_unit_resistance}, {"vision_on", &intf_unit_vision_cost},

		{"add_modification", &intf_add_modification}, {"remove_modifications", &intf_remove_modifications},
		// Static functions
		{"create", &intf_create_unit}, {"find_on_map", &dispatch<&game_lua_kernel::intf_get_units>},
		...
		{"create_animator", &dispatch<&game_lua_kernel::intf_create_animator>}, {"create_weapon", intf_create_attack},
	...
	static luaL_Reg const side_callbacks[]{{"is_enemy", &dispatch<&game_lua_kernel::intf_is_enemy>},
		{"matches", &dispatch<&game_lua_kernel::intf_match_side>},
		{"set_id", &dispatch<&game_lua_kernel::intf_set_side_id>}, {"append_ai", &intf_append_ai},
		{"debug_ai", &intf_debug_ai}, {"switch_ai", &intf_switch_ai},
		// Static functions
		{"find", &dispatch<&game_lua_kernel::intf_get_sides>}, {"get", &dispatch<&game_lua_kernel::intf_get_side>},
		...
		{"is_fogged", &dispatch2<&game_lua_kernel::intf_get_fog_or_shroud, true>}, {nullptr, nullptr}};
	std::vector<lua_cpp::Reg> const cpp_side_callbacks{
		...
		{"change_ai_component", std::bind(intf_modify_ai, std::placeholders::_1, "change")}, {nullptr, nullptr}};
	...
	static luaL_Reg const intf_callbacks[]{{"add_hex_overlay", &dispatch<&game_lua_kernel::intf_add_tile_overlay>},
		...
		{"end_turn", &dispatch<&game_lua_kernel::intf_end_turn>}, {"get_viewing_side", &intf_get_viewing_side},
		...
		{"handle_user_interact", &intf_handle_user_interact}, {nullptr, nullptr}};
	lua_getglobal(L, "wesnoth");
	lua_newtable(L);
	...
	static luaL_Reg const achievement_callbacks[]{{"set", &dispatch<&game_lua_kernel::intf_set_achievement>},
		...
		{"set_sub_achievement", &dispatch<&game_lua_kernel::intf_set_sub_achievement>}, {nullptr, nullptr}};
	lua_getglobal(L, "wesnoth");
	lua_newtable(L);
	...
	static luaL_Reg const audio_callbacks[]{{"play", &dispatch<&game_lua_kernel::intf_play_sound>}, {nullptr, nullptr}};
	lua_getglobal(L, "wesnoth");
	lua_newtable(L);
	...
	static luaL_Reg const path_callbacks[]{{"find_cost_map", &dispatch<&game_lua_kernel::intf_find_cost_map>},
		...
		{"find_vision_range", &dispatch<&game_lua_kernel::intf_find_vision_range>}, {nullptr, nullptr}};
	lua_getglobal(L, "wesnoth");
	lua_newtable(L);
	...
	static luaL_Reg const sync_callbacks[]{{"invoke_command", &intf_invoke_synced_command},
		{"run_unsynced", &intf_do_unsynced}, {"evaluate_single", &intf_synchronize_choice},
		{"evaluate_multiple", &intf_synchronize_choices}, {nullptr, nullptr}};
	lua_getglobal(L, "wesnoth");
	lua_newtable(L);
	...
	static luaL_Reg const schedule_callbacks[]{
		...
		{"replace", &dispatch<&game_lua_kernel::intf_replace_schedule>}, {nullptr, nullptr}};
	lua_getglobal(L, "wesnoth");
	lua_newtable(L);
	...
	static luaL_Reg const event_callbacks[]{{"add", &dispatch<&game_lua_kernel::intf_add_event>},
		...
		{"set_undoable", &dispatch<&game_lua_kernel::intf_allow_undo>}, {nullptr, nullptr}};
	lua_getglobal(L, "wesnoth");
	lua_newtable(L);
...
static bool is_handled_file_tag(std::string_view s)
{
	...
	static constexpr std::array handled_file_tags{"color_palette"sv, "color_range"sv, "display"sv, "end_level_data"sv,
		"era"sv, "event"sv, "generator"sv, "label"sv, "lua"sv, "map"sv, "menu_item"sv, "modification"sv,
		"modify_unit_type"sv, "music"sv, "options"sv, "side"sv, "sound_source"sv, "story"sv, "terrain_graphics"sv,
		"time"sv, "time_area"sv, "tunnel"sv, "undo_stack"sv, "variables"sv};
...
void game_lua_kernel::load_game(const config& level)
{
	...
	for(const auto [child_key, child_cfg] : level.all_children_view()) {
		if(is_handled_file_tag(child_key))
			continue;
		lua_createtable(L, 2, 0);
...
int game_lua_kernel::cfun_wml_action(lua_State* L)
{
	game_events::wml_action::handler h
		= reinterpret_cast<game_events::wml_action::handler>(lua_touserdata(L, lua_upvalueindex(1)));
...
{
	wml_conditional_handler h = reinterpret_cast<wml_conditional_handler>(lua_touserdata(L, lua_upvalueindex(1)));
 ...
 */
bool game_lua_kernel::run_wml_action(const std::string& cmd, const vconfig& cfg, const game_events::queued_event& ev)
{
	lua_State* L = mState;
...
bool game_lua_kernel::mouse_button_callback(
	const map_location& loc, const std::string& button, const std::string& event)
{
	lua_State* L = mState;
