...
static void simplemerge(
	t_translation::terrain_code old_t, t_translation::terrain_code& new_t, const terrain_type_data::merge_mode mode)
{
	switch(mode) {
...
void mapgen_gamemap::set_terrain(
	const map_location& loc, const terrain_code& terrain, const terrain_type_data::merge_mode mode, bool)
{
...
static void impl_merge_terrain(lua_State* L, gamemap_base& map, map_location loc)
{
	...
	if(auto gm = dynamic_cast<gamemap*>(&map)) {
		...
		map.set_terrain(loc, ter, mode, replace_if_failed);
}
...
static std::vector<gamemap::overlay_rule> read_rules_vector(lua_State* L, int index)
{
	...
	for(int i = 1, i_end = lua_rawlen(L, index); i <= i_end; ++i) {
		...
		if(luaW_tableget(L, -1, "layer")) {
			...
			rule.mode_ = str == "base" ? terrain_type_data::BASE
									   : (str == "overlay" ? terrain_type_data::OVERLAY : terrain_type_data::BOTH);
			lua_pop(L, 1);
		}
