...
static int intf_default_generate_height_map(lua_State* L)
{
	...
	if(!cfg.has_attribute("location_set")) {
		deprecated_message("generate_height_map(..., {location_set=false})", DEP_LEVEL::PREEMPTIVE, "1.17",
			"The default value of this option will be changed to true in 1.17.");
	}
	...
	default_map_generator_job job(seed);
	default_map_generator_job::height_map res
		= job.generate_height_map(width, height, iterations, hill_size, island_size, center_x, center_y);
...
static int intf_find_path(lua_State* L)
{
	...
	if(lua_isfunction(L, arg)) {
		const char* msg
			= lua_pushfstring(L, "%s expected, got %s", lua_typename(L, LUA_TFUNCTION), luaL_typename(L, 3));
	...
	, vars_(vars)
{
	...
	static luaL_Reg const map_callbacks[]{// Map methods
		{"find", &intf_mg_get_locations}, {"find_in_radius", &intf_mg_get_tiles_radius},
		// Static functions
		{"filter", &intf_terrainfilter_create}, {"create", &intf_terrainmap_create},
		{"generate_height_map", &intf_default_generate_height_map}, {"generate", &intf_default_generate},
		{nullptr, nullptr}};
	...
	static luaL_Reg const path_callbacks[]{{"find_path", &intf_find_path}, {nullptr, nullptr}};
	...
	static luaL_Reg const wml_callbacks[]{{"tovconfig", &lua_common::intf_tovconfig},
		...
		{"get_all_vars", &dispatch<&mapgen_lua_kernel::intf_get_all_vars>}, {nullptr, nullptr}};
...
{
	load_string(
		prog, "", std::bind(&lua_kernel_base::throw_exception, this, std::placeholders::_1, std::placeholders::_2));
	luaW_pushconfig(mState, generator);
	protected_call(
		1, 1, std::bind(&lua_kernel_base::throw_exception, this, std::placeholders::_1, std::placeholders::_2));
}
...
std::string mapgen_lua_kernel::create_map(
	const char* prog, const config& generator, utils::optional<uint32_t> seed) // throws game::lua_error
{
	random_seed_ = seed;
...
config mapgen_lua_kernel::create_scenario(
	const char* prog, const config& generator, utils::optional<uint32_t> seed) // throws game::lua_error
{
	random_seed_ = seed;
	default_rng_ = std::mt19937(get_random_seed());
