...
recruit::recruit(std::size_t team_index, bool hidden, const std::string& unit_name, const map_location& recruit_hex)
	...
	, // auto-ptr ownership transfer
	fake_unit_(temp_unit_->clone())
	, // temp_unit_ *copied* into new fake unit
	cost_(0)
{
	this->init();
}
...
void recruit::apply_temp_modifier(unit_map& unit_map)
{
	...
	DBG_WB << "Inserting future recruit [" << temp_unit_->id() << "] at position " << temp_unit_->get_location() << ".";
	...
	// in the past there was a bug where the map changed the unit ids here (because a unit with that id already existed)
	// which caused crashes later.
...
void recruit::do_hide()
{
	fake_unit_->set_hidden(true);
}
void recruit::do_show()
{
