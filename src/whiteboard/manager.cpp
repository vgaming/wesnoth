...
{
	if(wait_for_side_init_ || resources::gameboard == nullptr || executing_actions_
		|| resources::gameboard->is_observer() || resources::controller->is_linger_mode()
...
static void draw_numbers(const map_location& hex, side_actions::numbers_t numbers)
{
	...
	for(std::size_t i = 0; i < size; ++i) {
		...
		display::get_singleton()->draw_text_in_hex(
			hex, drawing_layer::actions_numbering, number_text, font_size, color, x_in_hex, y_in_hex);
...
void manager::on_mouseover_change(const map_location& hex)
{
	...
	if(!((selected_hex.valid() && hex_has_unit) || has_temp_move() || wait_for_side_init_ || executing_actions_)) {
		if(!highlighter_) {
...
void manager::create_temp_move()
{
	...
	const pathfind::marked_route& route = resources::controller->get_mouse_handler_base().get_current_route();
	...
	if(!temp_moved_unit)
		return;
	if(temp_moved_unit->side() != display::get_singleton()->viewing_team().side())
		return;
	...
	for(; curr_itor != end_itor; ++curr_itor) {
		...
		if(w != route.marks.end() && w->second.turns > 0) {
			...
			if(!move_arrow) {
				...
				move_arrow->set_color(team::get_side_color_id(viewer_side()));
			...
			if(path.size() >= 2) {
				...
				// So before assuming that a pre-existing fake_unit can be re-used, check that its ID matches the unit
				// being moved.
...
void manager::save_temp_move()
{
	if(has_temp_move() && !executing_actions_ && !resources::controller->is_linger_mode()) {
		...
		for(std::size_t i = 0; i < size; ++i) {
			...
			// path_cost() is incomplete as it for example doesn't handle skirmisher, we let the move action generate
			// the costs on it own. route.move_cost = path_cost(route.steps,*u);
			route.move_cost = -1;
...
void manager::save_temp_attack(const map_location& attacker_loc, const map_location& defender_loc, int weapon_choice)
{
	if(active_ && !executing_actions_ && !resources::controller->is_linger_mode()) {
		...
		side_actions& sa = *viewer_actions();
		sa.queue_attack(sa.get_turn_num_of(*attacking_unit), *attacking_unit, defender_loc, weapon_choice, *route_,
			move_arrow, fake_unit ? std::move(*fake_unit) : fake_unit_ptr());
