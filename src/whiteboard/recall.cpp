...
void recall::execute(bool& success, bool& complete)
{
	...
	bool const result = synced_context::run_and_throw(
		"recall", replay_helper::get_recall(temp_unit_->id(), recall_hex_, map_location::null_location()));
...
void recall::apply_temp_modifier(unit_map& unit_map)
{
	...
	DBG_WB << "Inserting future recall " << temp_unit_->name() << " [" << temp_unit_->id() << "] at position "
		   << temp_unit_->get_location() << ".";

	// temporarily remove unit from recall list
	unit_ptr it = resources::gameboard->teams()
					  .at(team_index())
					  .recall_list()
					  .extract_if_matches_id(temp_unit_->id(), &original_recall_pos_);
	assert(it);

	// Usually (temp_unit_ == it) is true here, but wml might have changed the original unit in which case not doing
	// 'temp_unit_ = it' would result in a gamestate change.
	temp_unit_ = it;
...
void recall::draw_hex(const map_location& hex)
{
	if(hex == recall_hex_) {
		...
		display::get_singleton()->draw_text_in_hex(
			hex, drawing_layer::actions_numbering, number_text.str(), font_size, color, x_offset, y_offset);
	}
}
