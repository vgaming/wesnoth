...
void terrain_type_data::lazy_initialization() const
{
	...
	for(const config& terrain_data : game_config_.child_range("terrain_type")) {
		...
		DBG_G << "create_terrain_maps: " << terrain.number() << " " << terrain.id() << " " << terrain.name() << " : "
		...
		if(!res.second) {
			...
			if(terrain == curr) {
				LOG_G << "Merging terrain " << terrain.number() << ": " << terrain.id() << " (" << terrain.name()
					  << ")";
				...
				if(clean_merge) {
					...
					LOG_G << "Merged terrain " << terrain.number() << ": " << terrain.id() << " (" << terrain.name()
						  << ") "
...
const t_translation::ter_list& terrain_type_data::underlying_mvt_terrain(
	const t_translation::terrain_code& terrain) const
{
	auto i = find_or_create(terrain);
...
const t_translation::ter_list& terrain_type_data::underlying_def_terrain(
	const t_translation::terrain_code& terrain) const
{
	auto i = find_or_create(terrain);
...
const t_translation::ter_list& terrain_type_data::underlying_union_terrain(
	const t_translation::terrain_code& terrain) const
{
	auto i = find_or_create(terrain);
...
{
	t_string str = get_terrain_info(terrain).description();
...
{
	t_string str = get_terrain_info(terrain).editor_name();
	const t_string& desc = get_terrain_info(terrain).description();
...
terrain_type_data::tcodeToTerrain_t::const_iterator terrain_type_data::find_or_create(
	t_translation::terrain_code terrain) const
{
	lazy_initialization();
