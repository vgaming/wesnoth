...
bool terrain_filter::match_internal(const map_location& loc, const unit* ref_unit, const bool ignore_xy) const
{
	...
	// Filter Areas
	if(cfg_.has_attribute("area") && fc_->get_tod_man().get_area_by_id(cfg_["area"]).count(loc) == 0)
		return false;
	...
	if(cfg_.has_child("filter_adjacent_location")) {
		...
		for(i = i_begin, i_end = adj_cfgs.end(); i != i_end; ++i) {
			...
			std::vector<map_location::direction> dirs = (*i).has_attribute("adjacent")
				? map_location::parse_directions((*i)["adjacent"])
			...
			for(j = dirs.begin(); j != j_end; ++j) {
				...
				if(fc_->get_disp_context().map().on_board(adj)) {
					if(cache_.adjacent_matches == nullptr) {
						while(index
							>= std::distance(cache_.adjacent_match_cache.begin(), cache_.adjacent_match_cache.end())) {
	...
	if(!tod_type.empty() || !tod_id.empty()) {
		...
		if(flat_) {
			...
			tod = fc_->get_tod_man().get_illuminated_time_of_day(
				fc_->get_disp_context().units(), fc_->get_disp_context().map(), loc);
		}
...
bool terrain_filter::match_impl(const map_location& loc, const unit* ref_unit) const
{
	...
	if(ref_unit) {
		if(fc_->get_disp_context().map().on_board(ref_unit->get_location())) {
			ref_unit_var.reset(
				new scoped_xy_unit("teleport_unit", ref_unit->get_location(), fc_->get_disp_context().units()));
		} else {
	...
	if(radius > max_loop_) {
		ERR_NG << "terrain_filter: radius greater than " << max_loop_ << ", restricting";
		radius = max_loop_;
	}
	if(radius == 0)
		...
		if(ref_unit) {
			get_tiles_radius(
				fc_->get_disp_context().map(), loc_vec, radius, hexes, false, filter_with_unit(r_filter, *ref_unit));
		} else {
	...
	for(i = hexes.begin(); i != hexes.end(); ++i) {
		...
		if(++loop_count > max_loop_) {
			...
			if(++temp != hexes.end()) {
				ERR_NG << "terrain_filter: loop count greater than " << max_loop_ << ", aborting";
				break;
			}
		}
	}
...
public:
	...
	static void filter_final(
		T&& src, location_set& dest, const terrain_filter&, const F1& f1, const F2& f2, const F3& f3)
	{
	...
	static void filter_special_loc(
		T&& src, location_set& dest, const terrain_filter& filter, const F1& f1, const F2& f2)
	{
		if(filter.cfg_.has_attribute("location_id")) {
			...
			filter_final(src, dest, filter, f1, f2,
				[matching_locs](const map_location& loc) { return matching_locs.count(loc) > 0; });
		} else {
	...
	static void filter_area(T&& src, location_set& dest, const terrain_filter& filter, const F1& f1)
	{
		if(filter.cfg_.has_attribute("area")) {
			...
			filter_special_loc(
				src, dest, filter, f1, [&area](const map_location& loc) { return area.find(loc) != area.end(); });
		} else {
	...
	static void filter_xy(T&& src, location_set& dest, const terrain_filter& filter, bool with_border)
	{
		if(filter.cfg_.has_attribute("x") || filter.cfg_.has_attribute("y")) {
			std::vector<map_location> xy_vector = filter.fc_->get_disp_context().map().parse_location_range(
				filter.cfg_["x"], filter.cfg_["y"], with_border);
			filter_area(src, dest, filter, [&xy_vector](const map_location& loc) {
...
struct cfg_to_loc
{
	...
	{
		return map_location(cfg, nullptr);
	}
...
void terrain_filter::get_locs_impl(std::set<map_location>& locs, const unit* ref_unit, bool with_border) const
{
	...
	if(cfg_.has_attribute("find_in")) {
		if(const game_data* gd = fc_->get_game_data()) {
			try {
				...
				terrain_filterimpl::filter_xy(
					ar | boost::adaptors::transformed(cfg_to_loc()), match_set, *this, with_border);
			} catch(const invalid_variablename_exception&) {
				// Do nothing
			}
		}
		...
		terrain_filterimpl::filter_special_loc(
			area, match_set, *this, terrain_filterimpl::no_filter(), terrain_filterimpl::no_filter());
	...
	int ors_left
		= std::count_if(cfg_.ordered_begin(), cfg_.ordered_end(), [](const auto& val) { return val.first == "or"; });
