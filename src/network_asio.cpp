	...
	, bytes_read_(0)
{
	...
	if(!ec) { // if numeric resolve succeeds then we got raw ip address so TLS host name validation would never pass
		...
		resolver_.async_resolve(
			host, service, std::bind(&connection::handle_resolve, this, std::placeholders::_1, std::placeholders::_2));
	}
...
void connection::fallback_to_unencrypted()
{
	...
	utils::get<raw_socket>(socket_)->async_connect(
		endpoint, std::bind(&connection::handle_connect, this, std::placeholders::_1, endpoint));
}
...
void connection::transfer(const config& request, config& response)
{
	...
	utils::visit(
		[this, &bufs, &response](auto&& socket) {
			...
			boost::asio::async_read(*socket, *read_buf_,
				...
				std::bind(
					&connection::handle_read, this, std::placeholders::_1, std::placeholders::_2, std::ref(response)));
		},
		socket_);
}
