...
void command_executor::surrender_game()
{
	if(gui2::show_message(
		   _("Surrender"), _("Do you really want to surrender the game?"), gui2::dialogs::message::yes_no_buttons)
		!= gui2::retval::CANCEL) {
...
void command_executor::show_menu(
	const std::vector<config>& items_arg, int xloc, int yloc, bool /*context_menu*/, display& gui)
{
...
void command_executor::execute_action(
	const std::vector<std::string>& items_arg, int /*xloc*/, int /*yloc*/, bool /*context_menu*/, display&)
{
...
void command_executor::queue_command(const SDL_Event& event, int index)
{
	...
	bool keypress = (event.type == SDL_KEYDOWN || event.type == SDL_TEXTINPUT) && !press_event_sent_;
	bool press = keypress
...
void command_executor::execute_command_wrap(const command_executor::queued_command& command)
{
	...
	if(!can_execute_command(ui_cmd) || do_execute_command(ui_cmd, command.press, command.release)) {
		return;
	}
