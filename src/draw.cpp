...
void draw::fill(const SDL_Rect& area, uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
...
void draw::fill(const SDL_Rect& area, uint8_t r, uint8_t g, uint8_t b)
{
...
{
	DBG_D << "line from (" << from_x << ',' << from_y << ") to (" << to_x << ',' << to_y << ')';
...
{
	DBG_D << "line from (" << from_x << ',' << from_y << ") to (" << to_x << ',' << to_y << ") with colour " << c;
	SDL_SetRenderDrawColor(renderer(), c.r, c.g, c.b, c.a);
	SDL_RenderDrawLine(renderer(), from_x, from_y, to_x, to_y);
}
...
{
	DBG_D << "circle (" << cx << ',' << cy << ") -> " << r << ", oct " << int(octants);
...
{
	DBG_D << "disc (" << cx << ',' << cy << ") -> " << r << ", oct " << int(octants);
...
void draw::cairo_circle(int cx, int cy, int r, const color_t& c, int thickness)
{
	...
	auto cairo_surface
		= cairo::create_surface(reinterpret_cast<uint8_t*>(sdl_surf->pixels), ::point(sdl_surf->w, sdl_surf->h));
	...
	cairo_set_source_rgba(ctx, c.r / 255.0, c.g / 255.0, c.b / 255.0, c.a / 255.0);
...
static SDL_RendererFlip get_flip(bool flip_h, bool flip_v)
{
	...
	return static_cast<SDL_RendererFlip>(static_cast<int>(flip_h ? SDL_FLIP_HORIZONTAL : SDL_FLIP_NONE)
		| static_cast<int>(flip_v ? SDL_FLIP_VERTICAL : SDL_FLIP_NONE));
}

void draw::flipped(const texture& tex, const SDL_Rect& dst, bool flip_h, bool flip_v)
{
	if(dst == sdl::empty_rect) {
	...
	DBG_D << "flipped (" << flip_h << '|' << flip_v << ") to " << dst;
...
void draw::tiled(const texture& tex, const SDL_Rect& dst, bool centered, bool mirrored)
{
	if(!tex) {
		DBG_D << "null tiled";
		return;
	}
	DBG_D << "tiled (" << centered << '|' << mirrored << ") " << dst;
...
void draw::tiled_highres(const texture& tex, const SDL_Rect& dst, bool centered, bool mirrored)
{
	if(!tex) {
		DBG_D << "null tiled_highres";
		return;
	}
	DBG_D << "tiled_highres (" << centered << '|' << mirrored << ") " << dst;
	...
	const SDL_Color& cBR)
{
	...
	draw::smooth_shaded(tex, dst, cTL, cTR, cBL, cBR, uv[0], uv[1], uv[2], uv[3]);
}
...
void draw::smooth_shaded(const texture& tex, const std::array<SDL_Vertex, 4>& verts)
{
	...
	for(const SDL_Vertex& v : verts) {
		DBG_D << "  {(" << v.position.x << ',' << v.position.y << ") " << v.color << " (" << v.tex_coord.x << ','
			  << v.tex_coord.y << ")}";
	}
...
draw::clip_setter::clip_setter(const SDL_Rect& clip)
	: c_(draw::get_clip())
	, clip_enabled_(draw::clip_enabled())
{
	...
	, clip_enabled_(draw::clip_enabled())
{
	...
	if(clip_enabled_) {
		...
		SDL_Rect c_view = {c_.x + v_.x - view.x, c_.y + v_.y - view.y, c_.w, c_.h};
...
{
	if(t) {
		DBG_D << "setting render target to " << t.w() << 'x' << t.h() << " texture";
	} else {
