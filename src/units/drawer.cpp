...
std::unique_ptr<image::locator> get_playing_ally_orb_image(orb_status os)
{
	...
	return std::make_unique<image::locator>(
		game_config::images::orb_two_color + "~RC(ellipse_red>" + allied_color + ")~RC(magenta>" + status_color + ")");
}
...
void draw_bar(int index, const energy_bar& data, const rect& bounds)
{
	...
	SDL_FPoint p2{std::min(p1.x + float(data.def_w) / tile_size, 1.0f - p1.x),
...
void unit_drawer::redraw_unit(const unit& u) const
{
	...
	if(ac.draw_bars_ && unit_rect.overlaps(disp.map_outside_area())) {
		...
		if(cfg_offset_x.empty() && cfg_offset_y.empty()) {
			const point s = display::scaled_to_zoom(image::get_size(u.default_anim_image()));
		...
		if(u.max_hitpoints() > 0) {
			bars.AGGREGATE_EMPLACE(energy_bar::get_height(u.max_hitpoints(), u.hp_bar_scaling()),
	...
	// height_adjust_unit is not scaled by zoom_factor here otherwise it results in a squared offset that results in
	// #5974 It appears the tiles and units are scaled together somewhere else
	...
	const int halo_x
		= static_cast<int>(adjusted_params.offset * xdst + (1.0 - adjusted_params.offset) * xsrc) + hex_size_by_2;
	const int halo_y = static_cast<int>(adjusted_params.offset * ydst + (1.0 - adjusted_params.offset) * ysrc)
	...
	if(!has_halo && !u.image_halo().empty()) {
		ac.unit_halo_ = halo_man.add(halo_x, halo_y, u.image_halo() + u.TC_image_mods(), map_location(-1, -1));
	}
	if(has_halo && u.image_halo().empty()) {
	...
	if(!has_abil_halo && !halos_abilities.empty()) {
		for(const std::string& halo_ab : halos_abilities) {
			halo::handle abil_halo = halo_man.add(halo_x, halo_y, halo_ab + u.TC_image_mods(), map_location(-1, -1));
			if(abil_halo->valid()) {
...
void unit_drawer::draw_ellipses(const unit& u, const frame_parameters& params) const
{
	...
	std::array images{image::get_texture(image::locator{path.str() + "-top.png", ipf}),
		image::get_texture(image::locator{path.str() + "-bottom.png", ipf})};

	// The division by 2 seems to have no real meaning,
	// It just works fine with the current center of ellipse
	// and prevent a too large adjust if submerge = 1.0
	const int y_shift = params.submerge > 0.0 ? params.y - static_cast<int>(params.submerge * hex_size_by_2) : params.y;

	disp.drawing_buffer_add(
		drawing_layer::unit_first, u.get_location(), [images = std::move(images), y_shift](const rect& dest) {
