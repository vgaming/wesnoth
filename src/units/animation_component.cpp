...
void unit_animation_component::set_standing(bool with_bars)
{
	if(prefs::get().show_standing_animations() && !u_.incapacitated()) {
		start_animation(std::chrono::milliseconds::max(), choose_animation(u_.loc_, "standing"), with_bars, "",
			{0, 0, 0}, STATE_STANDING);
	} else {
		start_animation(std::chrono::milliseconds::max(), choose_animation(u_.loc_, "_disabled_"), with_bars, "",
			{0, 0, 0}, STATE_STANDING);
	}
}
...
{
	start_animation(std::chrono::milliseconds::max(), choose_animation(u_.loc_, "_ghosted_"), with_bars);
	anim_->pause_animation();
}

void unit_animation_component::set_disabled_ghosted(bool with_bars)
{
	start_animation(std::chrono::milliseconds::max(), choose_animation(u_.loc_, "_disabled_ghosted_"), with_bars);
}

void unit_animation_component::set_idling()
{
	start_animation(
		std::chrono::milliseconds::max(), choose_animation(u_.loc_, "idling"), true, "", {0, 0, 0}, STATE_FORGET);
}
...
void unit_animation_component::set_selecting()
{
	if(prefs::get().show_standing_animations() && !u_.incapacitated()) {
		start_animation(
			std::chrono::milliseconds::max(), choose_animation(u_.loc_, "selected"), true, "", {0, 0, 0}, STATE_FORGET);
	} else {
		start_animation(std::chrono::milliseconds::max(), choose_animation(u_.loc_, "_disabled_selected_"), true, "",
			{0, 0, 0}, STATE_FORGET);
	}
}
	...
	STATE state)
{
	...
	anim_->start_animation(real_start_time, u_.loc_, u_.loc_.get_direction(u_.facing_), text, text_color, accelerate);
...
void unit_animation_component::refresh()
{
	...
	if(state_ != STATE_STANDING || get_current_animation_tick() < next_idling_ || !disp.tile_nearly_on_screen(u_.loc_)
		|| u_.incapacitated()) {
		return;
	}
...
std::vector<std::string> unit_animation_component::get_flags()
{
	...
	for(const auto& anim : animations_) {
		...
		std::copy_if(flags.begin(), flags.end(), std::inserter(result, result.begin()),
			[](const std::string& flag) { return !(flag.empty() || (flag.front() == '_' && flag.back() == '_')); });
	}
