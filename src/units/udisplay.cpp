	...
	display& disp)
{
	...
	disp.invalidate(a);
	temp_unit->set_facing(a.get_relative_dir(b));
	animator.replace_anim_if_invalid(temp_unit, "movement", a, b, step_num, false, "", {0, 0, 0},
		strike_result::type::invalid, nullptr, nullptr, step_left);
	animator.start_animations();
	...
	for(; current_ < path_index; ++current_) {
		...
		if(!is_enemy_ || !locs.empty()) {
			...
			if(!disp_->tile_fully_on_screen(path_[current_]) || !disp_->tile_fully_on_screen(path_[current_ + 1])) {
				...
				disp_->scroll_to_tiles(locs, game_display::ONSCREEN, true, false, 0.0, force_scroll_);
				if(temp_unit_ptr_->anim_comp().get_animation())
					temp_unit_ptr_->anim_comp().get_animation()->restart_animation();
			}

			if(tiles_adjacent(path_[current_], path_[current_ + 1]))
				wait_until_ = move_unit_between(path_[current_], path_[current_ + 1], temp_unit_ptr_.get_unit_ptr(),
					current_, path_.size() - (current_ + 2), animator_, *disp_);
...
void unit_mover::finish(const unit_ptr& u, map_location::direction dir)
{
	...
	const map_location::direction final_dir
		= current_ == 0 ? path_[0].get_relative_dir(path_[1]) : path_[current_ - 1].get_relative_dir(end_loc);
 ...
 */
void move_unit(const std::vector<map_location>& path,
	const unit_ptr& u,
	bool animate,
	...
	const unit_ptr& defender)
{
	...
	animator.add_animation(attacker.shared_from_this(), "draw_weapon", loc, defender_loc, 0, true, "", {0, 0, 0},
		strike_result::type::hit, attack, secondary_attack, 0);
	if(defender) {
		animator.add_animation(defender, "draw_weapon", defender_loc, loc, 0, true, "", {0, 0, 0},
			strike_result::type::miss, secondary_attack, attack, 0);
	}
	animator.start_animations();
	animator.wait_for_end();
}
	...
	const unit_ptr& secondary_unit)
{
	...
	unit_animator animator;
	if(primary_unit) {
		animator.add_animation(primary_unit, "sheath_weapon", primary_loc, secondary_loc, 0, true, "", {0, 0, 0},
			strike_result::type::invalid, primary_attack, secondary_attack, 0);
	}
	if(secondary_unit) {
		animator.add_animation(secondary_unit, "sheath_weapon", secondary_loc, primary_loc, 0, true, "", {0, 0, 0},
			strike_result::type::invalid, secondary_attack, primary_attack, 0);
	}
	...
	const unit_ptr& winner)
{
	...
	animator.add_animation(loser.shared_from_this(), "death", loc, winner_loc, 0, false, "", {0, 0, 0},
		strike_result::type::kill, attack, secondary_attack, 0);
	// but show the bars of the winner (avoid blinking and show its xp gain)
	if(winner) {
		animator.add_animation(winner, "victory", winner_loc, loc, 0, true, "", {0, 0, 0}, strike_result::type::kill,
			secondary_attack, attack, 0);
	}
	animator.start_animations();
	animator.wait_for_end();
	...
	bool attacking)
{
	...
	auto ctx = weapon->specials_context(
		attacker.shared_from_this(), defender.shared_from_this(), a, b, attacking, secondary_attack);
	utils::optional<decltype(ctx)> opp_ctx;

	if(secondary_attack) {
		opp_ctx.emplace(secondary_attack->specials_context(
			defender.shared_from_this(), attacker.shared_from_this(), b, a, !attacking, weapon));
	}
	...
	animator.add_animation(attacker.shared_from_this(), "attack", att->get_location(), def->get_location(), damage,
		true, text_2, (drain_amount >= 0) ? color_t(0, 255, 0) : color_t(255, 0, 0), hit_type, weapon, secondary_attack,
		swing);

	// note that we take an anim from the real unit, we'll use it later
	const unit_animation* defender_anim = def->anim_comp().choose_animation(
		def->get_location(), "defend", att->get_location(), damage, hit_type, weapon, secondary_attack, swing);
	...
	for(const unit_ability& ability : leadership_list) {
		...
		animator.add_animation(leader.get_shared_ptr(), "leading", ability.teacher_loc, att->get_location(), damage,
			true, "", {0, 0, 0}, hit_type, weapon, secondary_attack, swing);
	}
	...
	for(const unit_ability& ability : resistance_list) {
		...
		animator.add_animation(helper.get_shared_ptr(), "resistance", ability.teacher_loc, def->get_location(), damage,
			true, "", {0, 0, 0}, hit_type, weapon, secondary_attack, swing);
	}
	...
	for(const unit_ability& ability : abilities) {
		...
		assert(leader.valid());
		leader->set_facing(ability.teacher_loc.get_relative_dir(a));
		if(animator.has_animation(leader.get_shared_ptr(), "leading", ability.teacher_loc, att->get_location(), damage,
			   hit_type, weapon, secondary_attack, swing)
			&& leading_playable) {
			continue;
		}
		if(animator.has_animation(leader.get_shared_ptr(), "resistance", ability.teacher_loc, def->get_location(),
			   damage, hit_type, weapon, secondary_attack, swing)
			&& helping_playable) {
			continue;
		}
		animator.add_animation(leader.get_shared_ptr(), "teaching", ability.teacher_loc, att->get_location(), damage,
			true, "", {0, 0, 0}, hit_type, weapon, secondary_attack, swing);
	}
...
void unit_healing(unit& healed, const std::vector<unit*>& healers, int healing, const std::string& extra_text)
{
	...
	const bool some_healer_is_unfogged = (healers.end()
		!= std::find_if_not(healers.begin(), healers.end(), [&](unit* h) { return disp->fogged(h->get_location()); }));
	...
	for(unit* h : healers) {
		...
		animator.add_animation(h->shared_from_this(), "healing", h->get_location(), healed_loc, healing);
	}

	if(healing < 0) {
		animator.add_animation(healed.shared_from_this(), "poisoned", healed_loc, map_location::null_location(),
			-healing, false, number_and_text(-healing, extra_text), {255, 0, 0});
	} else if(healing > 0) {
		animator.add_animation(healed.shared_from_this(), "healed", healed_loc, map_location::null_location(), healing,
			false, number_and_text(healing, extra_text), {0, 255, 0});
	} else {
		animator.add_animation(healed.shared_from_this(), "healed", healed_loc, map_location::null_location(), 0, false,
			extra_text, {0, 255, 0});
	}
	animator.start_animations();
	animator.wait_for_end();
