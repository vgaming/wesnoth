...
std::vector<const unit*> unit_filter::all_matches_on_map(const map_location* loc, const unit* other_unit) const
{
	...
	for(const unit& u : fc_->get_disp_context().units()) {
		if(impl_.matches(
			   unit_filter_impl::unit_filter_args{u, loc ? *loc : u.get_location(), other_unit, fc_, use_flat_tod_})) {
			if(max_matches == 0) {
				return ret;
			}
			--max_matches;
...
{
	unit_filter_xy(const std::string& x, const std::string& y)
		: x_(x)
		, y_(y)
	{
	}
...
struct unit_filter_adjacent : public unit_filter_base
{
	...
	virtual bool matches(const unit_filter_args& args) const override
	{
		...
		for(map_location::direction dir : dirs) {
			...
			if(!is_enemy.empty()
				&& is_enemy.to_bool()
					!= args.context().get_disp_context().get_team(args.u.side()).is_enemy(unit_itor->side())) {
				continue;
			}
			++match_count;
		}
...
{
	unit_filter_child_literal(const vconfig& v, const F& f)
		: v_(v)
		, f_(f)
	{
	}
...
{
	unit_filter_attribute_parsed(T&& v, F&& f)
		: v_(std::move(v))
		, f_(std::move(f))
	{
	}
...
{
	unit_filter_attribute_literal(std::string&& v, C&& c, F&& f)
		: v_(std::move(v))
		, c_(std::move(c))
		, f_(std::move(f))
	{
...
void unit_filter_compound::create_attribute(const config::attribute_value v, C conv, F func)
{
	if(v.blank()) {
		...
		children_.emplace_back(
			new unit_filter_attribute_literal<C, F>(std::move(v.str()), std::move(conv), std::move(func)));
	} else {
		children_.emplace_back(
...
void get_ability_children_id(std::vector<ability_match>& id_result, const config& parent, const std::string& id)
{
...
void unit_filter_compound::fill(const vconfig& cfg)
{
	...
	create_attribute(
		literal["name"], [](const config::attribute_value& c) { return c.t_str(); },
		[](const t_string& str, const unit_filter_args& args) { return str == args.u.name(); });

	create_attribute(
		literal["id"], [](const config::attribute_value& c) { return utils::split(c.str()); },
	...
	create_attribute(
		literal["type"], [](const config::attribute_value& c) { return utils::split(c.str()); },
	...
	create_attribute(
		literal["type_adv_tree"], [](const config::attribute_value& c) { return utils::split(c.str()); },
		[](const std::vector<std::string>& types, const unit_filter_args& args) {
	...
	create_attribute(
		literal["variation"], [](const config::attribute_value& c) { return utils::split(c.str()); },
		[](const std::vector<std::string>& types, const unit_filter_args& args) {
			return std::find(types.begin(), types.end(), args.u.variation()) != types.end();
		});

	create_attribute(
		literal["has_variation"], [](const config::attribute_value& c) { return utils::split(c.str()); },
		[](const std::vector<std::string>& types, const unit_filter_args& args) {
			// If this unit is a variation itself then search in the base unit's variations.
			const unit_type* const type
				= args.u.variation().empty() ? &args.u.type() : unit_types.find(args.u.type().parent_id());
			assert(type);
	...
	create_attribute(
		literal["ability"], [](const config::attribute_value& c) { return utils::split(c.str()); },
	...
	create_attribute(
		literal["ability_type"], [](const config::attribute_value& c) { return utils::split(c.str()); },
		[](const std::vector<std::string>& abilities, const unit_filter_args& args) {
	...
	create_attribute(
		literal["ability_id_active"], [](const config::attribute_value& c) { return utils::split(c.str()); },
		[](const std::vector<std::string>& abilities, const unit_filter_args& args) {
			const unit_map& units = args.context().get_disp_context().units();
			for(const std::string& ability : abilities) {
	...
	create_attribute(
		literal["ability_type_active"], [](const config::attribute_value& c) { return utils::split(c.str()); },
		[](const std::vector<std::string>& abilities, const unit_filter_args& args) {
			for(const std::string& ability : abilities) {
	...
	create_attribute(
		...
		[](const std::vector<std::string>& check_traits, const unit_filter_args& args) {
			...
			std::set_intersection(check_traits.begin(), check_traits.end(), have_traits.begin(), have_traits.end(),
				std::back_inserter(isect));
			return !isect.empty();
		});

	create_attribute(
		literal["race"], [](const config::attribute_value& c) { return utils::split(c.str()); },
	...
	create_attribute(
		literal["gender"], [](const config::attribute_value& c) { return string_gender(c.str()); },
		[](unit_race::GENDER gender, const unit_filter_args& args) { return gender == args.u.gender(); });
	...
	create_attribute(
		literal["status"], [](const config::attribute_value& c) { return utils::split(c.str()); },
	...
	create_attribute(
		literal["has_weapon"], [](const config::attribute_value& c) { return c.str(); },
	...
	create_attribute(
		literal["role"], [](const config::attribute_value& c) { return c.str(); },
		[](const std::string& role, const unit_filter_args& args) { return args.u.get_role() == role; });

	create_attribute(
		literal["alignment"], [](const config::attribute_value& c) { return c.str(); },
		[](const std::string& alignment, const unit_filter_args& args) {
			return unit_alignments::get_string(args.u.alignment()) == alignment;
		});

	create_attribute(
		literal["ai_special"], [](const config::attribute_value& c) { return c.str(); },
		[](const std::string& ai_special, const unit_filter_args& args) {
			return (ai_special == "guardian") == args.u.get_state(unit::STATE_GUARDIAN);
		});

	create_attribute(
		literal["usage"], [](const config::attribute_value& c) { return utils::split(c.str()); },
	...
	create_attribute(
		literal["canrecruit"], [](const config::attribute_value& c) { return c.to_bool(); },
		[](bool canrecruit, const unit_filter_args& args) { return args.u.can_recruit() == canrecruit; });

	create_attribute(
		literal["recall_cost"], [](const config::attribute_value& c) { return utils::parse_ranges_unsigned(c.str()); },
		[](const std::vector<std::pair<int, int>>& ranges, const unit_filter_args& args) {
			return in_ranges(args.u.recall_cost(), ranges);
		});

	create_attribute(
		literal["level"], [](const config::attribute_value& c) { return utils::parse_ranges_unsigned(c.str()); },
		[](const std::vector<std::pair<int, int>>& ranges, const unit_filter_args& args) {
	...
	create_attribute(
		literal["vision_cost"], [](const config::attribute_value& c) { return utils::parse_ranges_unsigned(c.str()); },
		[](const std::vector<std::pair<int, int>>& ranges, const unit_filter_args& args) {
	...
	create_attribute(
		literal["jamming_cost"], [](const config::attribute_value& c) { return utils::parse_ranges_unsigned(c.str()); },
		[](const std::vector<std::pair<int, int>>& ranges, const unit_filter_args& args) {
	...
	create_attribute(
		literal["lua_function"], [](const config::attribute_value& c) { return c.str(); },
		...
		[](const config::attribute_value& c) {
			try {
				...
				lg::log_to_chat() << "Formula error while evaluating formula in unit filter: " << e.type << " at "
								  << e.filename << ':' << e.line << ")\n";
				ERR_WML << "Formula error while evaluating formula in unit filter: " << e.type << " at " << e.filename
						<< ':' << e.line << ")";
				return wfl::formula("");
			}
		},
	...
	create_attribute(
		literal["find_in"], [](const config::attribute_value& c) { return c.str(); },
		...
		if(cond) {
			...
			create_child(child.second, [](const vconfig& c, const unit_filter_args& args) {
				...
				for(const int viewer : viewers) {
					...
					// Check is_enemy() before invisible() to prevent infinite recursion in
					// [abilities][hides][filter_self][filter_vision]
					bool hiding = args.context().get_disp_context().get_team(viewer).is_enemy(args.u.side())
			...
			if(child.first == "experimental_filter_ability") {
				deprecated_message(
					"experimental_filter_ability", DEP_LEVEL::INDEFINITE, "", "Use filter_ability instead.");
			}
			create_child(child.second, [](const vconfig& c, const unit_filter_args& args) {
