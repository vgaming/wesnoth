...
bool game_config_manager::init_game_config(FORCE_RELOAD_CONFIG force_reload)
{
	...
	game_config::scoped_preproc_define title_screen(
	...
	FORCE_RELOAD_CONFIG force_reload, const game_classification* classification, const std::string& scenario_id)
{
	...
	game_config::scoped_preproc_define debug_mode("DEBUG_MODE", game_config::debug || game_config::mp_debug);
	...
	try {
		...
		if(reload_everything) {
			...
			for(const config& core : cores_cfg.child_range("core")) {
				...
				if(id.empty()) {
					events::call_in_main_thread([&]() {
						gui2::dialogs::wml_error::display(_("Error validating data core."),
			...
			if(!current_core_valid) {
				events::call_in_main_thread([&]() {
					gui2::dialogs::wml_error::display(_("Error loading core data."),
			...
			if(cmdline_opts_.validate_core) {
				validator.reset(new schema_validation::schema_validator(
					filesystem::get_wml_location("schema/game_config.cfg").value()));
		...
		// only after addon configs have been loaded do we check for which addons are needed and whether they exist to
		// be used
		LOG_CONFIG << "active_addons_ has size " << active_addons_.size()
				   << " and contents: " << utils::join(active_addons_);
		if(classification) {
			LOG_CONFIG << "Enabling only some add-ons!";
			std::set<std::string> active_addons = classification->active_addons(scenario_id);
			// IMPORTANT: this is a significant performance optimization, particularly for the worst case example of the
			// batched WML unit tests
			if(!reload_everything && active_addons == active_addons_) {
				...
				LOG_CONFIG << "active_addons has size " << active_addons_.size()
						   << " and contents: " << utils::join(active_addons);
				return;
			}
		...
		if(!game_config::no_addons) {
			...
			events::call_in_main_thread([&]() {
				gui2::dialogs::wml_error::display(_("Error loading custom game configuration files. The game will "
													"fallback to the default core files."),
					e.message);
			});
			prefs::get().set_core("default");
			...
			events::call_in_main_thread([&]() {
				gui2::dialogs::wml_error::display(
					_("Error loading default core game configuration files. The game will now exit."), e.message);
			});
			throw;
		}
	}
...
static void show_deprecated_warnings(config& umc_cfg)
{
		...
		for(auto& unit_type : units.child_range("unit_type")) {
			for(const auto& advancefrom : unit_type.child_range("advancefrom")) {
				auto symbols = utils::string_map{
					{"lower_level", advancefrom["unit"].str()}, {"higher_level", unit_type["id"].str()}};
				auto message = VGETTEXT(
					...
					"Error: [advancefrom] no longer works. ‘$lower_level’ units will not be able to advance to "
					"‘$higher_level’; please ask the add-on author to use [modify_unit_type] instead.",
					symbols);
	...
	static const std::set<std::string> deprecated_defines{"ENABLE_PARAGON", "DISABLE_GRAND_MARSHAL",
		"ENABLE_ARMAGEDDON_DRAKE", "ENABLE_DWARVISH_ARCANISTER", "ENABLE_DWARVISH_RUNESMITH", "ENABLE_WOLF_ADVANCEMENT",
		"ENABLE_NIGHTBLADE", "ENABLE_TROLL_SHAMAN", "ENABLE_ANCIENT_LICH", "ENABLE_DEATH_KNIGHT", "ENABLE_WOSE_SHAMAN"};
	...
	for(auto& campaign : umc_cfg.child_range("campaign")) {
		for(auto str : utils::split(campaign["extra_defines"])) {
			if(deprecated_defines.count(str) > 0) {
				...
				deprecated_message("campaign id='" + campaign["id"].str() + "' has extra_defines=" + str,
					DEP_LEVEL::REMOVED, {1, 15, 4},
...
void game_config_manager::load_addons_cfg()
{
	...
	for(const std::string& file : user_files) {
		if(filesystem::is_cfg(file)) {
			...
			const std::string log_msg = formatter()
				<< "The format '" << short_wml_path << "' (for single-file add-ons) is not supported anymore, use '"
	...
	filesystem::get_files_in_dir(user_campaign_dir, nullptr, &user_dirs, filesystem::name_mode::FILE_NAME_ONLY);
	...
	for(const std::string& addon_id : user_dirs) {
		...
		if(have_addon_pbl_info(addon_id)) {
			...
			try {
				...
				const std::string log_msg = formatter() << "The provided addon has an invalid pbl file"
														<< " for addon " << addon_id;
		...
		try {
			...
			static const std::set<std::string> tags_with_addon_id{
				"era", "modification", "resource", "multiplayer", "scenario", "campaign", "test"};
			...
			static const std::set<std::string> entry_tags{
				"era", "modification", "resource", "multiplayer", "scenario", "campaign"};
	...
	if(!error_addons.empty()) {
		...
		const std::string& msg1 = _n("The following add-on had errors and could not be loaded:",
			"The following add-ons had errors and could not be loaded:", n);
		const std::string& msg2 = _n("Please report this to the author or maintainer of this add-on.",
			"Please report this to the respective authors or maintainers of these add-ons.", n);

		const std::string& report = utils::join(error_log, "\n\n");
		events::call_in_main_thread([&]() { gui2::dialogs::wml_error::display(msg1, msg2, error_addons, report); });
	}
}
...
{
	game_config::scoped_preproc_define difficulty(classification.difficulty, !classification.difficulty.empty());
	game_config::scoped_preproc_define campaign(
		classification.campaign_define, !classification.campaign_define.empty());
	game_config::scoped_preproc_define scenario(
		classification.scenario_define, !classification.scenario_define.empty());
	game_config::scoped_preproc_define era(classification.era_define, !classification.era_define.empty());
	game_config::scoped_preproc_define multiplayer("MULTIPLAYER", classification.is_multiplayer());
	game_config::scoped_preproc_define mptest("MP_TEST", cmdline_opts_.mptest && classification.is_multiplayer());
