...
struct take_village_step : undo_action
{
	...
	take_village_step(int orig_village_owner, bool time_bonus, map_location loc)
...
public:
	...
	{
		return expected_end_ != real_end_;
	}
	/**
...
private: // functions
	...
	bool blocked() const
	{
		return blocked_loc_ != map_location::null_location();
	}
	/** Checks the expected route for hidden units. */
	void cache_hidden_units(const route_iterator& start, const route_iterator& stop);
	/** Fires the enter_hex or exit_hex event and updates our data as needed. */
	void fire_hex_event(const std::string& event_name, const route_iterator& current, const route_iterator& other);
	...
	route_iterator plot_turn(const route_iterator& start, const route_iterator& stop);
	...
	void post_wml(game_events::pump_result_t pump_res)
	{
		return post_wml(pump_res, full_end_);
	}
	...
	{
		return ambushed_ || blocked() || wml_removed_unit_ || wml_undo_disabled_ || fog_changed_ || teleport_failed_;
	}
	...
	inline bool check_for_obstructing_unit(const map_location& hex, const map_location& prev_hex);
	/** Moves the unit the next step. */
	inline bool do_move(
		const route_iterator& step_from, const route_iterator& step_to, unit_display::unit_mover& animator);
...
private: // data
	...
	route_iterator obstructed_; // Points to either full_end_ or a hex we cannot enter. This is used so that exit_hex
								// can fire before we decide we cannot enter this hex.
	...
	bool event_mutated_mid_move_; // Cache of wml_removed_unit_ || wml_move_aborted_ from just before the end-of-move
								  // handling.
	bool fog_changed_;
	bool sighted_;      // Records if sightings were made that could interrupt movement.
	bool sighted_stop_; // Records if sightings were made that did interrupt movement (the same as sighted_ unless
						// movement ended for another reason).
	...
	std::deque<int> moves_left_; // The front value is what the moving unit's remaining moves should be set to after the
								 // next step through the route.
...
unit_mover::unit_mover(const std::vector<map_location>& route,
	...
	, playing_team_is_viewing_(
		  display::get_singleton()->viewing_team_is_playing() || display::get_singleton()->show_everything())
	, route_(route)
	, begin_(route.begin())
	, full_end_(route.end())
...
unit_mover::~unit_mover()
{
	...
	if(!is_ai_move() && move_it_.valid() && move_it_->get_goto() == map_location::null_location()) {
...
inline void unit_mover::check_for_ambushers(const map_location& hex)
{
	...
	for(const map_location& loc : get_adjacent_tiles(hex)) {
		...
		if(neighbor_it != units.end() && current_team_->is_enemy(neighbor_it->side()) && neighbor_it->invisible(loc)) {
			// Ambushed!
			ambushed_ = true;
			ambush_stop_ = hex;
 ...
 */
inline bool unit_mover::check_for_obstructing_unit(const map_location& hex, const map_location& prev_hex)
{
	...
	if(!tiles_adjacent(hex, prev_hex)) {
		if(current_team_->is_enemy(blocking_unit->side())) {
			...
			const pathfind::teleport_map teleports
				= pathfind::get_teleport_locations(*move_it_, *current_team_, true, false, false);
 ...
 */
inline bool unit_mover::do_move(
	const route_iterator& step_from, const route_iterator& step_to, unit_display::unit_mover& animator)
{
 ...
 */
inline void unit_mover::handle_fog(const map_location& hex, bool new_animation)
{
	// Clear the fog.
	if(clearer_.clear_unit(
		   hex, *move_it_, *current_team_, nullptr, &enemy_count_, &friend_count_, spectator_, !new_animation)) {
...
inline bool unit_mover::is_reasonable_stop(const map_location& hex) const
{
	...
	return !resources::gameboard->map().is_village(hex) || current_team_->owns_village(hex);
}
 ...
 */
void unit_mover::cache_hidden_units(const route_iterator& start, const route_iterator& stop)
{
	// Clear the old cache.
	obstructed_ = full_end_;
	blocked_loc_ = map_location::null_location();
	...
	for(ambush_limit_ = start + 1; ambush_limit_ != stop; ++ambush_limit_) {
		...
		if(check_for_obstructing_unit(*ambush_limit_, *(ambush_limit_ - 1))) {
			...
			obstructed_
				= ambush_limit_++; // The limit needs to be after obstructed_ in order for the latter to do anything.
			break;
		}
 ...
 *
 */
void unit_mover::fire_hex_event(
	const std::string& event_name, const route_iterator& current, const route_iterator& other)
{
 ...
 */
unit_mover::route_iterator unit_mover::plot_turn(const route_iterator& start, const route_iterator& stop)
{
 ...
 *                  Set this to full_end_ (or do not supply it) to skip such recalculations (because movement has
 * finished).
 *
...
void unit_mover::post_wml(game_events::pump_result_t pump_res, const route_iterator& step)
{
	...
	current_team_ = &resources::gameboard->get_team(current_side_);
	current_uses_fog_
		= current_team_->fog_or_shroud() && (current_side_ != orig_side_ || current_team_->auto_shroud_updates());
...
void unit_mover::post_move()
{
	...
	if(move_it_.valid()) {
		// Update the moving unit.
		move_it_->set_interrupted_move(sighted_stop_ && !resources::whiteboard->is_executing_actions()
				? *(full_end_ - 1)
...
void unit_mover::feedback() const
{
	...
	if(playing_team_is_viewing_ && (enemy_count_ != 0 || friend_count_ != 0)) {
		...
		if(friend_count_ != 0 && enemy_count_ != 0) {
			...
			symbols["friendphrase"] = VNGETTEXT(
				"Part of 'Units sighted! (...)' sentence^1 friendly", "$friends friendly", friend_count_, symbols);
			// TRANSLATORS: This becomes the "enemyphrase" in "Units sighted! ($friendphrase, $enemyphrase)"
			symbols["enemyphrase"]
				= VNGETTEXT("Part of 'Units sighted! (...)' sentence^1 enemy", "$enemies enemy", enemy_count_, symbols);
...
static void move_unit_internal(unit_mover& mover)
{
	...
	config cn{
		...
		mover.stopped_early(),
		"final_hex_x",
		mover.final_hex().wml_x(),
		"final_hex_y",
		mover.final_hex().wml_y(),
	};
	bool matches_replay = checkup_instance->local_checkup(cn, co);
	if(!matches_replay) {
		replay::process_error("calculated movement destination (x=" + cn["final_hex_x"].str()
			+ " y=" + cn["final_hex_y"].str() + ") didn't match the original destination(x=" + co["final_hex_x"].str()
			+ " y=" + co["final_hex_y"].str() + ")\n");
...
void teleport_unit_from_replay(
	const std::vector<map_location>& steps, bool continued_move, bool skip_ally_sighted, bool show_move)
{
	...
	const std::vector<map_location>& steps, bool continued_move, move_unit_spectator& move_spectator)
{
	...
	if(resources::gameboard->units().find(steps.front()) != resources::gameboard->units().end()) {
		const team& current_team
			= resources::gameboard->teams()[resources::gameboard->units().find(steps.front())->side() - 1];
