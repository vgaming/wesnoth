...
bool animate_unit_advancement(const map_location& loc, std::size_t choice, const bool& fire_event, const bool animate)
{
	...
	if(animate && u != resources::gameboard->units().end() && !video::headless()
		&& !resources::controller->is_skipping_replay()) {
		unit_animator animator;
...
int get_advancement_index(const unit& u, const std::string& id)
{
	...
	}
	{
		...
		auto pick_iter = std::find_if(
			amla_options.begin(), amla_options.end(), [&](const config& adv) { return adv["id"].str() == id; });
...
public:
	unit_advancement_choice(const map_location& loc, int total_opt, int side_num, bool force_dialog)
		: loc_(loc)
		, nb_options_(total_opt)
		, side_num_(side_num)
		, force_dialog_(force_dialog)
	{
	}
	...
	virtual config query_user(int /*side*/) const
	{
		...
		if(!video::headless()
			&& (force_dialog_
				|| (t.is_local_human() && !t.is_droid() && !t.is_idle() && (is_current_side || !is_mp)))) {
			...
			const ai::unit_advancements_aspect& ai_advancement
				= ai::manager::get_singleton().get_advancement_aspect_for_side(side_num_);
...
void advance_unit_at(const advance_unit_params& params)
{
	...
	for(int advacment_number = 0; advacment_number < 20; advacment_number++) {
		...
		config selected = mp_sync::get_user_choice("choose",
			unit_advancement_choice(
				params.loc_, unit_helper::number_of_possible_advances(*u), u->side(), params.force_dialog_),
			side_for);
		// calls actions::advance_unit.
		bool result = animate_unit_advancement(
