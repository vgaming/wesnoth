...
{
	sight_data(
		std::size_t viewed_id, const map_location& viewed_loc, std::size_t viewer_id, const map_location& viewer_loc)
		: seen_id(viewed_id)
		, seen_loc(viewed_loc)
		, sighter_id(viewer_id)
		, sighter_loc(viewer_loc)
	{
	}
...
inline void shroud_clearer::record_sighting(
	const unit& seen, const map_location& seen_loc, std::size_t sighter_id, const map_location& sighter_loc)
{
 ...
 */
bool shroud_clearer::clear_loc(team& tm,
	...
	bool /*instant*/)
{
	...
	for(const pathfind::paths::step& dest : sight.destinations) {
		...
		if(clear_loc(
			   view_team, dest.curr, view_loc, real_loc, viewer_id, !known, *enemy_count, *friend_count, spectator))
	...
	for(const map_location& dest : sight.edges) {
		...
		if(clear_loc(view_team, dest, view_loc, real_loc, viewer_id, !known, *enemy_count, *friend_count, spectator))
			cleared_something = true;
	}
	...
	bool instant)
{
	...
	return clear_unit(view_loc, view_team, viewer.underlying_id(), viewer.vision(),
		viewer.get_state(unit::STATE_SLOWED), viewer.movement_type().get_vision(), viewer.get_location(), known_units,
		enemy_count, friend_count, spectator, instant);
}
 ...
 *         locations in visual range were fogged/shrouded under shared vision/maps).
 */
bool shroud_clearer::clear_unit(const map_location& view_loc, team& view_team, const clearer_info& viewer, bool instant)
{
	// Locate the unit in question.
	unit_map::const_iterator find_it = resources::gameboard->units().find(viewer.underlying_id);
	const map_location& real_loc
		= find_it == resources::gameboard->units().end() ? map_location::null_location() : find_it->get_location();

	return clear_unit(view_loc, view_team, viewer.underlying_id, viewer.sight_range, viewer.slowed, *viewer.costs,
 ...
 *         locations in visual range were fogged/shrouded under shared vision/maps).
 */
bool shroud_clearer::clear_unit(
	const map_location& view_loc, const unit& viewer, bool can_delay, bool invalidate, bool instant)
{
	...
	if(!viewing_team.fog_or_shroud())
		return false;
	if(can_delay && !viewing_team.auto_shroud_updates() && viewer.side() == resources::controller->current_side())
		return false;
...
bool shroud_clearer::clear_dest(const map_location& dest, const unit& viewer)
{
	...
	bool cleared_something = clear_loc(viewing_team, dest, dest, real_loc, viewer_id, true, enemies, friends);
...
game_events::pump_result_t shroud_clearer::fire_events()
{
	...
	for(const sight_data& event : sight_list) {
		...
		const map_location& sight_loc
			= find_it == units.end() ? map_location::null_location() : find_it->get_location();
 ...
 */
{
	...
	for(const unit& viewer : resources::gameboard->units()) {
		...
		if(needs_event[index] && distances[index] != 0) {
			if(can_see(viewer, target_loc, &jamming_cache[index])) {
				...
				std::size_t viewer_distance = distance_between(target_loc, viewer.get_location());
	...
	for(std::size_t i = 0; i != teams_size; ++i)
		if(second_units[i] != nullptr) {
			resources::game_events->pump().raise(
				sighted_str, target_entity, game_events::entity_location(*second_units[i]));
		}
