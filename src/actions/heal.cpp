...
struct heal_unit
{
	heal_unit(unit& patient, const std::vector<unit*>& treaters, int healing, bool poison)
		: healed(patient)
		, healers(treaters)
		, amount(healing)
		, cure_poison(poison)
 ...
 */
POISON_STATUS poison_progress(int side, const unit& patient, std::vector<unit*>& healers)
{
...
int heal_amount(int side, const unit& patient, std::vector<unit*>& healers)
{
	...
	if(patient.side() == side) {
		// Village healing?
		update_healing(healing, harming, resources::gameboard->map().gives_healing(patient.get_location()));
...
void animate_heals(std::list<heal_unit>& unit_list)
{
	...
	while(!unit_list.empty()) {
		...
		for(std::list<heal_unit>::iterator check_it = unit_list.begin(); check_it != unit_list.end(); ++check_it) {
		...
		if(nearest->cure_poison)
			cure_text = nearest->healed.gender() == unit_race::FEMALE ? _("female^cured") : _("cured");

		// The heal (animated, then actual):
		unit_display::unit_healing(nearest->healed, nearest->healers, nearest->amount, cure_text);
