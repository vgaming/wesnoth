...
{
	DBG_NG << "finding location for unit with id=[" << cfg["id"] << "] placement=[" << cfg["placement"] << "] x=["
		   << cfg["x"] << "] y=[" << cfg["y"] << "] for side " << team_.side();
	...
	for(const std::string& place : placements) {
		...
		if(place == "recall") {
			...
			if(place == "leader_passable") {
				deprecated_message("placement=leader_passable", DEP_LEVEL::PREEMPTIVE, {1, 15, 0},
					"Please use placement=leader and passable=yes instead");
				pass = true;
			}
		}
		...
		// "map", "map_passable", and "map_overwrite".
			...
			if(place == "map_passable") {
				deprecated_message("placement=map_passable", DEP_LEVEL::PREEMPTIVE, {1, 15, 0},
					"Please use placement=map and passable=yes instead");
				pass = true;
			} else if(place == "map_overwrite") {
				deprecated_message("placement=map_overwrite", DEP_LEVEL::PREEMPTIVE, {1, 15, 0},
					"Please use placement=map and overwrite=yes instead");
				vacant = false;
			}
		}
		...
		if(loc.valid() && board_->map().on_board(loc)) {
			if(vacant) {
				loc = find_vacant_tile(loc, pathfind::VACANT_ANY, pass ? pass_check : nullptr, nullptr, board_);
			}
			if(loc.valid() && board_->map().on_board(loc)) {
				return loc;
			}
...
void unit_creator::add_unit(const config& cfg, const vconfig* vcfg)
{
	...
	if(!recall_list_element) {
		...
		if(loc.valid()) {
			...
			LOG_NG << "wanted to insert unit on recall list, but recall list for side " << (cfg)["side"]
				   << "already contains id=" << id;
			return;
		}
	}
}
