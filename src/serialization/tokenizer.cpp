...
const token& tokenizer::next_token()
{
	...
	case token::LEFT_ANGLE_BRACKET:
		// if there aren't double left angle brackets, there is no extra handling needed - this is just a regular left
		// angle bracket
		...
		// keep getting characters and appending them to the current token's value until either the file ends or double
		// right angle brackets are found finding the end of the file first is an error since double left angle brackets
		// must always be closed by double right angle brackets
		for(;;) {
			next_char();
			if(current_ == EOF) {
	...
	// the main difference is the need to handle INLINED_PREPROCESS_DIRECTIVE_CHAR since double quotes don't affect the
	// preprocessor
	...
	case token::UNDERSCORE:
		// this check seems off - there are certainly other non-alphanumeric characters that shouldn't mean anything -
		// but it looks like the parser handles those cases
	...
	default:
		...
		// not quite sure how this works with non-ascii text particularly since the parser doesn't reference
		// token_type::MISC but maybe the default handling does what's needed
...
void tokenizer::skip_comment()
{
	...
	// used to point to either textdomain_ or file_, and populate that field with the value following the respective
	// command
	...
	// else this turned out to not be a #textdomain or a #line, then this is a normal comment so just read off
	// characters until finding the next line or the end of the file
	else {
	not_a_command:
