 ...
 */
std::string get_child_id(const std::string& parent_id, const unsigned row, const unsigned col)
{
	...
	std::string result = parent_id + "_C_" + std::to_string(row) + '_' + std::to_string(col);
 ...
 */
std::string get_child_widget_id(const std::string& parent_id, const unsigned row, const unsigned col)
{
...
void debug_layout_graph::generate_dot_file(const std::string& generator, const unsigned domain)
{
	...
	const std::string filename = filename_base_ + id + std::to_string(++sequence_number_) + "-" + generator + ".dot";

	std::ofstream file(filename.c_str());

	file << "//Basic layout graph for window id '" << window_->id() << "' using definition '" << window_->definition_
		 << "'.\n"
		 << "digraph window {\n"
...
void debug_layout_graph::widget_generate_info(
	std::ostream& out, const widget* widget, const std::string& id, const bool embedded) const
{
	assert(!id.empty());

	out << "\t" << id << " [label=<<table border=\"0\" cellborder=\"1\" cellspacing=\"0\">";
	...
	if(!grid) {
		...
		const scrollbar_container* scrollbar_container = dynamic_cast<const class scrollbar_container*>(widget);

		if(scrollbar_container) {
			widget_generate_info(out, scrollbar_container->content_grid_.get(), id + "_C", true);
			out << "\t" << id << " -> " << id << "_C"
				<< " [label=\"(content)\"];\n";
		}
		...
		if(generator) {
			for(std::size_t i = 0; i < generator->get_item_count(); ++i) {
				const std::string child_id = id + "_I_" + std::to_string(i);

				widget_generate_info(out, &generator->item(i), child_id, true);

				out << "\t" << id << " -> " << child_id << " [label=\"(item)\"];\n";
			}
		}
	}
	if(grid) {
...
void debug_layout_graph::widget_generate_basic_info(std::ostream& out, const widget* widget) const
{
	std::string header_background = level_ & (SIZE_INFO | STATE_INFO) ? " bgcolor=\"gray\"" : "";
...
void debug_layout_graph::widget_generate_state_info(std::ostream& out, const widget* widget) const
{
	...
	if(scrollbar_container) {
		out << "<tr><td>\n"
			<< "vertical_scrollbar_mode_=" << scrollbar_container->vertical_scrollbar_mode_ << '\n'
			<< "</td></tr>\n"
			<< "<tr><td>\n"
			<< "horizontal_scrollbar_mode_=" << scrollbar_container->horizontal_scrollbar_mode_ << '\n'
			<< "</td></tr>\n";
	}
}

void debug_layout_graph::widget_generate_size_info(std::ostream& out, const widget* widget) const
{
	out << "<tr><td>\n"
...
void debug_layout_graph::grid_generate_info(std::ostream& out, const grid* grid, const std::string& parent_id) const
{
	...
	for(unsigned row = 0; row < grid->get_rows(); ++row) {
		for(unsigned col = 0; col < grid->get_cols(); ++col) {
			...
			widget_generate_info(out, widget, get_child_widget_id(parent_id, row, col));
		}
	}
	...
	for(unsigned row = 0; row < grid->get_rows(); ++row) {
		for(unsigned col = 0; col < grid->get_cols(); ++col) {
			child_generate_info(out, grid->get_child(row, col), get_child_id(parent_id, row, col));
		}
	}
	...
	for(unsigned row = 0; row < grid->get_rows(); ++row) {
		for(unsigned col = 0; col < grid->get_cols(); ++col) {
			// grid -> child
			out << "\t" << parent_id << " -> " << get_child_id(parent_id, row, col) << " [label=\"(" << row << ','
				<< col << ")\"];\n";

			// child -> widget
			out << "\t" << get_child_id(parent_id, row, col) << " -> " << get_child_widget_id(parent_id, row, col)
				<< ";\n";
		}
	}
}

void debug_layout_graph::child_generate_info(std::ostream& out, const grid::child& child, const std::string& id) const
{
	assert(!id.empty());
	...
	case grid::VERTICAL_ALIGN_BOTTOM:
		...
		break;
	default:
		out << "unknown value(" << ((flags & grid::VERTICAL_MASK) >> grid::VERTICAL_SHIFT) << ")";
	}
	...
	case grid::HORIZONTAL_ALIGN_RIGHT:
		...
		break;
	default:
		out << "unknown value(" << ((flags & grid::HORIZONTAL_MASK) >> grid::HORIZONTAL_SHIFT) << ")";
	}
