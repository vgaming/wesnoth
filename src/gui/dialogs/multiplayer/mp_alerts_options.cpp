...
#define SETUP_ITEMS(pref_item_sound, pref_item_notif, pref_item_lobby)                                                 \
	{                                                                                                                  \
		...
		connect_signal_mouse_left_click(                                                                               \
			*sound, [sound](auto&&...) { prefs::get().set_##pref_item_sound(sound->get_value_bool()); });              \
                                                                                                                       \
		...
		if(!desktop::notifications::available()) {                                                                     \
			...
			connect_signal_mouse_left_click(                                                                           \
				*notif, [notif](auto&&...) { prefs::get().set_##pref_item_notif(notif->get_value_bool()); });          \
		}                                                                                                              \
                                                                                                                       \
		...
		connect_signal_mouse_left_click(                                                                               \
			*lobby, [lobby](auto&&...) { prefs::get().set_##pref_item_lobby(lobby->get_value_bool()); });              \
	}
...
void mp_alerts_options::pre_show()
{
	...
	in_lobby = find_widget<toggle_button>(
		prefs_list::turn_changed_sound, false, true); // If we get a sound for this then don't remove this button
	in_lobby->set_visible(widget::visibility::invisible);
