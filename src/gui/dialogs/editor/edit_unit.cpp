	...
	, addon_id_(addon_id)
{
	...
	connect_signal<event::SDL_KEY_DOWN>(std::bind(&editor_edit_unit::signal_handler_sdl_key_down, this,
...
void editor_edit_unit::pre_show()
{
	...
	connect_signal_mouse_left_click(find_widget<button>("browse_unit_image"),
		std::bind(&editor_edit_unit::select_file, this, "data/core/images/units", "unit_image"));
	connect_signal_mouse_left_click(
		find_widget<button>("preview_unit_image"), std::bind(&editor_edit_unit::update_image, this, "unit_image"));
	connect_signal_mouse_left_click(find_widget<button>("browse_portrait_image"),
		std::bind(&editor_edit_unit::select_file, this, "data/core/images/portraits", "portrait_image"));
	connect_signal_mouse_left_click(find_widget<button>("preview_portrait_image"),
		std::bind(&editor_edit_unit::update_image, this, "portrait_image"));

	connect_signal_notify_modified(
		find_widget<text_box>("name_box"), std::bind(&editor_edit_unit::button_state_change, this));
	connect_signal_notify_modified(
		find_widget<text_box>("id_box"), std::bind(&editor_edit_unit::button_state_change, this));
	...
	const config& defense_attr
		= game_config_.mandatory_child("units").mandatory_child("movetype").mandatory_child("defense");
	...
	const config& resistances_attr
		= game_config_.mandatory_child("units").mandatory_child("movetype").mandatory_child("resistance");
	...
	connect_signal_mouse_left_click(find_widget<button>("browse_small_profile_image"),
		std::bind(&editor_edit_unit::select_file, this, "data/core/images/portraits", "small_profile_image"));
	connect_signal_mouse_left_click(find_widget<button>("preview_small_profile_image"),
		std::bind(&editor_edit_unit::update_image, this, "small_profile_image"));
	connect_signal_mouse_left_click(
		find_widget<button>("load_movetype"), std::bind(&editor_edit_unit::load_movetype, this));
	connect_signal_notify_modified(
		find_widget<slider>("resistances_slider"), std::bind(&editor_edit_unit::store_resistances, this));
	connect_signal_notify_modified(
		find_widget<menu_button>("resistances_list"), std::bind(&editor_edit_unit::update_resistances, this));
	connect_signal_mouse_left_click(find_widget<toggle_button>("resistances_checkbox"),
		std::bind(&editor_edit_unit::enable_resistances_slider, this));

	connect_signal_notify_modified(
		find_widget<slider>("defense_slider"), std::bind(&editor_edit_unit::store_defenses, this));
	connect_signal_notify_modified(
		find_widget<menu_button>("defense_list"), std::bind(&editor_edit_unit::update_defenses, this));
	connect_signal_mouse_left_click(
		find_widget<toggle_button>("defense_checkbox"), std::bind(&editor_edit_unit::enable_defense_slider, this));

	connect_signal_notify_modified(
		find_widget<slider>("movement_costs_slider"), std::bind(&editor_edit_unit::store_movement_costs, this));
	connect_signal_notify_modified(
		find_widget<menu_button>("movement_costs_list"), std::bind(&editor_edit_unit::update_movement_costs, this));
	connect_signal_mouse_left_click(find_widget<toggle_button>("movement_costs_checkbox"),
	...
	// Connect signals
	connect_signal_mouse_left_click(find_widget<button>("browse_attack_image"),
		std::bind(&editor_edit_unit::select_file, this, "data/core/images/attacks", "attack_image"));
	connect_signal_mouse_left_click(
		find_widget<button>("preview_attack_image"), std::bind(&editor_edit_unit::update_image, this, "attack_image"));
	connect_signal_notify_modified(
		find_widget<menu_button>("atk_list"), std::bind(&editor_edit_unit::select_attack, this));
	connect_signal_mouse_left_click(find_widget<button>("atk_new"), std::bind(&editor_edit_unit::add_attack, this));
	connect_signal_mouse_left_click(
		find_widget<button>("atk_delete"), std::bind(&editor_edit_unit::delete_attack, this));
	connect_signal_mouse_left_click(find_widget<button>("atk_next"), std::bind(&editor_edit_unit::next_attack, this));
	connect_signal_mouse_left_click(find_widget<button>("atk_prev"), std::bind(&editor_edit_unit::prev_attack, this));
...
void editor_edit_unit::select_file(const std::string& default_dir, const std::string& id_stem)
{
	...
	dlg.set_title(_("Choose File")).set_ok_label(_("Select")).set_path(default_dir).set_read_only(true);
...
void editor_edit_unit::load_unit_type()
{
	...
	set_selected_from_string(find_widget<menu_button>("race_list"), race_list_, type->race_id());

	set_selected_from_string(
		find_widget<menu_button>("alignment_list"), align_list_, unit_alignments::get_string(type->alignment()));
	...
	set_selected_from_string(find_widget<menu_button>("movetype_list"), movetype_list_, type->movement_type_id());
	...
	set_selected_from_string(find_widget<menu_button>("usage_list"), usage_type_list_, type->usage());
...
void editor_edit_unit::update_resistances()
{
	find_widget<slider>("resistances_slider")
		.set_value(100 - resistances_[find_widget<menu_button>("resistances_list").get_value_string()].to_int());
...
void editor_edit_unit::update_defenses()
{
	find_widget<slider>("defense_slider")
		.set_value(100 - defenses_[find_widget<menu_button>("defense_list").get_value_string()].to_int());
...
void editor_edit_unit::update_movement_costs()
{
	find_widget<slider>("movement_costs_slider")
		.set_value(movement_[find_widget<menu_button>("movement_costs_list").get_value_string()].to_int());
...
void editor_edit_unit::update_attacks()
{
	...
	set_selected_from_string(find_widget<combobox>("attack_type_list"), resistances_list_, attack["type"]);

	find_widget<multimenu_button>("weapon_specials_list").select_options(attacks_.at(selected_attack_ - 1).first);
...
void editor_edit_unit::load_movetype()
{
	...
	for(const auto& movetype : game_config_.mandatory_child("units").child_range("movetype")) {
...
void editor_edit_unit::update_image(const std::string& id_stem)
{
	...
	if(rel_path.size() > 0) {
		...
		if(img_size.x > scale_size) {
			rel_path.append(
				"~SCALE(" + std::to_string(scale_size) + "," + std::to_string(scale_size * aspect_ratio) + ")");
		} else if(img_size.y > scale_size) {
			rel_path.append(
				"~SCALE(" + std::to_string(scale_size / aspect_ratio) + "," + std::to_string(scale_size) + ")");
		}
	}
...
{
	const std::string& message = _("Unsaved changes will be lost. Do you want to leave?");
	if(gui2::show_message(_("Confirm"), message, message::yes_no_buttons) == gui2::retval::OK) {
...
void editor_edit_unit::signal_handler_sdl_key_down(
