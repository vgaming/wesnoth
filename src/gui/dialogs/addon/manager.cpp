...
{
	explicit filter_transform(const std::vector<std::string>& filtertext)
...
std::string make_display_dependencies(
	const std::string& addon_id, const addons_list& addons_list, const addons_tracking_list& addon_states)
{
...
static std::string describe_status_verbose(const addon_tracking_info& state)
{
	...
	switch(state.state) {
	case ADDON_NONE:
		s = !state.can_publish ? _("addon_state^Not installed") : _("addon_state^Published, not installed");
		break;
	case ADDON_INSTALLED:
		s = !state.can_publish ? _("addon_state^Installed") : _("addon_state^Published");
		break;
	case ADDON_NOT_TRACKED:
		s = !state.can_publish ? _("addon_state^Installed, not tracking local version")
	...
	case ADDON_INSTALLED_LOCAL_ONLY:
		s = !state.can_publish ? _("addon_state^Installed, not ready to publish") : _("addon_state^Ready to publish");
		break;
	case ADDON_INSTALLED_BROKEN:
		s = !state.can_publish ? _("addon_state^Installed, broken") : _("addon_state^Published, broken");
		break;
	default:
...
void addon_manager::pre_show()
{
		...
		if(addr_box) {
			if(!client_.server_id().empty()) {
				auto full_id = formatter()
					<< client_.addr() << ' ' << font::unicode_em_dash << ' ' << client_.server_id();
	...
	list.set_install_function(std::bind(&addon_manager::install_addon, this, std::placeholders::_1));
	list.set_uninstall_function(std::bind(&addon_manager::uninstall_addon, this, std::placeholders::_1));
	list.set_update_function(std::bind(&addon_manager::update_addon, this, std::placeholders::_1));

	list.set_publish_function(std::bind(&addon_manager::publish_addon, this, std::placeholders::_1));
	list.set_delete_function(std::bind(&addon_manager::delete_addon, this, std::placeholders::_1));
	...
	for(const auto& f : status_filter_types_) {
		if(f.first == FILTER_ALL) {
			status_filter_entries.emplace_back(
				"label", t_string(f.second, GETTEXT_DOMAIN) + " (" + std::to_string(addons_.size()) + ")");
		} else if(f.first == FILTER_INSTALLED) {
			status_filter_entries.emplace_back(
				"label", t_string(f.second, GETTEXT_DOMAIN) + " (" + std::to_string(installed_addons().size()) + ")");
		} else {
	...
	connect_signal_notify_modified(status_filter, std::bind(&addon_manager::apply_filters, this));
	...
	std::map<ADDON_TYPE, int> type_counts = {{ADDON_SP_CAMPAIGN, 0}, {ADDON_SP_SCENARIO, 0}, {ADDON_SP_MP_CAMPAIGN, 0},
		{ADDON_MP_CAMPAIGN, 0}, {ADDON_MP_SCENARIO, 0}, {ADDON_MP_MAPS, 0}, {ADDON_MP_ERA, 0}, {ADDON_MP_FACTION, 0},
		{ADDON_MOD, 0}, {ADDON_CORE, 0}, {ADDON_THEME, 0}, {ADDON_MEDIA, 0}, {ADDON_OTHER, 0}, {ADDON_UNKNOWN, 0}};
	...
	for(const auto& f : type_filter_types_) {
		type_filter_entries.emplace_back("label",
			t_string(f.second, GETTEXT_DOMAIN) + " (" + std::to_string(type_counts[f.first]) + ")", "checkbox", false);
	}

	type_filter.set_values(type_filter_entries);

	connect_signal_notify_modified(type_filter, std::bind(&addon_manager::apply_filters, this));
	...
	connect_signal_notify_modified(language_filter, std::bind(&addon_manager::apply_filters, this));
	...
	connect_signal_notify_modified(order_dropdown, std::bind(&addon_manager::order_addons, this));
	...
	connect_signal_mouse_left_click(
		find_widget<button>("install"), std::bind(&addon_manager::install_selected_addon, this));

	connect_signal_mouse_left_click(
		find_widget<button>("uninstall"), std::bind(&addon_manager::uninstall_selected_addon, this));

	connect_signal_mouse_left_click(
		find_widget<button>("update"), std::bind(&addon_manager::update_selected_addon, this));

	connect_signal_mouse_left_click(
		find_widget<button>("publish"), std::bind(&addon_manager::publish_selected_addon, this));

	connect_signal_mouse_left_click(
		find_widget<button>("delete"), std::bind(&addon_manager::delete_selected_addon, this));

	if(game_config::addon_server_info) {
		connect_signal_mouse_left_click(find_widget<button>("info"), std::bind(&addon_manager::info, this));
	} else {
		find_widget<button>("info").set_visible(false);
	}

	connect_signal_mouse_left_click(
		find_widget<button>("update_all"), std::bind(&addon_manager::update_all_addons, this));

	connect_signal_mouse_left_click(find_widget<button>("show_help"), std::bind(&addon_manager::show_help, this));

	if(stacked_widget* stk = find_widget<stacked_widget>("main_stack", false, false)) {
		button& btn = find_widget<button>("details_toggle");
		connect_signal_mouse_left_click(
			btn, std::bind(&addon_manager::toggle_details, this, std::ref(btn), std::ref(*stk)));

		stk->select_layer(0);

		connect_signal_notify_modified(stk->get_layer_grid(1)->find_widget<menu_button>("version_filter"),
			std::bind(&addon_manager::on_selected_version_change, this));
	} else {
		connect_signal_notify_modified(
			find_widget<menu_button>("version_filter"), std::bind(&addon_manager::on_selected_version_change, this));
	}
	...
	list.set_callback_order_change(
		std::bind(&addon_manager::on_order_changed, this, std::placeholders::_1, std::placeholders::_2));
...
void addon_manager::load_addon_list()
{
	...
	if(addons_.empty()) {
		show_transient_message(
			_("No Add-ons Available"), _("There are no add-ons available for download from this server."));
	}
...
boost::dynamic_bitset<> addon_manager::get_name_filter_visibility() const
{
	...
	for(const auto& a : addons_) {
		const config& addon_cfg = *std::find_if(
			addon_cfgs.begin(), addon_cfgs.end(), [&a](const config& cfg) { return cfg["name"] == a.first; });
...
boost::dynamic_bitset<> addon_manager::get_status_filter_visibility() const
{
	...
	for(const auto& a : addons_) {
		...
		res.push_back((selection == FILTER_ALL)
...
void addon_manager::apply_filters()
{
	...
	boost::dynamic_bitset<> res = get_status_filter_visibility() & get_tag_filter_visibility()
		& get_type_filter_visibility() & get_lang_filter_visibility() & get_name_filter_visibility();
...
void addon_manager::uninstall_addon(const addon_info& addon)
{
	if(have_addon_pbl_info(addon.id) || have_addon_in_vcs_tree(addon.id)) {
		show_error_message(_("The following add-on appears to have publishing or version control information stored "
							 "locally, and will not be removed:")
	...
	if(!success) {
		gui2::show_error_message(
			_("The following add-on could not be deleted properly:") + " " + addon.display_title_full());
	} else {
...
void addon_manager::publish_addon(const addon_info& addon)
{
	...
	if(version_to_publish <= tracking_info_[addon_id].remote_version) {
		const int res = gui2::show_message(_("Warning"),
			_("The remote version of this add-on is greater or equal to the version being uploaded. Do you really wish "
			  "to continue?"),
	...
	// if the passphrase isn't provided from the _server.pbl, try to pre-populate it from the preferences before
	// prompting for it
	...
	if(!::image::exists(cfg["icon"].str())) {
		...
		if(!client_.upload_addon(
			   addon_id, server_msg, cfg, tracking_info_[addon_id].state == ADDON_INSTALLED_LOCAL_ONLY)) {
			const std::string& msg
				= _("The add-on was rejected by the server:") + "\n\n" + client_.get_last_server_error();
...
{
	const std::string addon_id = addon.id;
	const std::string& text = VGETTEXT("Deleting '$addon|' will permanently erase its download and upload counts on "
	...
	case ADDON_INSTALLED:
		if(!tracking_info_[addon.id].can_publish) {
			...
			int res = gui2::show_message(_("Uninstall add-on"),
				VGETTEXT("Do you want to uninstall '$addon|'?", symbols), gui2::dialogs::message::ok_cancel_buttons);
			if(res == gui2::retval::OK) {
...
static std::string format_addon_time(const std::chrono::system_clock::time_point& time)
{
	...
	const std::string format = prefs::get().use_twelve_hour_clock_format()
		// TRANSLATORS: Month + day of month + year + 12-hour time, eg 'November 02 2021, 1:59 PM'. Format for your
		// locale. Format reference:
		// https://www.boost.org/doc/libs/1_85_0/doc/html/date_time/date_time_io.html#date_time.format_flags
		? _("%B %d %Y, %I:%M %p")
		// TRANSLATORS: Month + day of month + year + 24-hour time, eg 'November 02 2021, 13:59'. Format for your
		// locale. Format reference:
		// https://www.boost.org/doc/libs/1_85_0/doc/html/date_time/date_time_io.html#date_time.format_flags
		: _("%B %d %Y, %H:%M");
...
void addon_manager::on_addon_select()
{
	...
	parent->find_widget<styled_widget>("dependencies")
		.set_label(!info->depends.empty() ? make_display_dependencies(info->id, addons_, tracking_info_)
	...
	parent->find_widget<styled_widget>("translations")
		.set_label(!languages.empty() ? languages : _("translations^None"));
