...
void statistics_dialog::pre_show()
{
	...
	title.set_label((formatter() << title.get_label()
								 << (current_team_.side_name().empty() ? "" : " (" + current_team_.side_name() + ")"))
			.str());
	...
	connect_signal_notify_modified(scenario_menu, std::bind(&statistics_dialog::on_scenario_select, this));
	...
	connect_signal_notify_modified(stat_list, std::bind(&statistics_dialog::on_primary_list_select, this));
...
void statistics_dialog::add_stat_row(
	const std::string& type, const statistics_t::stats::str_int_map& value, const bool has_cost)
{
	listbox& stat_list = find_widget<listbox>("stats_list_main");
...
{
	// This is displayed as a sum or difference, not as "actual/expected", to prevent the string in the next column,
	// str2.str(), from being mistaken for the result of the division.
	if(expected == 0) {
		str << "+0% (0 + 0)";
	} else {
...
void statistics_dialog::add_damage_row(const std::string& type,
	...
	const bool show_this_turn)
{
	...
	if(show_this_turn) {
		...
		// TODO: Setting the label to "" causes "This Turn" not to be drawn when changing back to the current scenario
		// view, so set the label to " " (a single space) instead.
		label& this_turn_header = find_widget<label>("damage_this_turn_header");
...
static hitrate_table_element tally(const statistics_t::stats::hitrate_map& by_cth, const bool more_is_better)
{
	...
	for(const auto& i : by_cth) {
		...
		tooltip << "\n"
				<< cth << "%: " << get_probability_string(i.second.hits / static_cast<double>(i.second.strikes))
				<< "% (N=" << i.second.strikes << ")";
	}
	...
	{
		config defender_cfg("id", "statistics_dialog_dummy_defender", "hide_help", true, "do_not_list", true,
		...
		for(const auto& i : by_cth) {
			int cth = i.first;
			config attacker_cfg("id", "statistics_dialog_dummy_attacker" + std::to_string(cth), "hide_help", true,
			...
			auto attack = std::make_shared<attack_type>(config(
				"type", "blade", "range", "melee", "name", "dummy attack", "damage", 1, "number", i.second.strikes));

			battle_context_unit_stats attacker_bc(&attacker_type, attack, true, &defender_type, nullptr, 100 - cth);
			defender_bc
				= battle_context_unit_stats(&defender_type, nullptr, false, &attacker_type, attack, 0 /* not used */);
		...
		const auto chance_of_exactly_N_hits
			= [&final_hp_dist](int n) { return final_hp_dist[final_hp_dist.size() - 1 - n]; };
...
void statistics_dialog::add_hits_row(const std::string& type,
	...
	const bool show_this_turn)
{
	...
	const auto tooltip_static_part
...
void statistics_dialog::update_lists()
{
	...
	add_damage_row(_("Inflicted"), stats.damage_inflicted, stats.expected_damage_inflicted, stats.turn_damage_inflicted,
		stats.turn_expected_damage_inflicted, show_this_turn);
	add_hits_row(_("Inflicted"), true, stats.by_cth_inflicted, stats.turn_by_cth_inflicted, show_this_turn);

	add_damage_row(_("Taken"), stats.damage_taken, stats.expected_damage_taken, stats.turn_damage_taken,
		stats.turn_expected_damage_taken, show_this_turn);
	add_hits_row(_("Taken"), false, stats.by_cth_taken, stats.turn_by_cth_taken, show_this_turn);
}
...
void statistics_dialog::on_primary_list_select()
{
	...
	for(const auto& i : *main_stat_table_[selected_row]) {
		...
		item["label"]
			= (formatter() << type->image() << "~RC(" << type->flag_rgb() << ">" << current_team_.color() << ")").str();
