...
namespace
{ // Some functions for use with parameters::eval.
...
const movetype::terrain_info::parameters movetype::terrain_defense::params_min_(0, 100, config_to_min, false, true);
const movetype::terrain_info::parameters movetype::terrain_defense::params_max_(0, 100, config_to_max, false, false);
...
private:
	...
	int calc_value(
		const t_translation::terrain_code& terrain, const terrain_info* fallback, unsigned recurse_count) const;
 ...
 */
bool movetype::terrain_info::data::config_has_changes(const config& new_values, bool overwrite) const
{
	if(overwrite) {
 ...
 */
void movetype::terrain_info::data::write(config& out_cfg, const std::string& child_name) const
{
	if(cfg_.empty())
		return;
	...
	const t_translation::terrain_code& terrain, const terrain_info* fallback, unsigned recurse_count) const
{
	...
	if(recurse_count > 100) {
		ERR_CF << "infinite terrain_info recursion on " << (params_.use_move ? "movement" : "defense") << ": "
	...
	if(game_config_manager::get()) {
		tdata = game_config_manager::get()
					->terrain_types(); // This permits to get terrain info in unit help pages from the help in title
									   // screen, even if there is no residual gamemap object
	}
	assert(tdata);

	// Get a list of underlying terrains.
	const t_translation::ter_list& underlying
		= params_.use_move ? tdata->underlying_mvt_terrain(terrain) : tdata->underlying_def_terrain(terrain);
	...
	if(terrain_type::is_indivisible(terrain, underlying)) {
		...
		if(result < params_.min_value) {
			WRN_CF << "Terrain '" << terrain << "' has evaluated to " << result << " ("
				   << (params_.use_move ? "cost" : "defense") << "), which is less than " << params_.min_value
				   << "; resetting to " << params_.min_value << ".";
			result = params_.min_value;
		}
		if(result > params_.max_value) {
			WRN_CF << "Terrain '" << terrain << "' has evaluated to " << result << " ("
				   << (params_.use_move ? "cost" : "defense") << "), which is more than " << params_.max_value
		...
		if(underlying.front() == t_translation::MINUS)
			...
			result = result == params_.max_value ? params_.min_value : params_.max_value;
 ...
 * @param[in] fallback  Used as a backup in case we are asked for data we do not have (think vision costs falling back
 * to movement costs).
 */
movetype::terrain_info::terrain_info(const parameters& params, const terrain_info* fallback)
 ...
 * to movement costs).
 */
movetype::terrain_info::terrain_info(const config& cfg, const parameters& params, const terrain_info* fallback)
 ...
 * to movement costs).
 */
movetype::terrain_info::terrain_info(const terrain_info& that, const terrain_info* fallback)
	: fallback_(fallback)
{
...
movetype::terrain_info::terrain_info(terrain_info&& that, const terrain_info* fallback)
 ...
 */
void movetype::terrain_info::merge(
	const config& new_values, bool overwrite, const std::vector<movetype::terrain_info*>& dependants)
{
 ...
 * @param[in]  child_name  If not empty, create and write to a child config with this tag.
 * @param[in]  merged      If true, our data will be merged with our fallback's, and it is possible an empty child will
 * be created. If false, data will not be merged, and an empty child will not be created.
 */
void movetype::terrain_info::write(config& cfg, const std::string& child_name, bool merged) const
{
	if(!merged)
